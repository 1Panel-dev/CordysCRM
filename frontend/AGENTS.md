...
- 代码必须包含详略得当的注释，重点帮助维护者理解组件职责、业务意图和非直观行为。
- 复杂 Props、Emits、组合式函数和共享工具应说明使用约束、数据来源及副作用。
- 权限判断、兼容处理、复杂表单联动、异步竞态和特殊数据转换应注释其业务原因。
- 不为简单赋值、显而易见的事件处理或名称清晰的计算属性添加逐行翻译式注释。
- 较长模板可用简短注释标识主要区域；不要为每个标签添加说明，也不要保留注释掉的旧模板。
- 修改行为时同步更新注释；与实现不一致的注释应立即修正或删除。
- import、响应式状态、计算属性、监听器、请求方法、事件处理和生命周期等不同逻辑块之间应合理空一行。
- 函数内部的参数准备、请求调用、结果转换、状态更新和异常处理等阶段之间应按逻辑块留白。
- 紧密相关的声明或连续操作保持在一起，不在每行代码之间机械插入空行。
- 单个组件或函数需要大量分段注释才能理解时，应优先拆分组件、组合式函数或具名辅助函数。
...
- 已运行对应包或全部工作区的生产构建。
- 复杂交互和特殊业务规则具有有效注释，且不存在过期或被注释掉的旧代码。
- 脚本与函数已按逻辑块合理留白，相关声明保持紧凑。
