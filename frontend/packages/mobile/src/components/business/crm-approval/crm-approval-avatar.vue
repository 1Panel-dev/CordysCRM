<template>
  ...
  >
    <van-image v-if="isShowAutoBot" round :size="props.size" class="cursor-pointer" @click="emit('toggleActive')">
      <CrmIcon name="iconicon_bot" :size="18" class="text-[var(--text-n10)]" />
    </van-image>
    <CrmAvatar
      v-else
    ...
    />
    <div v-if="props.approver?.approveResult" class="crm-approver-avatar-list__status">
      <CrmIcon
        :name="getStatusIcon(props.approver.approveResult)"
        :size="14"
        :color="getStatusColor(props.approver.approveResult)"
      />
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  function getStatusColor(status: ProcessStatusEnum) {
    if (props.signNode) {
