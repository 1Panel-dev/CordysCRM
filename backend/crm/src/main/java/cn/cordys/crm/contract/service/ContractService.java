...
public class ContractService {
  ...
  @Resource private ContractFieldService contractFieldService;
  @Resource private BaseMapper<Contract> contractMapper;
  @Resource private BaseService baseService;
  @Resource private ModuleFormService moduleFormService;
  @Resource private BaseMapper<ContractSnapshot> snapshotBaseMapper;
  @Resource private ExtContractMapper extContractMapper;
  @Resource private ModuleFormCacheService moduleFormCacheService;
  @Resource private PermissionCache permissionCache;
  @Resource private BaseMapper<Customer> customerBaseMapper;
  ...
  @Resource private BaseMapper<MessageTaskConfig> messageTaskConfigMapper;
  @Resource private DataScopeService dataScopeService;
  @Resource private BaseMapper<ContractPaymentRecord> contractPaymentRecordMapper;
   ...
   */
  @OperationLog(
      module = LogModule.CONTRACT_INDEX,
      type = LogType.ADD,
  ...
  public Contract add(ContractAddRequest request, String operatorId, String orgId) {
    ...
    baseService.handleAddLogWithSubTable(
        contract, moduleFields, Translator.get("products_info"), moduleFormConfigDTO);

    // 保存表单配置快照
    List<BaseModuleFieldValue> resolveFieldValues =
        moduleFormService.resolveSnapshotFields(
            moduleFields, moduleFormConfigDTO, contractFieldService, contract.getId());
  ...
  private String createContractNumber(ModuleFormConfigDTO moduleFormConfigDTO, String orgId) {
    BaseField numberField =
        moduleFormConfigDTO.getFields().stream()
            .filter(
                field -> field.isSerialNumber() && StringUtils.isNotEmpty(field.getBusinessKey()))
            .findFirst()
            .orElse(null);
    ...
    if (numberField != null) {
      ...
      return serialNumGenerator.generateByRules(
          ((SerialNumberField) numberField).getSerialNumberRules(),
          orgId,
  ...
  private void saveSnapshot(
      Contract contract, ModuleFormConfigDTO moduleFormConfigDTO, ContractGetResponse response) {
  ...
  public ContractGetResponse getWithDataPermissionCheck(String id, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, getResponse.getOwner(), PermissionConstants.CONTRACT_READ);
    return getResponse;
  }

  public ContractGetResponse getSnapshotWithDataPermissionCheck(
      String id, String userId, String orgId) {
  ...
  private ContractGetResponse get(
      Contract contract,
      List<BaseModuleFieldValue> contractFields,
      ModuleFormConfigDTO contractFormConfig) {
    ContractGetResponse contractGetResponse =
        BeanUtils.copyBean(new ContractGetResponse(), contract);
    contractGetResponse = baseService.setCreateUpdateOwnerUserName(contractGetResponse);

    String id = contract.getId();
    // 获取模块字段
    moduleFormService.processBusinessFieldValues(
        contractGetResponse, contractFields, contractFormConfig);
    contractFields =
        contractFieldService
            .setBusinessRefFieldValue(
                List.of(contractGetResponse),
                moduleFormService.getFlattenFormFields(
                    FormKey.CONTRACT.getKey(), contract.getOrganizationId()),
                new HashMap<>(Map.of(id, contractFields)))
            .get(id);
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            contractGetResponse, ContractGetResponse::getOwner, ContractGetResponse::getOwnerName);
    ...
    if (customer != null) {
      ...
      optionMap.put(
          BusinessModuleField.CONTRACT_CUSTOMER_NAME.getBusinessKey(),
    ...
    if (contractGetResponse.getOwner() != null) {
      UserDeptDTO userDeptDTO =
          baseService.getUserDeptMapByUserId(
              contractGetResponse.getOwner(), contract.getOrganizationId());
      if (userDeptDTO != null) {
    ...
    // 附件信息
    contractGetResponse.setAttachmentMap(
        moduleFormService.getAttachmentMap(contractFormConfig, contractFields));
  ...
  public ContractGetResponse get(String id) {
    ...
    List<BaseModuleFieldValue> contractFields =
        contractFieldService.getModuleFieldValuesByResourceId(id);
  ...
  public Contract update(ContractUpdateRequest request, String userId, String orgId) {
    ...
    ModuleFormConfigDTO saveModuleFormConfigDTO =
        JSON.parseObject(JSON.toJSONString(moduleFormConfigDTO), ModuleFormConfigDTO.class);
    Optional.ofNullable(oldContract)
        .ifPresentOrElse(
            item -> {
              List<BaseModuleFieldValue> originFields =
                  contractFieldService.getModuleFieldValuesByResourceId(request.getId());
              ...
              List<ContractSnapshot> contractSnapshots =
              ...
              if (CollectionUtils.isNotEmpty(contractSnapshots)) {
                ...
                if (first != null) {
                  ContractGetResponse response =
                      JSON.parseObject(first.getContractValue(), ContractGetResponse.class);
                  List<BaseModuleFieldValue> originModuleFields = response.getModuleFields();
                  originModuleFields.add(
                      new BaseModuleFieldValue("products", response.getProducts()));
  ...
  private void updateFields(
      List<BaseModuleFieldValue> moduleFields, Contract contract, String orgId, String userId) {
   ...
   * @return
   */
  public PagerWithOption<List<ContractListResponse>> list(
    ...
    List<ContractListResponse> list =
        extContractMapper.list(request, orgId, userId, deptDataPermission, source);
  ...
  private Map<String, List<OptionDTO>> buildOptionMap(
      List<ContractListResponse> list,
      List<ContractListResponse> buildList,
      ModuleFormConfigDTO formConfig) {
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(list, ContractListResponse::getModuleFields);
    // 获取选项值对应的 option
    Map<String, List<OptionDTO>> optionMap =
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList, ContractListResponse::getOwner, ContractListResponse::getOwnerName);
    optionMap.put(BusinessModuleField.CONTRACT_OWNER.getBusinessKey(), ownerFieldOption);
    return optionMap;
  }
  ...
  public List<ContractListResponse> buildList(List<ContractListResponse> list, String orgId) {
    ...
    List<String> contractIds =
        list.stream().map(ContractListResponse::getId).collect(Collectors.toList());
    Map<String, List<BaseModuleFieldValue>> contractFiledMap =
        contractFieldService.getResourceFieldMap(contractIds, true);
    Map<String, List<BaseModuleFieldValue>> resolvefieldValueMap =
        contractFieldService.setBusinessRefFieldValue(
            list,
            moduleFormService.getFlattenFormFields(FormKey.CONTRACT.getKey(), orgId),
            contractFiledMap);

    List<String> ownerIds = list.stream().map(ContractListResponse::getOwner).distinct().toList();
  ...
  public ModuleFormConfigDTO getFormSnapshot(String id, String orgId) {
    ...
    if (snapshot != null) {
      ...
      moduleFormConfigDTO =
          moduleFormCacheService.getBusinessFormConfig(FormKey.CONTRACT.getKey(), orgId);
    }
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.CONTRACT_READ, rolePermissions);
  }
  ...
  public void updateStage(ContractStageRequest request, String userId, String orgId) {
    ...
    oldMap.put(
        "contractStage", Translator.get("contract.stage." + contract.getStage().toLowerCase()));
    ...
    LogDTO logDTO =
        new LogDTO(
            ...
            userId,
            LogType.UPDATE,
            LogModule.CONTRACT_INDEX,
            contract.getName());
    Map<String, String> newMap = new HashMap<>();
    newMap.put(
        "contractStage", Translator.get("contract.stage." + request.getStage().toLowerCase()));
    ...
    if (Strings.CI.equals(request.getStage(), ContractStage.VOID.name())
        || Strings.CI.equals(request.getStage(), ContractStage.ARCHIVED.name())) {
      String event =
          Strings.CI.equals(request.getStage(), ContractStage.VOID.name())
              ? NotificationConstants.Event.CONTRACT_VOID
              : NotificationConstants.Event.CONTRACT_ARCHIVED;
      Customer customer = customerBaseMapper.selectByPrimaryKey(contract.getCustomerId());
  ...
  private void sendNotice(
      Contract contract, String userId, String orgId, String event, String customerName) {
    // 查询通知配置的接收范围
    List<String> receiveUserIds = new ArrayList<>();
    List<MessageTaskConfig> messageTaskConfigList =
    ...
    if (CollectionUtils.isNotEmpty(messageTaskConfigList)) {
      ...
      MessageTaskConfigDTO messageTaskConfigDTO =
          JSON.parseObject(messageTaskConfig.getValue(), MessageTaskConfigDTO.class);
      receiveUserIds =
          commonNoticeSendService.getNoticeReceiveUserIds(
              messageTaskConfigDTO, contract.getCreateUser(), contract.getOwner(), orgId);
    } else {
      // 默认通知创建人
  ...
  public CustomerContractStatisticResponse calculateContractStatisticByCustomerId(
      String customerId, String userId, String orgId, DeptDataPermissionDTO deptDataPermission) {
    return extContractMapper.calculateContractStatisticByCustomerId(
        customerId, userId, orgId, deptDataPermission);
  }
  ...
  public void approvalContract(ContractApprovalRequest request, String userId, String orgId) {
    ...
    LogDTO logDTO =
        getApprovalLogDTO(
            orgId, request.getId(), userId, contract.getName(), state, request.getApprovalStatus());
    logService.add(logDTO);
  }
  ...
  public String revoke(String id, String userId, String orgId) {
    ...
    if (!Strings.CI.equals(contract.getCreateUser(), userId)
        || !Strings.CI.equals(contract.getApprovalStatus(), ApprovalState.APPROVING.toString())) {
    ...
    LogDTO logDTO =
        getApprovalLogDTO(
            ...
            userId,
            contract.getName(),
            originApprovalStatus,
            ApprovalState.REVOKED.toString());
    logService.add(logDTO);
   ...
   * @param orgId
   */
  public BatchAffectSkipResponse batchApprovalContract(
      ContractApprovalBatchRequest request, String userId, String orgId) {
    List<String> ids =
        extContractMapper.selectByStatusAndIds(
            request.getIds(), ContractApprovalStatus.APPROVING.name());

    if (CollectionUtils.isEmpty(ids)) {
      return BatchAffectSkipResponse.builder()
          .success(0)
          .fail(0)
          .skip(request.getIds().size())
          .build();
    }
    LambdaQueryWrapper<ContractSnapshot> wrapper = new LambdaQueryWrapper<>();
    ...
    Map<String, ContractSnapshot> snapshotsMaps =
        contractSnapshots.stream()
            .collect(Collectors.toMap(ContractSnapshot::getContractId, Function.identity()));

    List<LogDTO> logs = new ArrayList<>();
    SqlSession sqlSession = sqlSessionFactory.openSession(ExecutorType.BATCH);
    ExtContractMapper batchUpdateMapper = sqlSession.getMapper(ExtContractMapper.class);
    ExtContractSnapshotMapper snapshotMapper =
        sqlSession.getMapper(ExtContractSnapshotMapper.class);

    ids.forEach(
        id -> {
          batchUpdateMapper.updateStatus(
              id, request.getApprovalStatus(), userId, System.currentTimeMillis());
          ContractSnapshot contractSnapshot = snapshotsMaps.get(id);
          ContractGetResponse response =
              JSON.parseObject(contractSnapshot.getContractValue(), ContractGetResponse.class);
          ...
          LogDTO logDTO =
              getApprovalLogDTO(
                  orgId, id, userId, response.getName(), state, request.getApprovalStatus());
          logs.add(logDTO);
        });
    ...
    return BatchAffectSkipResponse.builder()
        .success(ids.size())
        .fail(0)
        .skip(request.getIds().size() - ids.size())
        .build();
  }

  private LogDTO getApprovalLogDTO(
      String orgId, String id, String userId, String response, String state, String newState) {
    LogDTO logDTO =
        new LogDTO(orgId, id, userId, LogType.APPROVAL, LogModule.CONTRACT_INDEX, response);
    Map<String, String> oldMap = new HashMap<>();
    oldMap.put("approvalStatus", Translator.get("contract.approval_status." + state.toLowerCase()));
    logDTO.setOriginalValue(oldMap);
    Map<String, String> newMap = new HashMap<>();
    newMap.put(
        "approvalStatus", Translator.get("contract.approval_status." + newState.toLowerCase()));
    logDTO.setModifiedValue(newMap);
    return logDTO;
  }
  ...
  public List<FilterCondition> getDefaultSourceFilters() {
    ...
    stageCondition.setValue(
        List.of(
            ContractStage.PENDING_SIGNING.name(),
            ContractStage.SIGNED.name(),
            ContractStage.IN_PROGRESS.name(),
            ContractStage.COMPLETED_PERFORMANCE.name()));
  ...
  private BigDecimal sumContractRecordAmount(String contractId) {
    ...
    List<ContractPaymentRecord> contractPaymentRecords =
        contractPaymentRecordMapper.selectListByLambda(paymentRecordWrapper);
