...
public class CustomerService {
  ...
  @Resource private CustomerFieldService customerFieldService;
  @Resource private CustomerCollaborationService customerCollaborationService;
  @Resource private CustomerOwnerHistoryService customerOwnerHistoryService;
  @Resource private CustomerPoolService customerPoolService;
  @Resource private BaseMapper<CustomerPoolRecycleRule> customerPoolRecycleRuleMapper;
  @Resource private ModuleFormCacheService moduleFormCacheService;
  @Resource private ModuleFormService moduleFormService;
  @Resource private CustomerRelationService customerRelationService;
  ...
  @Resource private ExtCustomerPoolMapper extCustomerPoolMapper;
  ...
  @Resource private ExtFollowUpRecordMapper extFollowUpRecordMapper;
  @Resource private ExtFollowUpPlanMapper extFollowUpPlanMapper;
  @Resource private BaseMapper<CustomerCollaboration> customerCollaborationMapper;
  ...
  public PagerWithOption<List<CustomerListResponse>> transitionList(
    ...
    request.setTransitionDataPermission(
        dataScopeService.getDeptDataPermission(
            userId, orgId, null, PermissionConstants.CUSTOMER_MANAGEMENT_READ));
    return list(request, userId, orgId, null);
  }

  public Map<String, List<OptionDTO>> buildOptionMap(
      String orgId, List<CustomerListResponse> list, List<CustomerListResponse> buildList) {
    // 处理自定义字段选项数据
    ModuleFormConfigDTO customerFormConfig = getFormConfig(orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(list, CustomerListResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList, CustomerListResponse::getOwner, CustomerListResponse::getOwnerName);
  ...
  public PagerWithOption<List<CustomerListResponse>> sourceList(
      CustomerPageRequest request,
      String userId,
      String orgId,
      DeptDataPermissionDTO deptDataPermission) {
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<CustomerListResponse> list =
        extCustomerMapper.sourceList(request, orgId, userId, deptDataPermission);
    List<CustomerListResponse> buildList = buildListData(list, orgId);
    Map<String, List<OptionDTO>> optionMap = buildOptionMap(orgId, list, buildList);
    return PageUtils.setPageInfoWithOption(page, buildList, optionMap);
  }
  ...
  public List<CustomerListResponse> buildListData(List<CustomerListResponse> list, String orgId) {
    ...
    List<String> customerIds =
        list.stream().map(CustomerListResponse::getId).collect(Collectors.toList());

    Map<String, List<BaseModuleFieldValue>> caseCustomFiledMap =
        customerFieldService.getResourceFieldMap(customerIds, true);

    List<String> ownerIds = list.stream().map(CustomerListResponse::getOwner).distinct().toList();

    List<String> followerIds =
        list.stream().map(CustomerListResponse::getFollower).distinct().toList();
    List<String> createUserIds =
        list.stream().map(CustomerListResponse::getCreateUser).distinct().toList();
    List<String> updateUserIds =
        list.stream().map(CustomerListResponse::getUpdateUser).distinct().toList();
    List<String> userIds =
    ...
    // 获取负责人默认公海信息
    Map<String, CustomerPool> ownersDefaultPoolMap =
        customerPoolService.getOwnersDefaultPoolMap(ownerIds, orgId);
    List<String> poolIds =
        ownersDefaultPoolMap.values().stream().map(CustomerPool::getId).distinct().toList();
    ...
    if (CollectionUtils.isEmpty(poolIds)) {
      ...
      List<CustomerPoolRecycleRule> recycleRules =
          customerPoolRecycleRuleMapper.selectListByLambda(recycleRuleWrapper);
      recycleRuleMap =
          recycleRules.stream()
              .collect(Collectors.toMap(CustomerPoolRecycleRule::getPoolId, rule -> rule));
    }
    ...
    list.forEach(
        customerListResponse -> {
          ...
          customerListResponse.setRecyclePoolName(
              reservePool != null ? reservePool.getName() : null);
          // 计算剩余归属天数
          customerListResponse.setReservedDays(
              customerPoolService.calcReservedDay(
                  reservePool,
                  reservePool != null ? recycleRuleMap.get(reservePool.getId()) : null,
                  customerListResponse.getCollectionTime(),
                  customerListResponse.getCreateTime()));
          ...
          if (StringUtils.isNotEmpty(customerListResponse.getFollower())) {
            String followerName =
                baseService.getAndCheckOptionName(
                    userNameMap.get(customerListResponse.getFollower()));
            customerListResponse.setFollowerName(followerName);
          }
          String createUserName =
              baseService.getAndCheckOptionName(
                  userNameMap.get(customerListResponse.getCreateUser()));
          customerListResponse.setCreateUserName(createUserName);
          String updateUserName =
              baseService.getAndCheckOptionName(
                  userNameMap.get(customerListResponse.getUpdateUser()));
          ...
          if (StringUtils.isNotBlank(customerListResponse.getReasonId())) {
            String reasonName =
                baseService.getAndCheckOptionName(dictMap.get(customerListResponse.getReasonId()));
  ...
  public CustomerGetResponse getWithDataPermissionCheck(String id, String userId, String orgId) {
    ...
    boolean hasPermission =
        dataScopeService.hasDataPermission(
            userId, orgId, getResponse.getOwner(), PermissionConstants.CUSTOMER_MANAGEMENT_READ);
    if (!hasPermission) {
      List<CustomerCollaboration> collaborations =
          customerCollaborationService.selectByCustomerIdAndUserId(getResponse.getId(), userId);
  ...
  public CustomerGetResponse get(String id) {
    ...
    CustomerGetResponse customerGetResponse =
        BeanUtils.copyBean(new CustomerGetResponse(), customer);
    customerGetResponse = baseService.setCreateUpdateOwnerUserName(customerGetResponse);
    // 获取模块字段
    List<BaseModuleFieldValue> customerFields =
        customerFieldService.getModuleFieldValuesByResourceId(id);
    ModuleFormConfigDTO customerFormConfig = getFormConfig(customer.getOrganizationId());

    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerFormConfig, customerFields);

    // 补充负责人选项
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            customerGetResponse, CustomerGetResponse::getOwner, CustomerGetResponse::getOwnerName);
    optionMap.put(BusinessModuleField.CUSTOMER_OWNER.getBusinessKey(), ownerFieldOption);
    ...
    DictConfigDTO dictConf =
        dictService.getDictConf(DictModule.CUSTOMER_POOL_RS.name(), customer.getOrganizationId());
    List<Dict> dictList = dictConf.getDictList();
    Map<String, String> dictMap =
        dictList.stream().collect(Collectors.toMap(Dict::getId, Dict::getName));
    ...
    if (customerGetResponse.getOwner() != null) {
      // 获取负责人默认公海信息
      Map<String, CustomerPool> ownersDefaultPoolMap =
          customerPoolService.getOwnersDefaultPoolMap(
              List.of(customerGetResponse.getOwner()), customer.getOrganizationId());
      List<String> poolIds =
          ownersDefaultPoolMap.values().stream().map(CustomerPool::getId).distinct().toList();
      Map<String, CustomerPoolRecycleRule> recycleRuleMap;
      if (CollectionUtils.isEmpty(poolIds)) {
      ...
      // 计算剩余归属天数
      customerGetResponse.setReservedDays(
          customerPoolService.calcReservedDay(
              reservePool,
              reservePool != null ? recycleRuleMap.get(reservePool.getId()) : null,
              customerGetResponse.getCollectionTime(),
              customerGetResponse.getCreateTime()));

      UserDeptDTO userDeptDTO =
          baseService.getUserDeptMapByUserId(
              customerGetResponse.getOwner(), customer.getOrganizationId());
      if (userDeptDTO != null) {
    ...
    if (customerGetResponse.getFollower() != null) {
      Map<String, String> userNameMap =
          baseService.getUserNameMap(List.of(customerGetResponse.getFollower()));
      String followerName =
          baseService.getAndCheckOptionName(userNameMap.get(customerGetResponse.getFollower()));
      customerGetResponse.setFollowerName(followerName);
    }

    if (StringUtils.isNotBlank(customerGetResponse.getReasonId())) {
      String reasonName =
          baseService.getAndCheckOptionName(dictMap.get(customerGetResponse.getReasonId()));
      customerGetResponse.setReasonName(reasonName);
    }

    // 附件信息
    customerGetResponse.setAttachmentMap(
        moduleFormService.getAttachmentMap(customerFormConfig, customerFields));

    return customerGetResponse;
  }

  @OperationLog(
      module = LogModule.CUSTOMER_INDEX,
      type = LogType.ADD,
  ...
  public Customer add(CustomerAddRequest request, String userId, String orgId) {
    ...
    commonNoticeSendService.sendNotice(
        ...
        NotificationConstants.Event.CUSTOMER_ADD,
        customer.getName(),
        userId,
        orgId,
  ...
  public Customer update(CustomerUpdateRequest request, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, originCustomer.getOwner(), PermissionConstants.CUSTOMER_MANAGEMENT_UPDATE);
    ...
    if (StringUtils.isNotBlank(request.getOwner())) {
      if (!Strings.CS.equals(request.getOwner(), originCustomer.getOwner())) {
        ...
        customerContactService.updateContactOwner(
            request.getId(), request.getOwner(), originCustomer.getOwner(), orgId);
    ...
    List<BaseModuleFieldValue> originCustomerFields =
        customerFieldService.getModuleFieldValuesByResourceId(request.getId());

    if (BooleanUtils.isTrue(request.getAgentInvoke())) {
      customerFieldService.updateModuleFieldByAgent(
          customer, originCustomerFields, request.getModuleFields(), orgId, userId);
    } else {
      // 更新模块字段
    ...
    baseService.handleUpdateLog(
        ...
        originCustomer.getName());
    return customer;
  }

  private void updateModuleField(
      Customer customer, List<BaseModuleFieldValue> moduleFields, String orgId, String userId) {
  ...
  public void delete(String id, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, originCustomer.getOwner(), PermissionConstants.CUSTOMER_MANAGEMENT_DELETE);
    ...
    commonNoticeSendService.sendNotice(
        NotificationConstants.Module.CUSTOMER,
        NotificationConstants.Event.CUSTOMER_DELETED,
        originCustomer.getName(),
        userId,
        orgId,
        List.of(originCustomer.getOwner()),
        true);
  }
  ...
  public void batchTransfer(CustomerBatchTransferRequest request, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, owners, PermissionConstants.CUSTOMER_MANAGEMENT_UPDATE);
    // 添加责任人历史
  ...
  private void sendTransferNotice(
      List<Customer> originCustomers, String toUser, String userId, String orgId) {
    originCustomers.forEach(
        customer ->
            commonNoticeSendService.sendNotice(
                NotificationConstants.Module.CUSTOMER,
  ...
  public void batchDelete(List<String> ids, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, owners, PermissionConstants.CUSTOMER_MANAGEMENT_DELETE);
  ...
  private List<String> getOwners(List<Customer> customers) {
    return customers.stream().map(Customer::getOwner).distinct().toList();
  }
  ...
  public BatchAffectResponse batchToPool(
    ...
    List<String> owners = getOwners(customers);
    dataScopeService.checkDataPermission(
        currentUser, orgId, owners, PermissionConstants.CUSTOMER_MANAGEMENT_RECYCLE);
    ...
    for (Customer customer : customers) {
      ...
      LogDTO logDTO =
          new LogDTO(
              ...
              customer.getName());
      String detail =
          Translator.getWithArgs("customer.to.pool", customer.getName(), customerPool.getName());
      logDTO.setDetail(detail);
      logs.add(logDTO);
      // 消息通知
      commonNoticeSendService.sendNotice(
          NotificationConstants.Module.CUSTOMER,
          NotificationConstants.Event.CUSTOMER_MOVED_HIGH_SEAS,
          customer.getName(),
          currentUser,
          orgId,
          List.of(customer.getOwner()),
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.CUSTOMER_MANAGEMENT_READ, rolePermissions);
  }
  ...
  public void downloadImportTpl(HttpServletResponse response, String currentOrg) {
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            response,
            moduleFormService.getCustomImportHeadsNoRef(FormKey.CUSTOMER.getKey(), currentOrg),
            Translator.get("customer.import_tpl.name"),
            Translator.get(SheetKey.DATA),
            Translator.get(SheetKey.COMMENT),
            new CustomTemplateWriteHandler(
                moduleFormService.getAllCustomImportFields(FormKey.CUSTOMER.getKey(), currentOrg)),
  ...
  public ImportResponse realImport(MultipartFile file, String currentOrg, String currentUser) {
    try {
      List<BaseField> fields =
          moduleFormService.getAllFields(FormKey.CUSTOMER.getKey(), currentOrg);
      CustomImportAfterDoConsumer<Customer, BaseResourceSubField> afterDo =
          (customers, customerFields, customerFieldBlobs) -> {
            List<LogDTO> logs = new ArrayList<>();
            customers.forEach(
                customer -> {
                  customer.setCollectionTime(customer.getCreateTime());
            ...
            customerFieldMapper.batchInsert(
                customerFields.stream()
                    .map(field -> BeanUtils.copyBean(new CustomerField(), field))
                    .toList());
            customerFieldBlobMapper.batchInsert(
                customerFieldBlobs.stream()
                    .map(field -> BeanUtils.copyBean(new CustomerFieldBlob(), field))
                    .toList());
            // record logs
            logService.batchAdd(logs);
          };
      CustomFieldImportEventListener<Customer> eventListener =
  ...
  private ImportResponse checkImportExcel(MultipartFile file, String currentOrg) {
    try {
      ...
      CustomFieldCheckEventListener eventListener =
          new CustomFieldCheckEventListener(fields, "customer", "customer_field", currentOrg);
      FastExcelFactory.read(file.getInputStream(), eventListener)
          .headRowNumber(1)
          .ignoreEmptyRow(true)
          .sheet()
  ...
  public void batchUpdate(ResourceBatchEditRequest request, String userId, String organizationId) {
    ...
    if (Strings.CS.equals(
        field.getBusinessKey(), BusinessModuleField.CUSTOMER_OWNER.getBusinessKey())) {
      // 修改负责人，走批量转移接口
    ...
    customerFieldService.batchUpdate(
        request,
        field,
        originCustomers,
        Customer.class,
        LogModule.CUSTOMER_INDEX,
        extCustomerMapper::batchUpdate,
        userId,
        organizationId);
  }
  ...
  public void merge(CustomerMergeRequest request, String currentUser, String currentOrgId) {
    ...
    List<CustomerContact> toMergeContacts =
        customerContactMapper.selectListByLambda(contactWrapper);
    ...
    if (!Strings.CS.equals(oldCustomer.getOwner(), request.getOwnerId())) {
      ...
      // 客户负责人变更，联系人同步更新
      customerContactService.updateContactOwner(
          request.getToMergeId(), request.getOwnerId(), oldCustomer.getOwner(), currentOrgId);
      // 如果责任人有修改，则添加责任人历史
  ...
  public List<ChartResult> chart(
    ...
    CustomerChartAnalysisDbRequest customerChartAnalysisDbRequest =
        BeanUtils.copyBean(new CustomerChartAnalysisDbRequest(), chartAnalysisDbRequest);
    List<ChartResult> chartResults =
        extCustomerMapper.chart(customerChartAnalysisDbRequest, userId, orgId, deptDataPermission);
   ...
   * @param currentOrgId 当前组织ID
   */
  private void mergeCollaboration(
      CustomerMergeRequest request, String currentUser, String currentOrgId) {
    // 被合并客户的协作人
    LambdaQueryWrapper<CustomerCollaboration> mergeCollaborationWrapper =
        new LambdaQueryWrapper<>();
    mergeCollaborationWrapper.in(CustomerCollaboration::getCustomerId, request.getMergeIds());
    List<CustomerCollaboration> mergeCollaborations =
        customerCollaborationMapper.selectListByLambda(mergeCollaborationWrapper);
    List<String> toCollaborationUserIds =
        mergeCollaborations.stream().map(CustomerCollaboration::getUserId).distinct().toList();
    Map<String, String> toCollaborationMap =
        mergeCollaborations.stream()
            .collect(
                Collectors.toMap(
                    CustomerCollaboration::getUserId, CustomerCollaboration::getCollaborationType));
    // 被合并的客户负责人
    List<Customer> mergeCustomers = customerMapper.selectByIds(request.getMergeIds());
    List<String> toCollaborationOwnerIds = mergeCustomers.stream().map(Customer::getOwner).toList();
    // 合并去重
    List<String> mergeIds =
        Stream.of(toCollaborationUserIds, toCollaborationOwnerIds)
            .flatMap(Collection::stream)
            .distinct()
            .toList();
    // 合并客户已存在的协作人
    ...
    List<CustomerCollaboration> customerCollaborations =
        customerCollaborationMapper.selectListByLambda(collaborationWrapper);
    List<String> collaborationUserIds =
        customerCollaborations.stream().map(CustomerCollaboration::getUserId).toList();
    for (String mergeId : mergeIds) {
      if (collaborationUserIds.contains(mergeId)
          || Strings.CS.equals(mergeId, request.getOwnerId())) {
      ...
      collaborationAddRequest.setCollaborationType(
          toCollaborationMap.getOrDefault(mergeId, "COLLABORATION"));
   ...
   * @return 日志列表
   */
  private List<LogDTO> getMergeRelateLogs(
      CustomerMergeRequest mergeRequest, String currentUser, String currentOrgId) {
    List<LogDTO> logs = new ArrayList<>();

    List<Customer> mergeCustomers = customerMapper.selectByIds(mergeRequest.getMergeIds());
    Map<String, String> customerMap =
        mergeCustomers.stream().collect(Collectors.toMap(Customer::getId, Customer::getName));
    // 联系人日志
    List<CustomerContact> mergeContacts =
        extCustomerContactMapper.getMergeContactList(mergeRequest, currentOrgId);
    ...
    // 商机日志
    List<Opportunity> mergeOpportunities =
        extOpportunityMapper.getMergeOpportunityList(mergeRequest, currentOrgId);
    ...
    // 跟进记录/计划日志
    List<FollowUpRecord> mergeRecords =
        extFollowUpRecordMapper.getMergeRecordList(mergeRequest, currentOrgId);
    List<String> recordCustomerIds =
        mergeRecords.stream().map(FollowUpRecord::getCustomerId).toList();
    List<FollowUpPlan> mergePlans =
        extFollowUpPlanMapper.getMergePlanList(mergeRequest, currentOrgId);
      ...
      if (recordCustomerIds.contains(mergeId)) {
        LogDTO logDTO =
            new LogDTO(
                ...
                currentUser,
                LogType.UPDATE,
                LogModule.FOLLOW_UP_RECORD,
                customerMap.get(mergeId));
