...
public class ClueOwnerHistoryService {
  @Resource private BaseMapper<ClueOwner> clueOwnerMapper;
  @Resource private BaseService baseService;
  @Resource private ExtClueOwnerMapper extClueOwnerMapper;
  ...
  private List<ClueOwnerListResponse> buildListData(String orgId, List<ClueOwner> owners) {
    ...
    configLambdaQueryWrapper
        .eq(DictConfig::getModule, DictModule.CLUE_POOL_RS.name())
  ...
  public ClueOwner add(Clue clue, String userId, Boolean addReason) {
    ...
    if (addReason
        && StringUtils.isNotBlank(clue.getReasonId())
        && !Strings.CS.equals(clue.getReasonId(), "system")) {
