  ...
  public List<JsonDifferenceDTO> handleLogField(List<JsonDifferenceDTO> differences, String orgId) {
    ...
    for (JsonDifferenceDTO differ : differences) {
      ...
      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.QUOTATION_OPPORTUNITY.getBusinessKey())) {
        setOpportunityName(differ);
        continue;
      }

      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.QUOTATION_PRODUCT_AMOUNT.getBusinessKey())) {
      ...
      if (Strings.CS.equals(differ.getColumn(), "approvalStatus")) {
        ...
        if (differ.getOldValue() != null) {
          differ.setOldValueName(
              Translator.get("log.approvalStatus." + differ.getOldValueName().toString()));
        }
        if (differ.getNewValue() != null) {
          differ.setNewValueName(
              Translator.get("log.approvalStatus." + differ.getNewValueName().toString()));
        }
      }
    }
    return differences;
