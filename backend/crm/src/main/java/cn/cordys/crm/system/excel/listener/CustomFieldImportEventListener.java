...
public class CustomFieldImportEventListener<T> extends CustomFieldCheckEventListener {
  ...
  /** 主表数据 */
  @Getter private final List<T> dataList;
  ...
  public CustomFieldImportEventListener(
      List<BaseField> fields,
      Class<T> clazz,
      String currentOrg,
      String operator,
      String fieldTable,
      CustomImportAfterDoConsumer<T, BaseResourceSubField> consumer,
      int batchSize,
      Map<Integer, List<CellExtra>> mergeCellMap,
      Map<Integer, Map<Integer, String>> mergeRowDataMap) {
    super(
        fields,
        EntityTableMapper.generateTableName(clazz),
        fieldTable,
        currentOrg,
        mergeCellMap,
        mergeRowDataMap);
  ...
  public void invokeHeadMap(Map<Integer, String> headMap, AnalysisContext context) {
    ...
    Optional<BaseField> anySerial =
        this.fieldMap.values().stream().filter(BaseField::isSerialNumber).findAny();
  ...
  public void invoke(Map<Integer, String> data, AnalysisContext analysisContext) {
    ...
    if (dataList.size() >= batchSize
        || fields.size() >= batchSize
        || blobFields.size() > batchSize) {
      batchProcessData();
    }
  }
  ...
  public void doAfterAllAnalysed(AnalysisContext analysisContext) {
    if (CollectionUtils.isNotEmpty(this.dataList)
        || CollectionUtils.isNotEmpty(this.fields)
        || CollectionUtils.isNotEmpty(this.blobFields)) {
      batchProcessData();
    }
  ...
  private void buildEntityFromRow(Integer rowIndex, Map<Integer, String> rowData) {
    try {
      ...
      headMap.forEach(
          (k, v) -> {
            ...
            if (!refSubMap.containsKey(field.getName())
                && !isNormalRow(rowIndex)
                && !isMergeFirstRow(rowIndex)) {
              // 除合并的首行外, 其余合并行非子表字段都跳过
              return;
            }
            if (businessFieldMap.containsKey(field.getInternalKey())
                && !refSubMap.containsKey(field.getName())) {
              try {
                setPropertyValue(
                    mergedTmpEntity,
                    businessFieldMap.get(field.getInternalKey()).getBusinessKey(),
                    val);
              } catch (Exception e) {
        ...
        if (serialField != null) {
          ...
          String serialNo =
              serialNumGenerator.generateByRules(
                  ((SerialNumberField) serialField).getSerialNumberRules(),
                  currentOrg,
      ...
      throw new GenericException(
          Translator.getWithArgs("import.error", rowIndex + 1).concat(" " + e.getMessage()));
    }
  }
  ...
  private boolean isMergeFirstRow(int rowIndex) {
    ...
    return cellExtras != null
        && cellExtras.stream()
            .anyMatch(
                extra ->
                    rowIndex == extra.getFirstRowIndex() && extra.getLastRowIndex() > rowIndex);
  }
  ...
  private boolean isMergedLastRow(int rowIndex) {
    ...
    return cellExtras != null
        && cellExtras.stream()
            .anyMatch(
                extra ->
                    rowIndex == extra.getLastRowIndex()
                        && extra.getLastRowIndex() > extra.getFirstRowIndex());
  }
