...
public class UserImportEventListener extends AnalysisEventListener<Map<Integer, String>> {
  ...
  private static final Map<String, BiConsumer<UserExcelData, String>> FIELD_SETTERS =
      new HashMap<>();
  ...
  @Getter protected List<UserExcelData> list = new ArrayList<>();
  ...
  public UserImportEventListener(Class<?> clazz, String operatorId, String orgId) {
    ...
    this.departmentTree =
        Objects.requireNonNull(CommonBeanFactory.getBean(DepartmentService.class)).getTree(orgId);
  ...
  private void saveData() {
    if (CollectionUtils.isNotEmpty(list)) {
      organizationUserService.saveImportData(
          list, departmentTree, departmentMap, operatorId, orgId);
    }
  }
    ...
    if (CollectionUtils.isNotEmpty(list)) {
      for (UserExcelData userExcelData : list) {
        if (Strings.CI.equals(userExcelData.getEmail(), email)) {
          errMsg.append(Translator.get("email.repeat")).append(email).append(ERROR_MSG_SEPARATOR);
          break;
        }
      }
    }
    ...
    if (CollectionUtils.isNotEmpty(list)) {
      for (UserExcelData userExcelData : list) {
        if (Strings.CI.equals(userExcelData.getPhone(), phone)) {
          errMsg.append(Translator.get("phone.repeat")).append(phone).append(ERROR_MSG_SEPARATOR);
          break;
        }
      }
    }
