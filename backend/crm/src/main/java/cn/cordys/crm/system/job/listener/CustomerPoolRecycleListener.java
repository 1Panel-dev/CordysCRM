 ...
 * 客户池回收任务
 *
 * <p>该定时任务负责根据设定的规则自动将符合条件的客户从个人池回收到公共客户池。 回收规则基于CustomerPoolRecycleRule中定义的条件，如最后跟进时间、最后更新时间等。
 */
@Component
@Slf4j
public class CustomerPoolRecycleListener implements ApplicationListener<ExecuteEvent> {
   ...
   * 执行客户回收
   *
   * <p>回收流程： 1. 获取启用了自动回收的客户池 2. 找出符合回收规则的客户 3. 执行回收操作并发送通知
   */
  public void recycle() {
    log.info("开始回收客户资源");
    ...
    // 获取池与负责人的映射关系
    Map<List<String>, CustomerPool> ownersDefaultPoolMap =
        customerPoolService.getOwnersBestMatchPoolMap(pools);
    List<String> recycleOwnersIds =
        ownersDefaultPoolMap.keySet().stream().flatMap(List::stream).toList();
  ...
  private List<Customer> getCustomersForRecycle(List<String> recycleOwnersIds) {
    ...
    customerQueryWrapper
        .in(Customer::getOwner, recycleOwnersIds)
  ...
  private Map<String, CustomerPoolRecycleRule> getPoolRecycleRules(List<CustomerPool> pools) {
    ...
    List<CustomerPoolRecycleRule> recycleRules =
        customerPoolRecycleRuleMapper.selectListByLambda(ruleQueryWrapper);
   ...
   */
  private void recycleCustomers(
      List<Customer> customers,
      Map<List<String>, CustomerPool> ownersDefaultPoolMap,
      Map<String, CustomerPoolRecycleRule> recycleRuleMap) {
    customers.forEach(
        customer ->
            ownersDefaultPoolMap.forEach(
                (ownerIds, pool) -> {
