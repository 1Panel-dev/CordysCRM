...
public class SysLoginLogService {
  @Resource private ExtLoginLogMapper extLoginLogMapper;
  ...
  private List<LoginLogListResponse> handleData(List<LoginLogListResponse> list) {
    if (CollectionUtils.isNotEmpty(list)) {
      List<String> userIds =
          list.stream().map(LoginLogListResponse::getOperator).collect(Collectors.toList());
      Map<String, String> userNameMap = baseService.getUserNameMap(userIds);
      list.forEach(
          item ->
              item.setOperatorName(
                  userNameMap.getOrDefault(item.getOperator(), StringUtils.EMPTY)));
    }
    return list;
  }
