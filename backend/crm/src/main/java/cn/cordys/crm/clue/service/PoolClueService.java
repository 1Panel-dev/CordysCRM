...
public class PoolClueService {
  ...
  @Resource private BaseMapper<ClueOwner> ownerMapper;
  @Resource private BaseMapper<CluePool> poolMapper;
  @Resource private BaseMapper<User> userMapper;
  @Resource private BaseMapper<CluePoolPickRule> pickRuleMapper;
  @Resource private BaseMapper<CluePoolRecycleRule> recycleRuleMapper;
  @Resource private ExtClueCapacityMapper extClueCapacityMapper;
  ...
  public List<CluePoolDTO> getPoolOptions(String currentUser, String currentOrgId) {
    ...
    Map<String, CluePoolPickRule> pickRuleMap =
        pickRules.stream()
    ...
    Map<String, List<CluePoolHiddenField>> hiddenFieldMap =
        cluePoolService.getCluePoolHiddenFieldsByPoolIds(poolIds).stream()
            .collect(Collectors.groupingBy(CluePoolHiddenField::getPoolId));

    List<BaseField> fields =
        moduleFormCacheService
            .getBusinessFormConfig(FormKey.CLUE.getKey(), currentOrgId)
            .getFields();
        ...
        pool -> {
          ...
          if (scopeIds.contains(currentUser)
            ...
            recycleRule.setConditions(
                JSON.parseArray(cluePoolRecycleRule.getCondition(), RuleConditionDTO.class));
            ...
            poolDTO.setFieldConfigs(
                cluePoolService.getCluePoolFieldConfigs(fields, hiddenFieldIds));
            options.add(poolDTO);
          }
        });
    return options;
   ...
   */
  @OperationLog(
      module = LogModule.CLUE_POOL_INDEX,
      type = LogType.ASSIGN,
      resourceId = "{#request.clueId}")
  ...
  public void validateCapacity(int processCount, String ownUserId, String currentOrgId) {
    ...
    if (capacity != null && capacity - ownCount < processCount) {
      throw new GenericException(
          Translator.getWithArgs("customer.capacity.over", Math.max(capacity - ownCount, 0)));
    }
  }
   ...
   * @param ownerId 拥有人ID
   */
  private void ownClue(
      String clueId,
      String ownerId,
      CluePoolPickRule pickRule,
    ...
    if (!isPoolAdmin && pickRule != null && pickRule.getLimitNew()) {
      LocalDateTime joinPoolTime =
          LocalDateTime.ofInstant(
              Instant.ofEpochMilli(clue.getUpdateTime()), ZoneId.systemDefault());
      ...
      if (releaseDate.isAfter(LocalDateTime.now())) {
        ...
        throw new GenericException(
            Translator.getWithArgs("pool.data.release.date", releaseDate.format(formatter)));
      }
    }
      ...
      if (CollectionUtils.isNotEmpty(clueOwners)) {
        ...
        if (Strings.CS.equals(lastOwner.getOwner(), ownerId)
            && System.currentTimeMillis()
                < pickRule.getPickIntervalDays() * DAY_MILLIS + lastOwner.getEndTime()) {
          ...
          throw new GenericException(
              Translator.getWithArgs(
                  "customer.pre_owner.pick.limit", nextPickTime.format(formatter)));
        }
      }
    }
    clue.setPoolId(null);
    ...
    if (Strings.CS.equals(logType, LogType.ASSIGN)) {
      commonNoticeSendService.sendNotice(
          ...
          NotificationConstants.Event.CLUE_DISTRIBUTED,
          clue.getName(),
          operateUserId,
          currentOrgId,
