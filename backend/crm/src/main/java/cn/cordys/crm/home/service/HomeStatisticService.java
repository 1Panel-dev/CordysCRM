...
public class HomeStatisticService {
  ...
  public HomeClueStatistic getClueStatistic(
      HomeStatisticBaseSearchRequest request,
    ...
    try {
      for (HomeStatisticPeriod statisticPeriod : HomeStatisticPeriod.values()) {
        HomeStatisticSearchRequest searchRequest =
            BeanUtils.copyBean(new HomeStatisticSearchRequest(), request);
        searchRequest.setPeriod(statisticPeriod.name());
        HomeStatisticSearchWrapperRequest wrapperRequest =
            new HomeStatisticSearchWrapperRequest(searchRequest, deptDataPermission, orgId, userId);

        // 多线程执行
        Future<HomeStatisticSearchResponse> getNewClueStatistic =
            executor.submit(
  ...
  public HomeOpportunityStatistic getOpportunityStatistic(
    ...
    for (HomeStatisticPeriod statisticPeriod : HomeStatisticPeriod.values()) {
      ...
      // 多线程执行
      Future<HomeStatisticSearchResponse> getNewOpportunityStatistic =
          executor.submit(
              () -> getStatisticSearchResponse(wrapperRequest, this::getNewOpportunityCount));
      Future<HomeStatisticSearchResponse> getNewOpportunityTotalAmount =
          executor.submit(
  ...
  public HomeSuccessOpportunityStatistic getSuccessOpportunityStatistic(
    ...
    for (HomeStatisticPeriod statisticPeriod : HomeStatisticPeriod.values()) {
      ...
      // 多线程执行
      Future<HomeStatisticSearchResponse> getSuccessOpportunityStatistic =
          executor.submit(
              () -> getStatisticSearchResponse(wrapperRequest, this::getSuccessOpportunityCount));
      Future<HomeStatisticSearchResponse> getSuccessOpportunityStatisticAmount =
          executor.submit(
  ...
  private HomeStatisticSearchWrapperRequest copyHomeStatisticSearchWrapperRequest(
      HomeStatisticSearchWrapperRequest request) {
    return new HomeStatisticSearchWrapperRequest(
        BeanUtils.copyBean(new HomeStatisticSearchRequest(), request.getStaticRequest()),
  ...
  public Long getNewOpportunityAmount(HomeStatisticSearchWrapperRequest request) {
    return Optional.ofNullable(extOpportunityMapper.selectOpportunityCount(request, true, false))
        .orElse(0L);
  }
  ...
  public Long getSuccessOpportunityAmount(HomeStatisticSearchWrapperRequest request) {
    return Optional.ofNullable(extOpportunityMapper.selectOpportunityCount(request, true, true))
        .orElse(0L);
  }
   ...
   * @return
   */
  public HomeStatisticSearchResponse getStatisticSearchResponse(
      HomeStatisticSearchWrapperRequest request,
    ...
    try {
      ...
      if (request.comparePeriod()) {
        HomeStatisticSearchWrapperRequest periodRequest =
            copyHomeStatisticSearchWrapperRequest(request);
  ...
  private Double getPriorPeriodCompareRate(Long count, Long periodCount) {
    return periodCount == null || periodCount == 0
        ? null
        : (count - periodCount) * 100.0 / periodCount;
  }
   ...
   * @return
   */
  public DeptDataPermissionDTO getDeptDataPermissionDTO(
      HomeStatisticBaseSearchRequest request, String permission) {
    if (Strings.CS.equals(request.getSearchType(), BusinessSearchType.DEPARTMENT.name())) {
      DeptDataPermissionDTO deptDataPermission =
          dataScopeService.getDeptDataPermission(
              SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), permission);
      if (deptDataPermission.getAll()) {
        ...
        Set<String> deptIds =
            request.getDeptIds().stream()
                .filter(
                    deptId ->
                        CollectionUtils.isNotEmpty(deptDataPermission.getDeptIds())
      ...
      return dataScopeService.getDeptDataPermission(
          SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), permission);
    } else {
  ...
  public List<BaseTreeNode> getDepartmentTree(String userId, String orgId) {
    Map<String, List<RolePermissionDTO>> dataScopeRoleMap =
        permissionCache.getRolePermissions(userId, orgId).stream()
    ...
    boolean hasDeptAndChildPermission =
        dataScopeRoleMap.get(RoleDataScope.DEPT_AND_CHILD.name()) != null;
    boolean hasDeptCustomPermission =
        dataScopeRoleMap.get(RoleDataScope.DEPT_CUSTOM.name()) != null;
    ...
    if (hasDeptAndChildPermission || hasDeptCustomPermission) {
      ...
      if (hasDeptCustomPermission) {
        ...
        List<String> customDeptRolesIds =
            dataScopeRoleMap.get(RoleDataScope.DEPT_CUSTOM.name()).stream()
  ...
  /**
   * 剪枝 从树中移除不在已选部门ID中的节点，同时保留已选部门ID的子节点
   *
   * @param tree
   * @param deptIds
   * @return
