...
public class CustomerPoolService {
  ...
  @Resource private BaseMapper<CustomerPool> customerPoolMapper;
  @Resource private BaseMapper<CustomerPoolHiddenField> customerPoolHiddenFieldMapper;
  @Resource private BaseMapper<CustomerPoolPickRule> customerPoolPickRuleMapper;
  ...
  public List<CustomerPoolDTO> page(BasePageRequest request, String organizationId) {
    ...
    List<String> poolIds = pools.stream().map(CustomerPoolDTO::getId).toList();
    ...
    List<CustomerPoolPickRule> pickRules =
        customerPoolPickRuleMapper.selectListByLambda(pickRuleWrapper);
    ...
    Map<String, List<CustomerPoolHiddenField>> hiddenFieldMap =
        getCustomerPoolHiddenFieldByPoolIds(poolIds).stream()
            .collect(Collectors.groupingBy(CustomerPoolHiddenField::getPoolId));

    List<BaseField> fields =
        moduleFormCacheService
            .getBusinessFormConfig(FormKey.CUSTOMER.getKey(), organizationId)
            .getFields();
        ...
        .forEach(
            condition -> {
              if (Strings.CS.equals(
                  condition.getOperator(), RecycleConditionOperator.DYNAMICS.name())) {
  ...
  public List<CustomerPoolFieldConfigDTO> getFieldConfigs(
      List<BaseField> fields, Set<String> hiddenFieldIds) {
    return fields.stream()
        .map(
            field -> {
              ...
              hiddenFieldDTO.setEditable(
                  !Strings.CS.equals(
                      field.getInternalKey(), BusinessModuleField.CUSTOMER_NAME.getKey()));
  ...
  public void update(
      CustomerPoolUpdateRequest request, String currentUserId, String organizationId) {
  ...
  private void batchInsertCustomerPoolHiddenFields(String poolId, Set<String> fieldIds) {
    ...
    List<CustomerPoolHiddenField> customerPoolHiddenFields =
        fieldIds.stream()
            .map(
                fieldId -> {
  ...
  public boolean checkNoPick(String id) {
    ...
    wrapper.eq(Customer::getPoolId, id).eq(Customer::getInSharedPool, true);
   ...
   * @return 默认公海
   */
  public Map<String, CustomerPool> getOwnersDefaultPoolMap(
      List<String> ownerIds, String organizationId) {
    Map<String, CustomerPool> poolMap = new HashMap<>(4);
    List<CustomerPool> pools = extCustomerPoolMapper.getAllPool(organizationId);
    Map<String, List<String>> ownerScopeMap =
        userExtendService.getMultiScopeMap(ownerIds, organizationId);
    ownerIds.forEach(
        ownerId -> {
    ...
    return pools.stream()
        .filter(
            pool -> {
              ...
              return CollectionUtils.isNotEmpty(poolScopes)
                  && CollectionUtils.containsAny(scopeIds, poolScopes);
            })
   ...
   * @return 剩余归属天数
   */
  public Integer calcReservedDay(
      CustomerPool pool,
      CustomerPoolRecycleRule recycleRule,
      Long collectionTime,
      Long createTime) {
    ...
    List<RuleConditionDTO> conditions =
        JSON.parseArray(recycleRule.getCondition(), RuleConditionDTO.class);
  ...
  public Map<List<String>, CustomerPool> getOwnersBestMatchPoolMap(List<CustomerPool> pools) {
    ...
    for (CustomerPool pool : pools) {
      ...
      List<String> ownerIds =
          userExtendService.getScopeOwnerIds(scopeIds, pool.getOrganizationId());
      List<String> defaultOwnerIds =
          ownerIds.stream().distinct().filter(ownerId -> !exitOwnerIds.contains(ownerId)).toList();
  ...
  public boolean checkRecycled(Customer customer, CustomerPoolRecycleRule recycleRule) {
    boolean allMatch =
        Strings.CS.equals(CombineSearch.SearchMode.AND.name(), recycleRule.getOperator());
    List<RuleConditionDTO> conditions =
        JSON.parseArray(recycleRule.getCondition(), RuleConditionDTO.class);
    if (allMatch) {
  ...
  private boolean matchTime(RuleConditionDTO condition, Customer customer) {
    if (Strings.CS.equals(condition.getColumn(), RecycleConditionColumnKey.STORAGE_TIME)) {
      if (condition.getScope().contains(RecycleConditionScopeKey.CREATED)) {
        ...
        return RecycleConditionUtils.matchTime(condition, customer.getCreateTime())
            || RecycleConditionUtils.matchTime(condition, customer.getCollectionTime());
      }
    } else {
