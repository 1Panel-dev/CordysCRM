    ...
    receivers.forEach(
        receiver -> {
          ...
          notification.setSubject(
              StringUtils.isBlank(subjectText)
                  ? Translator.get("notice.default.subject")
                  : subjectText);
          ...
          stringRedisTemplate
              .opsForZSet()
              .add(USER_PREFIX + receiver.getUserId(), id, System.currentTimeMillis());
          stringRedisTemplate.opsForValue().set(MSG_PREFIX + id, messageText);
          // 限制 Redis 只存 5 条消息
          Set<String> oldNotificationIds =
              stringRedisTemplate
                  .opsForZSet()
