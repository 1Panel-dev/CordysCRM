...
public class ThirdDepartment {
  ...
  public static List<ThirdDepartment> buildDepartmentTree(
      String internalId, List<ThirdDepartment> departments) {
    ...
    for (ThirdDepartment department : departments) {
      ...
      department.setCrmParentId(
          department.getIsRoot() ? internalId : department.getCrmParentId()); // 生成新的 ParentId
    }
  ...
  public static List<ThirdDepartment> buildDepartmentTreeMultiple(
      String internalId,
              ...
              () -> {
                if (department.getIsRoot()) {
                  ...
                  currentDepartmentList.stream()
                      .filter(
                          dept ->
                              Strings.CI.equalsAny(dept.getResourceId(), department.getParentId()))
                      .findFirst()
                      .ifPresentOrElse(
                          parent -> {
