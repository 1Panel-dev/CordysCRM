...
public class SSOService {
  ...
  @Resource private BaseMapper<User> userBaseMapper;
  @Resource private BaseMapper<UserExtend> userExtendBaseMapper;
  @Resource private UserLoginService userLoginService;
  @Resource private OAuthUserService oauthUserService;
  @Resource private TokenService tokenService;

  public SessionUser exchangeGitOauth2(String code) {
    validateCode(code);

    OrganizationConfigDetail configDetail =
        getAuthConfigDetail(UserSource.GITHUB_OAUTH2.toString());
    String content = new String(configDetail.getContent(), StandardCharsets.UTF_8);
    Map<String, String> config =
        JSON.parseObject(content, new TypeReference<HashMap<String, String>>() {});

    Map<String, Object> resultObj = getGitHubUserInfo(code, config);

    // 解析用户信息映射
    Map<String, String> mapping =
        JSON.parseObject(config.get("mapping"), new TypeReference<HashMap<String, String>>() {});
    ...
    LoginRequest loginRequest =
        createLoginRequest(name, enableUser.getPassword(), ThirdConfigTypeConstants.WECOM.name());
  ...
  public SessionUser exchangeWeComOauth2(String code) {
    ...
    OrganizationConfigDetail configDetail =
        getAuthConfigDetail(ThirdDetailType.WECOM_SYNC.toString());
    String content = new String(configDetail.getContent(), StandardCharsets.UTF_8);
  ...
  public SessionUser exchangeWeComCode(String code) {
    ...
    WecomThirdConfigRequest weComConfig =
        JSON.MAPPER.convertValue(config.getConfig(), WecomThirdConfigRequest.class);
  ...
  public SessionUser exchangeDingTalkCode(String code) {
    ...
    DingTalkThirdConfigRequest dingTalkConfig =
        JSON.MAPPER.convertValue(config.getConfig(), DingTalkThirdConfigRequest.class);
  ...
  private SessionUser getWeComSessionUser(
      String code, WecomThirdConfigRequest weComConfig, String authenticateType) {
    // 获取assess_token
    String assessToken =
        tokenService.getAssessToken(weComConfig.getCorpId(), weComConfig.getAppSecret());
    ...
    String email =
        StringUtils.isNotBlank(weComUser.getBizMail())
  ...
  private SessionUser getDingTalkSessionUser(
      String code, DingTalkThirdConfigRequest dingTalkConfig, String authenticateType) {
    // 获取用户assess_token
    String assessToken =
        tokenService.getDingTalkUserToken(
            dingTalkConfig.getAgentId(), dingTalkConfig.getAppSecret(), code);
    if (StringUtils.isBlank(assessToken)) {
    ...
    String dingTalkToken =
        tokenService.getDingTalkToken(dingTalkConfig.getAgentId(), dingTalkConfig.getAppSecret());
    // 根据unionid查询用户信息
    String userIdByUnionId =
        oauthUserService.getUserIdByUnionId(dingTalkToken, dingTalkUserResponse.getUnionId());
  ...
  private OrganizationConfigDetail getAuthConfigDetail(String source) {
    ...
    List<OrganizationConfigDetail> enableOrganizationConfigDetails =
        extOrganizationConfigDetailMapper.getEnableOrganizationConfigDetails(
            organizationConfig.getId(), List.of(source));
  ...
  private LoginRequest createThirdPartyLoginRequest(
      UserDTO user, String platform, String authenticateType) {
    LoginRequest request = new LoginRequest();
  ...
  private ThirdConfigBaseDTO<?> getThirdPartyConfig(String type) {
    IntegrationConfigService integrationConfigService =
        CommonBeanFactory.getBean(IntegrationConfigService.class);
    assert integrationConfigService != null;
    List<ThirdConfigBaseDTO<?>> synOrganizationConfigs =
        integrationConfigService.getThirdConfig(DEFAULT_ORGANIZATION_ID);

    if (CollectionUtils.isEmpty(synOrganizationConfigs)) {
      throw new GenericException(Translator.get(ERROR_THIRD_CONFIG_NOT_EXIST));
    }

    ThirdConfigBaseDTO<?> config =
        synOrganizationConfigs.stream()
  ...
  public SessionUser exchangeDingTalkOauth2(String code) {
    ...
    OrganizationConfigDetail configDetail =
        getAuthConfigDetail(ThirdDetailType.DINGTALK_SYNC.name());
    String content = new String(configDetail.getContent(), StandardCharsets.UTF_8);
    ThirdConfigBaseDTO<?> config = JSON.parseObject(content, ThirdConfigBaseDTO.class);
  ...
  public SessionUser exchangeLarkCode(String code) {
    ...
    LarkThirdConfigRequest larkConfig =
        JSON.MAPPER.convertValue(config.getConfig(), LarkThirdConfigRequest.class);
  ...
  private SessionUser getLarkSessionUser(
      String code, LarkThirdConfigRequest larkConfig, String loginType, Boolean isMobile) {
    if (isMobile) {
      // 移动端需要变更域名
    ...
    // 获取用户assess_token
    String assessToken =
        tokenService.getLarkUserAccessToken(
            larkConfig.getAgentId(), larkConfig.getAppSecret(), larkConfig.getRedirectUrl(), code);
    if (StringUtils.isBlank(assessToken)) {
      throw new GenericException(Translator.get(ERROR_AUTH_GET_USER_ERROR));
    }
    ...
    // 创建登录请求并登录
    LoginRequest loginRequest =
        createThirdPartyLoginRequest(enableUser, ThirdConfigTypeConstants.LARK.name(), loginType);
