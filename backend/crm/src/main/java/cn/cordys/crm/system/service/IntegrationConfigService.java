...
public class IntegrationConfigService {
  ...
  @Resource private ExtOrganizationConfigDetailMapper extOrganizationConfigDetailMapper;

  @Resource private BaseMapper<OrganizationConfigDetail> organizationConfigDetailBaseMapper;

  @Resource private BaseMapper<OrganizationConfig> organizationConfigBaseMapper;
  ...
  public List<ThirdConfigBaseDTO<?>> getThirdConfig(String organizationId) {
    List<OrganizationConfigDetail> organizationConfigDetails =
        initConfig(organizationId, SessionUtils.getUserId());
   ...
   */
  @OperationLog(
      module = LogModule.SYSTEM_BUSINESS_THIRD,
      type = LogType.UPDATE,
      operator = "{#userId}")
  public void editThirdConfig(
      ThirdConfigBaseDTO<Object> configDTO, String organizationId, String userId) {
    // 获取或创建组织配置
    ...
    List<OrganizationConfigDetail> existingDetails =
        extOrganizationConfigDetailMapper.getOrgConfigDetailByType(
            organizationConfig.getId(), null, types);
    ...
    if (lastSyncType != null
        && !Strings.CI.equals(lastSyncType, configDTO.getType())
        && getEnable(configDTO)) {
      // 关闭其他平台按钮
    ...
    if (CollectionUtils.isEmpty(existingDetails)) {
      ...
      // 更新已有配置
      handleExistingConfigDetails(
          configDTO, userId, token, types, organizationConfig, existingDetails, typeEnableMap);
    }
  }
  ...
  public boolean testConnection(ThirdConfigBaseDTO<?> configDTO, String orgId, String userId) {
    String token = getToken(configDTO);
    if (ThirdConfigTypeConstants.WECOM.name().equals(configDTO.getType())
        && StringUtils.isNotBlank(token)) {
      WecomThirdConfigRequest config =
          JSON.MAPPER.convertValue(configDTO.getConfig(), WecomThirdConfigRequest.class);
  ...
  public boolean getSyncStatus(String orgId, String type, String syncResource) {
    OrganizationConfig syncStatus =
        extOrganizationConfigMapper.getSyncStatus(orgId, type, syncResource);
  ...
  public ThirdConfigBaseDTO<?> getThirdConfigForPublic(String type, String orgId) {
    ...
    OrganizationConfig config =
        extOrganizationConfigMapper.getOrganizationConfig(orgId, configType);
    ...
    List<OrganizationConfigDetail> details =
        extOrganizationConfigDetailMapper.getOrganizationConfigDetails(config.getId(), null);
  ...
  public List<OptionDTO> getThirdTypeList(String orgId) {
    ...
    List<OrganizationConfigDetail> details =
        extOrganizationConfigDetailMapper.getOrgConfigDetailByType(config.getId(), null, codeTypes);
  ...
  public void switchThirdPartySetting(String type, String organizationId) {
    ...
    extOrganizationConfigMapper.updateSyncFlag(
        organizationId, type, OrganizationConfigConstants.ConfigType.THIRD.name(), false);
  ...
  public ThirdConfigBaseDTO<?> getApplicationConfig(
      String organizationId, String userId, String type) {
  ...
  private void buildDetailData(
      List<OrganizationConfigDetail> details, List<ThirdConfigBaseDTO<?>> configDTOs) {
  ...
  private <T> ThirdConfigBaseDTO<?> buildDto(
      String content, Class<T> configClass, Consumer<T> enableSetter, String type) {
  ...
  private List<OrganizationConfigDetail> initConfig(String organizationId, String userId) {
    ...
    // 检查当前类型下是否还有数据
    List<OrganizationConfigDetail> organizationConfigDetails =
        extOrganizationConfigDetailMapper.getOrganizationConfigDetails(
            organizationConfig.getId(), null);

    OrganizationConfigDetail tenderConfig =
        organizationConfigDetails.stream()
            .filter(
                detail ->
                    Strings.CI.contains(detail.getType(), ThirdConfigTypeConstants.TENDER.name()))
            .findFirst()
            .orElse(null);
  ...
  private void initTender(String userId, OrganizationConfig organizationConfig) {
    ...
    OrganizationConfigDetail detail =
        createConfigDetail(userId, organizationConfig, JSON.toJSONString(tenderConfig));
  ...
  private ThirdConfigBaseDTO<?> getThirdConfigurationDTOByType(
    ...
    List<OrganizationConfigDetail> detailList =
        organizationConfigDetails.stream().filter(t -> t.getType().contains(finalType)).toList();
   ...
   * @param typeEnableMap 类型启用映射
   */
  private void addIntegrationDetail(
      ThirdConfigBaseDTO<Object> configDTO,
      String userId,
      String token,
      List<String> types,
    ...
    switch (typeConstants) {
      ...
      case DINGTALK -> {
        DingTalkThirdConfigRequest dingTalkConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), DingTalkThirdConfigRequest.class);
        ...
        verify = configDTO.getVerify();
        addLog(
            new HashMap<>(), configDTO, null, JSON.parseToMap(JSON.toJSONString(dingTalkConfig)));
      }
      case LARK -> {
        LarkThirdConfigRequest larkConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), LarkThirdConfigRequest.class);
      ...
      case DE -> {
        DeThirdConfigRequest deConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), DeThirdConfigRequest.class);
      ...
      case SQLBOT -> {
        SqlBotThirdConfigRequest sqlBotConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), SqlBotThirdConfigRequest.class);
      ...
      case MAXKB -> {
        MaxKBThirdConfigRequest mkConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), MaxKBThirdConfigRequest.class);
      ...
      case TENDER -> {
        TenderThirdConfigRequest tenderConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), TenderThirdConfigRequest.class);
        tenderConfig.setTenderAddress(TenderApiPaths.TENDER_API);
      ...
      case QCC -> {
        QccThirdConfigRequest qccConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), QccThirdConfigRequest.class);
  ...
  private void handleExistingConfigDetails(
    ...
    Map<String, OrganizationConfigDetail> existDetailTypeMap =
        existingDetails.stream()
            .collect(
                Collectors.toMap(
                    OrganizationConfigDetail::getType, Function.identity(), (left, right) -> left));

    for (String type : types) {
      OrganizationConfigDetail detail = existDetailTypeMap.get(type);
      if (detail == null) {
        addIntegrationDetail(
            configDTO, userId, token, List.of(type), organizationConfig, typeEnableMap);
        continue;
      }

      boolean needResetSync =
          BooleanUtils.isTrue(organizationConfig.isSync())
      ...
      updateExistingDetail(
          configDTO, userId, token, detail, typeEnableMap.get(type), organizationConfig.getId());
    }
  }
  ...
  private void updateIntegrationDetail(
    ...
    switch (typeConstants) {
      ...
      case DINGTALK -> {
        DingTalkThirdConfigRequest config =
            JSON.MAPPER.convertValue(configDTO.getConfig(), DingTalkThirdConfigRequest.class);
        ...
        DingTalkThirdConfigRequest oldConfig =
            parseOldConfig(detail, DingTalkThirdConfigRequest.class);

        addConfigLog(oldConfig, configDTO, id, config);
      }

      case LARK -> {
        LarkThirdConfigRequest config =
            JSON.MAPPER.convertValue(configDTO.getConfig(), LarkThirdConfigRequest.class);
      ...
      case SQLBOT -> {
        SqlBotThirdConfigRequest config =
            JSON.MAPPER.convertValue(configDTO.getConfig(), SqlBotThirdConfigRequest.class);
      ...
      case MAXKB -> {
        MaxKBThirdConfigRequest config =
            JSON.MAPPER.convertValue(configDTO.getConfig(), MaxKBThirdConfigRequest.class);
      ...
      case TENDER -> {
        TenderThirdConfigRequest config =
            JSON.MAPPER.convertValue(configDTO.getConfig(), TenderThirdConfigRequest.class);
   ...
   * @param newConfig 新配置
   */
  private void addConfigLog(
      Object oldConfig, ThirdConfigBaseDTO<?> newDto, String id, Object newConfig) {
    addLog(
   ...
   * @return 配置详情
   */
  private OrganizationConfigDetail createConfigDetail(
      String userId, OrganizationConfig organizationConfig, String jsonString) {
  ...
  private List<String> getDetailTypes(String type) {
    ...
    return switch (typeConstants) {
      ...
      case SQLBOT ->
          List.of(ThirdDetailType.SQLBOT_CHAT.name(), ThirdDetailType.SQLBOT_BOARD.name());
  ...
  private String getToken(ThirdConfigBaseDTO<?> configDTO) {
    ...
    switch (type) {
      ...
      case DINGTALK -> {
        DingTalkThirdConfigRequest dingTalkConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), DingTalkThirdConfigRequest.class);
        return tokenService.getDingTalkToken(
            dingTalkConfig.getAgentId(), dingTalkConfig.getAppSecret());
      }
      case LARK -> {
        LarkThirdConfigRequest larkConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), LarkThirdConfigRequest.class);
      ...
      case MAXKB -> {
        MaxKBThirdConfigRequest mkConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), MaxKBThirdConfigRequest.class);
        return tokenService.getMaxKBToken(mkConfig.getMkAddress(), mkConfig.getAppSecret())
            ? "true"
            : null;
      }
      case TENDER -> {
      ...
      case QCC -> {
        QccThirdConfigRequest qccConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), QccThirdConfigRequest.class);
        return tokenService.getQcc(
                qccConfig.getQccAddress(), qccConfig.getQccAccessKey(), qccConfig.getQccSecretKey())
            ? "true"
            : null;
      }
      default -> {
   ...
   * @param enable 是否启用
   */
  private void updateOrganizationConfigDetail(
      String jsonString, String userId, OrganizationConfigDetail detail, Boolean enable) {
    detail.setContent(jsonString.getBytes());
    detail.setUpdateTime(System.currentTimeMillis());
    detail.setUpdateUser(userId);
   ...
   * @return 配置DTO
   */
  private ThirdConfigBaseDTO<?> getConfigurationByType(
      String type, List<OrganizationConfigDetail> details) {
   ...
   * @return 配置DTO
   */
  private ThirdConfigBaseDTO<?> getNormalConfiguration(
      String type, List<OrganizationConfigDetail> details) {
  ...
  private boolean syncCorpId(byte[] content, ThirdConfigBaseDTO<?> configDTO) {
    ...
    WecomThirdConfigRequest oldConfig =
        JSON.parseObject(new String(content), WecomThirdConfigRequest.class);
    switch (typeConstants) {
      case WECOM -> {
        WecomThirdConfigRequest weComConfig =
            JSON.MAPPER.convertValue(configDTO.getConfig(), WecomThirdConfigRequest.class);
