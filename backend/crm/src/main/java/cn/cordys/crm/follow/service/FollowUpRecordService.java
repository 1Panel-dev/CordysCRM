...
public class FollowUpRecordService extends BaseFollowUpService {
  ...
  @Resource private FollowUpRecordFieldService followUpRecordFieldService;
  ...
  public FollowUpRecord add(FollowUpRecordAddRequest request, String userId, String orgId) {
    ...
    // 保存自定义字段
    followUpRecordFieldService.saveModuleField(
        followUpRecord, orgId, userId, request.getModuleFields(), false);

    followUpRecordMapper.insert(followUpRecord);

    handleFollowTimeAndFollower(
        request.getCustomerId(),
        request.getOpportunityId(),
        request.getClueId(),
        request.getFollowTime(),
        request.getOwner());
   ...
   * @param owner
   */
  private void handleFollowTimeAndFollower(
      String customerId, String opportunityId, String clueId, Long followTime, String owner) {
   ...
   */
  @OperationLog(
      module = LogModule.FOLLOW_UP_RECORD,
      type = LogType.UPDATE,
  ...
  public FollowUpRecord update(FollowUpRecordUpdateRequest request, String userId, String orgId) {
    ...
    Optional.ofNullable(followUpRecord)
        .ifPresentOrElse(
            record -> {
              // 更新跟进记录
              ...
              List<BaseModuleFieldValue> originCustomerFields =
                  followUpRecordFieldService.getModuleFieldValuesByResourceId(request.getId());
              // 更新模块字段
              ...
              handleFollowTimeAndFollower(
                  updateFollowUpRecord.getCustomerId(),
                  updateFollowUpRecord.getOpportunityId(),
                  updateFollowUpRecord.getClueId(),
                  updateFollowUpRecord.getFollowTime(),
                  updateFollowUpRecord.getOwner());
              baseService.handleUpdateLog(
                  ...
                  Translator.get("update_follow_up_record"));
            },
            () -> {
  ...
  private void updateModuleField(
      FollowUpRecord updateFollowUpRecord,
    ...
    // 再保存
    followUpRecordFieldService.saveModuleField(
        updateFollowUpRecord, orgId, userId, moduleFields, true);
  }

  private FollowUpRecord newRecord(
      FollowUpRecord record, FollowUpRecordUpdateRequest request, String userId) {
   ...
   * @return
   */
  public PagerWithOption<List<FollowUpRecordListResponse>> poolList(
      FollowUpRecordPageRequest request,
      String userId,
      String orgId,
      String resourceType,
      String type) {
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<FollowUpRecordListResponse> list =
        extFollowUpRecordMapper.selectPoolList(request, userId, orgId, resourceType, type);
    buildListData(list, orgId);

    // 处理自定义字段选项数据
    ModuleFormConfigDTO customerFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.FOLLOW_RECORD.getKey(), orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            list, FollowUpRecordListResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            list, FollowUpRecordListResponse::getOwner, FollowUpRecordListResponse::getOwnerName);
    ...
    List<OptionDTO> contactFieldOption =
        moduleFormService.getBusinessFieldOption(
            list,
            FollowUpRecordListResponse::getContactId,
            FollowUpRecordListResponse::getContactName);
  ...
  private void buildListData(List<FollowUpRecordListResponse> list, String orgId) {
    ...
    Map<String, List<BaseModuleFieldValue>> recordCustomFieldMap =
        followUpRecordFieldService.getResourceFieldMap(ids, true);

    List<String> createUserIds =
        list.stream().map(FollowUpRecordListResponse::getCreateUser).toList();
    List<String> updateUserIds =
        list.stream().map(FollowUpRecordListResponse::getUpdateUser).toList();
    List<String> ownerIds = list.stream().map(FollowUpRecordListResponse::getOwner).toList();
    List<String> userIds =
        Stream.of(createUserIds, updateUserIds, ownerIds)
    ...
    Map<String, UserResponse> userDeptMap =
        baseService.getUserDepAndPhoneByUserIds(ownerIds, orgId);

    List<String> customerIds =
        list.stream().map(FollowUpRecordListResponse::getCustomerId).toList();
    Map<String, String> customerMap = baseService.getCustomerMap(customerIds);

    List<String> opportunityIds =
        list.stream().map(FollowUpRecordListResponse::getOpportunityId).toList();
    ...
    list.forEach(
        recordListResponse -> {
          ...
          List<BaseModuleFieldValue> customerFields =
              recordCustomFieldMap.get(recordListResponse.getId());
          ...
          recordListResponse.setOpportunityName(
              opportunityMap.get(recordListResponse.getOpportunityId()));
  ...
  public FollowUpRecordDetailResponse get(String id, String orgId) {
    ...
    FollowUpRecordDetailResponse response =
        BeanUtils.copyBean(new FollowUpRecordDetailResponse(), followUpRecord);
    ...
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            List.of(response), FollowUpRecordDetailResponse::getModuleFields);
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerFormConfig, moduleFieldValues);
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            response,
            FollowUpRecordDetailResponse::getOwner,
            FollowUpRecordDetailResponse::getOwnerName);
    ...
    List<OptionDTO> contactFieldOption =
        moduleFormService.getBusinessFieldOption(
            response,
            FollowUpRecordDetailResponse::getContactId,
            FollowUpRecordDetailResponse::getContactName);
    ...
    List<OptionDTO> customerOption =
        moduleFormService.getBusinessFieldOption(
            response,
            FollowUpRecordDetailResponse::getCustomerId,
            FollowUpRecordDetailResponse::getCustomerName);
    optionMap.put(BusinessModuleField.FOLLOW_RECORD_CUSTOMER.getBusinessKey(), customerOption);

    // 商机
    List<OptionDTO> opportunityOption =
        moduleFormService.getBusinessFieldOption(
            response,
            FollowUpRecordDetailResponse::getOpportunityId,
            FollowUpRecordDetailResponse::getOpportunityName);
    optionMap.put(
        BusinessModuleField.FOLLOW_RECORD_OPPORTUNITY.getBusinessKey(), opportunityOption);

    // 线索
    List<OptionDTO> clueOption =
        moduleFormService.getBusinessFieldOption(
            response,
            FollowUpRecordDetailResponse::getClueId,
            FollowUpRecordDetailResponse::getClueName);
    ...
    response.setAttachmentMap(
        moduleFormService.getAttachmentMap(customerFormConfig, moduleFieldValues));
    return response;
  }
  ...
  public PagerWithOption<List<FollowUpRecordListResponse>> list(
    ...
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<FollowUpRecordListResponse> list =
        extFollowUpRecordMapper.selectList(
            request, userId, orgId, resourceType, type, customerData);
    buildListData(list, orgId);
   ...
   * @return 记录的汇总列表
   */
  public PagerWithOption<List<FollowUpRecordListResponse>> totalList(
    ...
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    // 解析当前用户数据权限
    List<FollowUpRecordListResponse> recordList =
        extFollowUpRecordMapper.selectTotalList(
            request, userId, orgId, clueDataPermission, customerDataPermission);
  ...
  public Map<String, List<OptionDTO>> buildOptionMap(
      String orgId, List<FollowUpRecordListResponse> recordList) {
    // 处理自定义字段选项数据
    ModuleFormConfigDTO customerFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.FOLLOW_RECORD.getKey(), orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            recordList, FollowUpRecordListResponse::getModuleFields);
    // 获取选项值对应的 option
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerFormConfig, moduleFieldValues);
    // 补充负责人选项
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            recordList,
            FollowUpRecordListResponse::getOwner,
            FollowUpRecordListResponse::getOwnerName);
    optionMap.put(BusinessModuleField.OPPORTUNITY_OWNER.getBusinessKey(), ownerFieldOption);
    // 联系人
    List<OptionDTO> contactFieldOption =
        moduleFormService.getBusinessFieldOption(
  ...
  private void updateClue(FollowUpRecord followUpRecord) {
    if (StringUtils.isNotBlank(followUpRecord.getClueId())) {
      FollowUpRecord record =
          extFollowUpRecordMapper.selectRecord(
              null, null, followUpRecord.getClueId(), followUpRecord.getOrganizationId(), "CLUE");
  ...
  private void updateCustomerOrOpportunity(FollowUpRecord followUpRecord) {
    ...
    if (StringUtils.isNotBlank(followUpRecord.getCustomerId())) {
      FollowUpRecord record =
          extFollowUpRecordMapper.selectRecord(
              followUpRecord.getCustomerId(),
              null,
              null,
              followUpRecord.getOrganizationId(),
              "CUSTOMER");
      Optional.ofNullable(record)
          .ifPresent(
              r -> {
    ...
    if (StringUtils.isNotBlank(followUpRecord.getOpportunityId())) {
      FollowUpRecord record =
          extFollowUpRecordMapper.selectRecord(
              null,
              followUpRecord.getOpportunityId(),
              null,
              followUpRecord.getOrganizationId(),
              "CUSTOMER");
      Optional.ofNullable(record)
  ...
  public void deleteByCustomerIds(List<String> customerIds) {
    ...
    List<String> ids = getByCustomerIds(customerIds).stream().map(FollowUpRecord::getId).toList();
    deleteByIds(ids);
  }
  ...
  public void deleteByClueIds(List<String> clueIds) {
    ...
    List<String> ids = getByClueIds(clueIds).stream().map(FollowUpRecord::getId).toList();
    deleteByIds(ids);
  }
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String currentUser, String organizationId) {
    List<RolePermissionDTO> rolePermissions =
        permissionCache.getRolePermissions(currentUser, organizationId);
    // 由于统一的记录/计划页面会展示多个业务模块的数据，因此需要合并多个数据权限, 展示范围更大的数据权限配置
    ResourceTabEnableDTO clueTabConfig =
        PermissionUtils.getTabEnableConfig(
            currentUser, PermissionConstants.CLUE_MANAGEMENT_READ, rolePermissions);
    ResourceTabEnableDTO customerTabConfig =
        PermissionUtils.getTabEnableConfig(
            currentUser, PermissionConstants.CUSTOMER_MANAGEMENT_READ, rolePermissions);
