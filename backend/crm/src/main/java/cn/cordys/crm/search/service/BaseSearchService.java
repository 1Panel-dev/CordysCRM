...
public abstract class BaseSearchService<T extends BasePageRequest, R> {
  ...
  @Resource private BaseMapper<Module> moduleMapper;

  @Resource private BaseMapper<UserSearchConfig> userSearchConfigBaseMapper;

  @Resource private BaseMapper<SearchFieldMaskConfig> searchFieldMaskConfigBaseMapper;
  ...
  @Resource private BaseMapper<CluePool> cluePoolMapper;
  ...
  public FilterCondition getFilterCondition(
      String name, Object value, String operator, String type) {
    ...
    nameCondition.setMultipleValue(
        Strings.CI.equals(type, FieldType.DATA_SOURCE_MULTIPLE.toString()));
  ...
  public List<String> getCustomerIds(String keyword, String orgId) {
    ...
    return customerOptions.stream().map(OptionDTO::getId).toList();
  }
   ...
   */
  public void buildOtherFilterCondition(
      String orgId,
    ...
    if (StringUtils.isBlank(userSearchConfig.getDataSourceType())
        && !Strings.CI.equals(userSearchConfig.getType(), FieldType.PHONE.toString())) {
    ...
    if (Strings.CI.equals(userSearchConfig.getType(), FieldType.PHONE.toString())) {
      ...
      FilterCondition filterCondition =
          getFilterCondition(
              name,
              keyword,
              FilterCondition.CombineConditionOperator.EQUALS.toString(),
    ...
    if (Strings.CI.equals(userSearchConfig.getType(), FieldType.DATA_SOURCE.toString())
        || Strings.CI.equals(
            userSearchConfig.getType(), FieldType.DATA_SOURCE_MULTIPLE.toString())) {
      ...
      extModuleFieldMapper
          .getSourceOptionsByName(tableName, keyword, orgId)
          .forEach(option -> ids.add(option.getId()));
  ...
  public List<SearchFieldMaskConfig> getSearchFieldMaskConfigs(String orgId, String moduleType) {
    ...
    sysWrapper
        .eq(SearchFieldMaskConfig::getModuleType, moduleType)
        .eq(SearchFieldMaskConfig::getOrganizationId, orgId);
  ...
  public List<UserSearchConfig> getUserSearchConfigs(String userId, String orgId) {
    ...
    wrapper.eq(UserSearchConfig::getUserId, userId).eq(UserSearchConfig::getOrganizationId, orgId);
   ...
   * @return 自定义字段值列表
   */
  public List<BaseModuleFieldValue> getBaseModuleFieldValues(
      Set<String> fieldIdSet,
      String id,
      Map<String, List<BaseModuleFieldValue>> moduleFiledMap,
    ...
    Map<String, Object> fieldValueMap =
        baseModuleFieldValues.stream()
            .collect(
                Collectors.toMap(
                    BaseModuleFieldValue::getFieldId, BaseModuleFieldValue::getFieldValue));
    // 获取自定义字段选项，用于补充数据源的值
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(moduleFormConfigDTO, baseModuleFieldValues);
    List<BaseField> fields = moduleFormConfigDTO.getFields();
    Map<String, String> fieldTypeMap =
        fields.stream().collect(Collectors.toMap(BaseField::getId, BaseField::getType));
   ...
   * @return 脱敏后的字段值
   */
  private Object setStringFieldValue(
      SearchFieldMaskConfig searchFieldMaskConfig, Object fieldValue) {
    int length = ((String) fieldValue).length();
    if (Strings.CI.equals(searchFieldMaskConfig.getType(), FieldType.PHONE.toString())
        || Strings.CI.equals(searchFieldMaskConfig.getType(), FieldType.SERIAL_NUMBER.toString())) {
      // fieldValue后6位以*替代
      fieldValue = getPhoneFieldValue(fieldValue, length);
        ...
        || Strings.CI.equals(searchFieldMaskConfig.getType(), FieldType.DATA_SOURCE.toString())) {
   ...
   * @return 内置字段列表
   */
  public <A> List<BaseModuleFieldValue> buildInternalField(
    ...
    for (String s : capitalizedList) {
      try {
        ...
        internalKeyMap.forEach(
            (k, v) -> {
              if (Strings.CI.equals(v, s)) {
                baseModuleFieldValue.setFieldId(k);
              }
            });
        SearchFieldMaskConfig searchFieldMaskConfig =
            searchFieldMaskConfigMap.get(baseModuleFieldValue.getFieldId());
