...
public class UserRoleService {
  ...
  @Resource private BaseMapper<Role> roleMapper;
  ...
  private Map<String, String> getUserAvatarMap(List<RoleUserListResponse> users) {
    ...
    return userExtendMapper.selectByIds(userIds.toArray(String[]::new)).stream()
  ...
  private Map<String, String> getDeptNameMap(List<RoleUserListResponse> users) {
    ...
    return departmentMapper
        .selectByIds(
            users.stream()
                .map(RoleUserListResponse::getDepartmentId)
                .distinct()
                .toArray(String[]::new))
        .stream()
  ...
  private Map<String, List<UserRoleConvert>> getUserRoleMap(
      String orgId, List<RoleUserListResponse> users) {
    if (CollectionUtils.isEmpty(users)) {
      return Map.of();
    }
    List<String> userIds =
        users.stream().map(RoleUserListResponse::getUserId).collect(Collectors.toList());
  ...
  public List<DeptUserTreeNode> getDeptUserTree(String orgId, String roleId) {
    ...
    Set<String> currentRoleUserIds =
        new HashSet<>(extUserRoleMapper.getUserIdsByRoleIds(List.of(roleId)));
    userNodes.forEach(
        userNode -> {
  ...
  public List<RoleUserTreeNode> getRoleUserTree(String orgId, String roleId) {
    ...
    List<RoleUserTreeNode> treeNodes =
        list.stream()
            .filter(role -> !StringUtil.equals(roleId, role.getId()))
            .map(
                (role) -> {
  ...
  public void relateUser(RoleUserRelateRequest request, String operator, String orgId) {
    ...
    SubListUtils.dealForSubList(
        ...
        (subUserIds) -> {
          ...
          // 过滤掉已删除的用户
          subUserIds =
              subUserIds.stream().filter(userId -> userNameMap.get(userId) != null).toList();

          List<String> currentRoleUserIds =
              extUserRoleMapper.getUserIdsByRoleIds(List.of(request.getRoleId()));
          // 去除已关联的用户，添加未关联的用户
          List<UserRole> userRoles =
              ListUtils.subtract(subUserIds, currentRoleUserIds).stream()
                  .map(
                      userId -> {
              ...
              u -> {
                ...
                LogDTO logDTO =
                    new LogDTO(
                        ...
                        operator,
                        LogType.ADD_USER,
                        LogModule.SYSTEM_ROLE,
                        role.getName());
                String detail =
                    Translator.getWithArgs(
                        "role.add_user", role.getName(), userNameMap.get(u.getUserId()));
  ...
  public void deleteRoleUser(String id, String userId, String orgId) {
    ...
    LogDTO logDTO =
        new LogDTO(
            ...
            LogModule.SYSTEM_ROLE,
            role.getName());
    String detail = Translator.getWithArgs("role.remove_user", role.getName(), userName);
    logDTO.setDetail(detail);
  ...
  public void batchDeleteRoleUser(List<String> ids, String userId, String orgId) {
    ...
    userRoles.forEach(
        userRole -> {
          ...
          String detail =
              Translator.getWithArgs(
                  "role.remove_user", role.getName(), userNameMap.get(userRole.getUserId()));
          logDTO.setDetail(detail);
          logs.add(logDTO);
  ...
  public List<RoleUserOptionResponse> getUserOptionByRoleId(String organizationId, String roleId) {
    List<RoleUserOptionResponse> roleUserOptions =
        extUserRoleMapper.selectUserOptionByRoleId(organizationId, roleId);
    Set<String> roleUserIdSet =
        new HashSet<>(extUserRoleMapper.getUserIdsByRoleIds(List.of(roleId)));

    roleUserOptions.forEach(
        userOption -> userOption.setEnabled(!roleUserIdSet.contains(userOption.getId())));
