 ...
 * 商机报价单即将到期和到期提醒监听器
 *
 * <p>该监听器负责监听执行事件，当触发时检查并提醒到期的商机报价单。
 */
@Component
@Slf4j
public class NoticeExpireJob {
  ...
  @Resource private ExtMessageTaskConfigMapper extMessageTaskConfigMapper;
  ...
  @Resource private BaseMapper<Contract> contractBaseMapper;
   ...
   * 定时检查到期或即将到期的消息通知任务
   *
   * <p>该方法每天8点执行一次，检查商机报价单，回款计划以及合同的到期情况，并发送相应的通知提醒。
   */
  ...
  private void contractExpiredRemind() {
    ...
    for (String organizationId : organizationIds) {
      ...
      if (expiredEvent == null
          || (!expiredEvent.getDingTalkEnable()
              && !expiredEvent.getEmailEnable()
              && !expiredEvent.getSysEnable()
              && !expiredEvent.getWeComEnable()
              && !expiredEvent.getLarkEnable())) {
        log.info("组织{}合同到期提醒未开启", organizationId);
        continue;
      }
      // 查询合同到期的消息通知配置
      MessageTaskConfig expiredConfig =
          extMessageTaskConfigMapper.getConfigByModuleAndEvent(
              NotificationConstants.Module.CONTRACT,
              NotificationConstants.Event.CONTRACT_EXPIRED,
              organizationId);
      ...
      MessageTaskConfigDTO expiredConfigDTO =
          JSON.parseObject(expiredConfig.getValue(), MessageTaskConfigDTO.class);
      long timestamp = LocalDate.now().atStartOfDay(ZoneId.systemDefault()).toEpochSecond() * 1000;

      long timestampOld =
          LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toEpochSecond() * 1000;
      ...
      for (Contract contract : contractList) {
        ...
        if (!Strings.CI.equals(contract.getStage(), ContractStage.COMPLETED_PERFORMANCE.name())
            && !Strings.CI.equals(contract.getStage(), ContractStage.ARCHIVED.name())) {
          contract.setStage(ContractStage.COMPLETED_PERFORMANCE.name());
          batchUpdateMapper.updateStage(
              contract.getId(),
              contract.getStage(),
  ...
  private void contractExpiringRemind() {
    ...
    for (String organizationId : organizationIds) {
      ...
      MessageTask expiringEvent =
          extMessageTaskMapper.getMessageByModuleAndEvent(
              NotificationConstants.Module.CONTRACT,
              NotificationConstants.Event.CONTRACT_EXPIRING,
              organizationId);
      // 判断是否开启
      if (expiringEvent == null
          || (!expiringEvent.getDingTalkEnable()
              && !expiringEvent.getEmailEnable()
              && !expiringEvent.getSysEnable()
              && !expiringEvent.getWeComEnable()
              && !expiringEvent.getLarkEnable())) {
        log.info("组织{}合同即将到期提醒未开启", organizationId);
        continue;
      }
      // 查询合同即将到期的消息通知配置
      MessageTaskConfig expiringConfig =
          extMessageTaskConfigMapper.getConfigByModuleAndEvent(
              NotificationConstants.Module.CONTRACT,
              NotificationConstants.Event.CONTRACT_EXPIRING,
              organizationId);
      ...
      MessageTaskConfigDTO expiringConfigDTO =
          JSON.parseObject(expiringConfig.getValue(), MessageTaskConfigDTO.class);
      ...
      for (TimeDTO timeDTO : expiringTimeList) {
        if (timeDTO.getTimeUnit().equals("DAY")) {
          long startTime =
              LocalDate.now()
                      .plusDays(timeDTO.getTimeValue() + 1)
                      .atStartOfDay(ZoneId.systemDefault())
                      .toEpochSecond()
                  * 1000;
          long endTime =
              LocalDate.now()
                      .plusDays(timeDTO.getTimeValue())
                      .atStartOfDay(ZoneId.systemDefault())
                      .toEpochSecond()
                  * 1000;
        ...
        if (timeDTO.getTimeUnit().equals("DAY")) {
          ...
          long endTime =
              LocalDate.now()
                      ...
                      .toEpochSecond()
                  * 1000;
          List<OpportunityQuotation> quotationList =
              getOpportunityQuotationList(organizationId, startTime, endTime);
          ...
          for (OpportunityQuotation opportunityQuotation : quotationList) {
            ...
            Opportunity opportunity =
                opportunityBaseMapper.selectByPrimaryKey(opportunityQuotation.getOpportunityId());
            if (opportunity == null) {
            ...
            sendNotice(
                expiringConfigDTO,
                NotificationConstants.Module.OPPORTUNITY,
                opportunityQuotation.getCreateUser(),
                opportunityQuotation.getOrganizationId(),
                NotificationConstants.Event.BUSINESS_QUOTATION_EXPIRING,
                customer.getName(),
                opportunityQuotation.getCreateUser(),
                opportunityQuotation.getCreateUser(),
  ...
  private void quotationExpiredRemind() {
    ...
    for (String organizationId : organizationIds) {
      ...
      long timestampOld =
          LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toEpochSecond() * 1000;
      List<OpportunityQuotation> quotationList =
          getOpportunityQuotationList(organizationId, timestamp, timestampOld);
      if (CollectionUtils.isEmpty(quotationList)) {
   ...
   * @return 报价单列表
   */
  private List<OpportunityQuotation> getOpportunityQuotationList(
      String organizationId, long timestamp, long timestampOld) {
    return extOpportunityQuotationMapper.getQuotationByTimestamp(
        timestamp, timestampOld, organizationId);
  }
   ...
   * @return 回款计划列表
   */
  private List<ContractPaymentPlan> getContractPaymentPlanList(
      String organizationId, long timestamp, long timestampOld) {
        ...
        if (timeDTO.getTimeUnit().equals("DAY")) {
          ...
          long endTime =
              LocalDate.now()
                      ...
                      .toEpochSecond()
                  * 1000;
          List<ContractPaymentPlan> paymentPlanList =
              getContractPaymentPlanList(organizationId, startTime, endTime);
          ...
          for (ContractPaymentPlan paymentPlan : paymentPlanList) {
            ...
            sendNotice(
                expiringConfigDTO,
                NotificationConstants.Module.CONTRACT,
                paymentPlan.getCreateUser(),
                paymentPlan.getOrganizationId(),
                NotificationConstants.Event.CONTRACT_PAYMENT_EXPIRING,
                customer.getName(),
                paymentPlan.getCreateUser(),
                paymentPlan.getOwner(),
                timeDTO.getTimeValue(),
                paymentPlan.getId());
          }
        }
  ...
  private void contractPaymentPlanExpiredRemind() {
    ...
    for (String organizationId : organizationIds) {
      ...
      long timestampOld =
          LocalDate.now().minusDays(1).atStartOfDay(ZoneId.systemDefault()).toEpochSecond() * 1000;
      List<ContractPaymentPlan> paymentPlanList =
          getContractPaymentPlanList(organizationId, timestamp, timestampOld);
      if (CollectionUtils.isEmpty(paymentPlanList)) {
  ...
  private void sendNotice(
    ...
    List<String> receiveUserIds =
        commonNoticeSendService.getNoticeReceiveUserIds(
            messageTaskConfigDTO, createUser, owner, orgId);
    ...
    commonNoticeSendService.sendNotice(
        model, event, paramMap, userId, orgId, receiveUserIds, false);
  }
}
