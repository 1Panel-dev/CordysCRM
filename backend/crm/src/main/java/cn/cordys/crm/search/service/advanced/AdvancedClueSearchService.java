...
@Service
public class AdvancedClueSearchService
    extends BaseSearchService<CluePageRequest, AdvancedClueResponse> {
  ...
  @Override
  public PagerWithOption<List<AdvancedClueResponse>> startSearch(
      CluePageRequest request, String orgId, String userId) {
  ...
  public Map<String, List<OptionDTO>> buildClueOptionMap(
      String orgId, List<ClueListResponse> list, List<AdvancedClueResponse> buildList) {
    // 处理自定义字段选项数据
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList, AdvancedClueResponse::getOwner, AdvancedClueResponse::getOwnerName);
  ...
  public List<AdvancedClueResponse> buildClueList(
      List<ClueListResponse> list, String orgId, String userId) {
    if (CollectionUtils.isEmpty(list)) {
