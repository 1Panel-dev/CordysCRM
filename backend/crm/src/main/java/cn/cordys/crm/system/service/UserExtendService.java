...
public class UserExtendService {
  ...
  public List<String> getScopeOwnerIds(List<String> scopeIds, String orgId) {
    ...
    List<String> ownerIds =
        scopes.stream()
            .filter(scope -> Strings.CS.equals(scope.getScope(), ScopeKey.USER.name()))
            .map(ScopeNameDTO::getId)
            .collect(Collectors.toList());
    List<ScopeNameDTO> roleList =
        scopes.stream()
            .filter(scope -> Strings.CS.equals(scope.getScope(), ScopeKey.ROLE.name()))
            .toList();
    List<ScopeNameDTO> dptList =
        scopes.stream()
            .filter(scope -> Strings.CS.equals(scope.getScope(), ScopeKey.DEPARTMENT.name()))
            .toList();
    ...
    if (!CollectionUtils.isEmpty(dptList)) {
      ...
      List<OrganizationUser> organizationUsers =
          organizationUserMapper.selectListByLambda(queryWrapper);
  ...
  public Map<String, List<String>> getMultiScopeMap(List<String> ownerIds, String organizationId) {
    ...
    Map<String, List<String>> ownerRoleMap =
        allOwnerRoles.stream()
            .collect(
   ...
   */
  public Map<String, List<String>> getOwnerAncestorMap(
    ...
    wrapper
        .in(OrganizationUser::getUserId, ownerIds)
        .eq(OrganizationUser::getOrganizationId, organizationId);
    List<OrganizationUser> organizationUsers = organizationUserMapper.selectListByLambda(wrapper);
    if (organizationUsers.isEmpty()) {
      return ancestorMap;
    }
    Map<String, String> ownerDepIdMap =
        organizationUsers.stream()
            .collect(
                Collectors.toMap(OrganizationUser::getUserId, OrganizationUser::getDepartmentId));
  ...
  public List<String> getParentDepartmentIds(String userId, String currentOrgId) {
    ...
    wrapper
        ...
        .eq(OrganizationUser::getOrganizationId, currentOrgId);
    List<OrganizationUser> organizationUsers = organizationUserMapper.selectListByLambda(wrapper);
   ...
   * @return 是否含有重复对象元素
   */
  public boolean hasDuplicateScopeObj(
      List<String> scopeIds, List<String> targetScopeIds, String currentOrgId) {
