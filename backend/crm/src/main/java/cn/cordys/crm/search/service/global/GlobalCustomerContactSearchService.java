...
@Service
public class GlobalCustomerContactSearchService
    extends BaseSearchService<BasePageRequest, GlobalCustomerContactResponse> {
  ...
  @Override
  public Pager<List<GlobalCustomerContactResponse>> startSearchNoOption(
      ...
      for (UserSearchConfig userSearchConfig : contactSearchConfigs) {
        ...
        if (StringUtils.isBlank(userSearchConfig.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
                ...
                BusinessModuleField.CUSTOMER_CONTACT_CUSTOMER.getBusinessKey())
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(),
                BusinessModuleField.CUSTOMER_CONTACT_PHONE.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
        }
    ...
    // 搜索客户
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<GlobalCustomerContactResponse> globalContactResponses =
        extCustomerContactMapper.globalSearchList(request, orgId);
    if (CollectionUtils.isEmpty(globalContactResponses)) {
      return PageUtils.setPageInfo(page, List.of());
    }
    // 获取系统设置的脱敏字段
    List<SearchFieldMaskConfig> searchFieldMaskConfigs =
        getSearchFieldMaskConfigs(orgId, SearchModuleEnum.SEARCH_ADVANCED_CONTACT);
    List<GlobalCustomerContactResponse> buildList =
        buildListData(globalContactResponses, orgId, userId, searchFieldMaskConfigs, fieldIdSet);
    return PageUtils.setPageInfo(page, buildList);
  }

  private List<GlobalCustomerContactResponse> buildListData(
      List<GlobalCustomerContactResponse> list,
      String orgId,
      String userId,
      List<SearchFieldMaskConfig> searchFieldMaskConfigs,
      Set<String> fieldIdSet) {
    List<String> customerContactIds =
        list.stream().map(GlobalCustomerContactResponse::getId).collect(Collectors.toList());
    Map<String, List<BaseModuleFieldValue>> customerFiledMap =
        customerContactFieldService.getResourceFieldMap(customerContactIds, true);

    List<String> ownerIds =
        list.stream().map(GlobalCustomerContactResponse::getOwner).distinct().toList();
    ...
    List<String> customerIds =
        list.stream()
            .map(GlobalCustomerContactResponse::getCustomerId)
            .distinct()
            ...
            searchFieldMaskConfigMap.forEach(
                (fieldId, searchFieldMaskConfig) -> {
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "customerId")) {
                    customerContactResponse.setCustomerName(
                        (String) getInputFieldValue(customerName, customerName.length()));
                  }
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "name")) {
                    customerContactResponse.setName(
                        (String)
                            getInputFieldValue(
                                customerContactResponse.getName(),
                                customerContactResponse.getName().length()));
                  }
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "phone")
                      && StringUtils.isNotBlank(customerContactResponse.getPhone())) {
                    customerContactResponse.setPhone(
                        (String)
                            getPhoneFieldValue(
                                customerContactResponse.getPhone(),
                                customerContactResponse.getPhone().length()));
                  }
                });
          }
