...
public class ApprovalActionService {
	...
	public void addSign(ApprovalAddSignRequest request, String userId, String orgId) {
		// 审批流是否允许加签
		ApprovalFlow approvalFlow = getFlowOfInstanceId(request.getInstanceId());
	...
	public void approve(ApprovalActionRequest request, String currentUserId, String currentOrgId) {
		...
		List<ApprovalTask> appendTasks = appendTask(currentTask, currentUserId);
	 ...
	 * @return 审批流
	 */
	private ApprovalFlow getFlowOfInstanceId(String id) {
		ApprovalInstance approvalInstance = approvalInstanceMapper.selectByPrimaryKey(id);
		return approvalFlowMapper.selectByPrimaryKey(approvalInstance.getFlowId());
	}
	 ...
	 */
	private List<ApprovalTask> appendTask(ApprovalTask currentTask, String currentUserId) {
		List<ApprovalTask> approvalTasks = new ArrayList<>();
		...
		if (Strings.CI.equals(currentTask.getType(), ApprovalTaskType.SN.name())) {
			...
			List<BaseModuleFieldValue> fvs = approvalResourceService.compressResourceDetail(instance.getType(), instance.getResourceId());
