...
public class DashboardService extends DashboardSortService {
  ...
  @Resource private ExtDashboardMapper extDashboardMapper;
  @Resource private BaseMapper<Dashboard> dashboardMapper;
  @Resource private UserExtendService userExtendService;
  @Resource private DashboardModuleService dashboardModuleService;
  @Resource private BaseMapper<DashboardCollection> dashboardCollectionMapper;
  @Resource private ExtDashboardCollectionMapper extDashboardCollectionMapper;
  ...
  public void updateDashboard(DashboardUpdateRequest request, String orgId, String userId) {
    ...
    // 添加日志上下文
    String resourceName =
        Optional.ofNullable(dashboard.getName()).orElse(originalDashboard.getName());
    OperationLogContext.setContext(
        LogContextInfo.builder()
   ...
   * @return
   */
  public Pager<List<DashboardPageResponse>> getList(
      DashboardPageRequest request, String userId, String orgId) {
    ...
    List<DashboardPageResponse> dashboardList =
        extDashboardMapper.list(request, userId, orgId, departmentIds);
  ...
  private void handleData(List<DashboardPageResponse> dashboardList, String userId) {
    if (CollectionUtils.isNotEmpty(dashboardList)) {
      ...
      dashboardList.forEach(
          dashboard -> {
            dashboard.setMembers(
                userExtendService.getScope(JSON.parseArray(dashboard.getScopeId(), String.class)));
   ...
   * @return
   */
  public List<DashboardPageResponse> collectList(
      BasePageRequest request, String userId, String orgId) {
    List<DashboardPageResponse> dashboardList =
        extDashboardCollectionMapper.collectList(request, userId, orgId);
    handleData(dashboardList, userId);
  ...
  public void editPos(DashboardEditPosRequest request, String userId, String orgId) {
    ...
    if (!Strings.CS.equals(request.getDashboardModuleId(), dashboard.getDashboardModuleId())) {
      dashboardModuleService.checkDashboardModule(request.getDashboardModuleId());
      checkDashboardName(
          dashboard.getName(), request.getDashboardModuleId(), orgId, dashboard.getId());
