...
public class OpportunityService {
  ...
  public PagerWithOption<List<OpportunityListResponse>> list(
    ...
    List<OpportunityListResponse> list =
        extOpportunityMapper.list(request, orgId, userId, deptDataPermission, source);
  ...
  public OpportunitySearchStatisticResponse searchStatistic(
      OpportunitySearchStatisticRequest request,
      String userId,
      String orgId,
      DeptDataPermissionDTO deptDataPermission) {
    OpportunitySearchStatisticResponse response =
        extOpportunityMapper.searchStatistic(request, orgId, userId, deptDataPermission);
  ...
  public Map<String, List<OptionDTO>> buildOptionMap(
      String orgId, List<OpportunityListResponse> list, List<OpportunityListResponse> buildList) {
    // 处理自定义字段选项数据
  ...
  public List<OpportunityListResponse> buildListData(
      List<OpportunityListResponse> list, String orgId) {
   ...
   */
  @OperationLog(
      module = LogModule.OPPORTUNITY_INDEX,
      type = LogType.ADD,
  ...
  public Opportunity add(OpportunityAddRequest request, String operatorId, String orgId) {
    ...
    // 自定义字段
    opportunityFieldService.saveModuleField(
        opportunity, orgId, operatorId, request.getModuleFields(), false);
  ...
  public Opportunity update(OpportunityUpdateRequest request, String userId, String orgId) {
    ...
    Optional.ofNullable(oldOpportunity)
        .ifPresentOrElse(
            item -> {
              ...
              List<BaseModuleFieldValue> originCustomerFields =
                  opportunityFieldService.getModuleFieldValuesByResourceId(request.getId());
              if (BooleanUtils.isTrue(request.getAgentInvoke())) {
                opportunityFieldService.updateModuleFieldByAgent(
                    updateOpportunity,
              ...
              baseService.handleUpdateLog(
                  ...
                  originCustomerFields,
                  request.getModuleFields(),
                  oldOpportunity.getId(),
                  oldOpportunity.getName());
            },
            () -> {
  ...
  private Opportunity newOpportunity(
      Opportunity item, OpportunityUpdateRequest request, String userId) {
  ...
  public void delete(String id, String userId, String orgId) {
    ...
    commonNoticeSendService.sendNotice(
        ...
        NotificationConstants.Event.BUSINESS_DELETED,
        opportunity.getName(),
        userId,
        orgId,
        List.of(opportunity.getOwner()),
        true);
  }
  ...
  public void transfer(OpportunityTransferRequest request, String userId, String orgId) {
    List<StageConfigResponse> stageConfigList =
        extOpportunityStageConfigMapper.getStageConfigList(orgId);
    // 过滤出成功阶段
    StageConfigResponse successConfig =
        stageConfigList.stream()
            .filter(
                config ->
                    Strings.CI.equals(config.getType(), OpportunityStageType.END.name())
                        && Strings.CI.equals(config.getRate(), "100"))
            .findFirst()
            .get();
    ...
    for (int i = 0; i < ids.size(); i++) {
      batchUpdateMapper.transfer(
          request.getOwner(),
          userId,
          ids.get(i),
          System.currentTimeMillis(),
          nextPos + i,
          stageConfigList.getFirst().getId());
    }
    ...
    opportunityList.forEach(
        opportunity -> {
          ...
          extCustomerContactMapper.updateContactById(
              opportunity.getContactId(), request.getOwner());
        });
  ...
  private void sendTransferNotice(
      List<Opportunity> opportunityList, String toUser, String userId, String orgId) {
    opportunityList.forEach(
        opportunity ->
            commonNoticeSendService.sendNotice(
                NotificationConstants.Module.OPPORTUNITY,
                NotificationConstants.Event.BUSINESS_TRANSFER,
                opportunity.getName(),
                userId,
                orgId,
                List.of(toUser),
  ...
  public OpportunityDetailResponse get(String id) {
    ...
    List<BaseModuleFieldValue> fieldValueList =
        opportunityFieldService.getModuleFieldValuesByResourceId(id);
    response.setModuleFields(fieldValueList);
    List<String> userIds =
        Stream.of(
                Arrays.asList(
                    response.getCreateUser(),
                    response.getUpdateUser(),
                    response.getOwner(),
    ...
    Map<String, String> contactMap =
        baseService.getContactMap(
            StringUtils.isEmpty(response.getContactId()) ? null : List.of(response.getContactId()));
    Map<String, OpportunityRule> ownersDefaultRuleMap =
        opportunityRuleService.getOwnersDefaultRuleMap(
            List.of(response.getOwner()), response.getOrganizationId());
    ...
    List<StageConfigResponse> stageConfigList =
        extOpportunityStageConfigMapper.getStageConfigList(response.getOrganizationId());
    Map<String, StageConfigResponse> endConfigMaps =
        stageConfigList.stream()
            .filter(config -> Strings.CI.equals(config.getType(), OpportunityStageType.END.name()))
            .collect(Collectors.toMap(StageConfigResponse::getId, Function.identity()));
    ...
    // 计算保留天数(成功失败阶段不计算)
    response.setReservedDays(
        endConfigMaps.containsKey(response.getStage())
            ? null
            : opportunityRuleService.calcReservedDay(
                ownersDefaultRuleMap.get(response.getOwner()), response.getCreateTime()));
    ...
    DictConfigDTO dictConf =
        dictService.getDictConf(
            DictModule.OPPORTUNITY_FAIL_RS.name(), response.getOrganizationId());
    List<Dict> dictList = dictConf.getDictList();
    Map<String, String> dictMap =
        dictList.stream().collect(Collectors.toMap(Dict::getId, Dict::getName));
    ...
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerFormConfig, fieldValueList);

    // 补充负责人选项
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            response, OpportunityDetailResponse::getOwner, OpportunityDetailResponse::getOwnerName);
    ...
    List<OptionDTO> contactFieldOption =
        moduleFormService.getBusinessFieldOption(
            response,
            OpportunityDetailResponse::getContactId,
            OpportunityDetailResponse::getContactName);
    optionMap.put(BusinessModuleField.OPPORTUNITY_CONTACT.getBusinessKey(), contactFieldOption);

    List<OptionDTO> customerOption =
        moduleFormService.getBusinessFieldOption(
            response,
            OpportunityDetailResponse::getCustomerId,
            OpportunityDetailResponse::getCustomerName);
    ...
    // 附件信息
    response.setAttachmentMap(
        moduleFormService.getAttachmentMap(customerFormConfig, fieldValueList));
  ...
  public void updateStage(OpportunityStageRequest request, String orgId) {
    ...
    final List<StageConfigResponse> stageConfigList =
        extOpportunityStageConfigMapper.getStageConfigList(orgId);

    final Optional<StageConfigResponse> successOpt =
        stageConfigList.stream()
            .filter(
                cfg ->
    ...
    final Optional<StageConfigResponse> failOpt =
        stageConfigList.stream()
            .filter(
                cfg ->
                    Strings.CI.equals(cfg.getType(), OpportunityStageType.END.name())
    ...
    final boolean isSuccessStage =
        successOpt.map(cfg -> Strings.CI.equals(request.getStage(), cfg.getId())).orElse(false);
    final boolean isFailStage =
        failOpt.map(cfg -> Strings.CI.equals(request.getStage(), cfg.getId())).orElse(false);
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.OPPORTUNITY_MANAGEMENT_READ, rolePermissions);
  }
  ...
  public void downloadImportTpl(HttpServletResponse response, String currentOrg) {
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            response,
            moduleFormService.getCustomImportHeadsNoRef(FormKey.OPPORTUNITY.getKey(), currentOrg),
            Translator.get("opportunity.import_tpl.name"),
            ...
            new CustomTemplateWriteHandler(
                moduleFormService.getAllCustomImportFields(
                    FormKey.OPPORTUNITY.getKey(), currentOrg)),
  ...
  public ImportResponse realImport(MultipartFile file, String currentOrg, String currentUser) {
    try {
      List<StageConfigResponse> stageConfigList =
          extOpportunityStageConfigMapper.getStageConfigList(currentOrg);

      List<BaseField> fields =
          moduleFormService.getAllFields(FormKey.OPPORTUNITY.getKey(), currentOrg);
      long nextPos = getNextPos(currentOrg, stageConfigList.getFirst().getId());
      CustomImportAfterDoConsumer<Opportunity, BaseResourceSubField> afterDo =
          (opportunities, opportunityFields, opportunityFieldBlobs) -> {
            List<LogDTO> logs = new ArrayList<>();
            ...
            opportunityFieldMapper.batchInsert(
                opportunityFields.stream()
                    .map(field -> BeanUtils.copyBean(new OpportunityField(), field))
                    .toList());
            opportunityFieldBlobMapper.batchInsert(
                opportunityFieldBlobs.stream()
                    .map(field -> BeanUtils.copyBean(new OpportunityFieldBlob(), field))
                    .toList());
            // record logs
            logService.batchAdd(logs);
          };
      CustomFieldImportEventListener<Opportunity> eventListener =
  ...
  private ImportResponse checkImportExcel(MultipartFile file, String currentOrg) {
    try {
      List<BaseField> fields =
          moduleFormService.getAllCustomImportFields(FormKey.OPPORTUNITY.getKey(), currentOrg);
      CustomFieldCheckEventListener eventListener =
          new CustomFieldCheckEventListener(fields, "opportunity", "opportunity_field", currentOrg);
      FastExcelFactory.read(file.getInputStream(), eventListener)
          .headRowNumber(1)
          .ignoreEmptyRow(true)
          .sheet()
  ...
  public void batchUpdate(ResourceBatchEditRequest request, String userId, String organizationId) {
    BaseField field =
        opportunityFieldService.getAndCheckField(request.getFieldId(), organizationId);
    if (Strings.CS.equals(
        field.getBusinessKey(), BusinessModuleField.OPPORTUNITY_OWNER.getBusinessKey())) {
      // 修改负责人，走批量转移接口
    ...
    if (Strings.CS.equals(
        field.getBusinessKey(), BusinessModuleField.OPPORTUNITY_PRODUCTS.getBusinessKey())) {
    ...
    opportunityFieldService.batchUpdate(
        request,
        field,
        originOpportunities,
        Opportunity.class,
        LogModule.OPPORTUNITY_INDEX,
        extOpportunityMapper::batchUpdate,
        userId,
        organizationId);
  }
  ...
  public List<ChartResult> chart(
    ...
    List<ChartResult> chartResults =
        extOpportunityMapper.chart(chartAnalysisDbRequest, userId, orgId, deptDataPermission);
  ...
  public List<BaseField> getChartFields(String orgId) {
    ...
    List<OptionProp> options =
        extOpportunityStageConfigMapper.getStageConfigList(orgId).stream()
            .map(
                config -> {
