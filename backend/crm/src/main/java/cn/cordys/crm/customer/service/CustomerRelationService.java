...
public class CustomerRelationService {
  @Resource private BaseMapper<CustomerRelation> customerRelationMapper;
  ...
  public List<CustomerRelationListResponse> list(String customerId) {
    ...
    // 查询上级集团的客户关系
    List<CustomerRelationListResponse> result =
        sourceRelations.stream()
            .map(
                item -> {
    ...
    // 查询下级子公司的客户关系
    List<CustomerRelationListResponse> subsidiaryCustomer =
  ...
  public void save(String customerId, List<CustomerRelationSaveRequest> requests) {
    ...
    // 再创建
    List<CustomerRelation> relations =
        requests.stream()
            .filter(
                request ->
                    StringUtils.isNotBlank(request.getCustomerId())
                        && StringUtils.isNotBlank(request.getRelationType()))
            .map(item -> getCustomerRelation(item, customerId))
            .toList();
  ...
  /**
   * 校验添加的子公司是否已经有所属的集团了 一个公司只能有一个集团
   *
   * @param customerId
   * @param relations
   */
  private void checkTargetCustomer(String customerId, List<CustomerRelation> relations) {
    List<String> targetCustomerIds =
        relations.stream()
            ...
            .filter(targetCustomerId -> !Strings.CS.equals(targetCustomerId, customerId))
            .toList();
    ...
    List<CustomerRelation> customerRelations =
        customerRelationMapper.selectListByLambda(customerRelation).stream()
    ...
    if (!customerRelations.isEmpty()) {
      ...
      String userNames =
          customers.stream().map(OptionDTO::getName).distinct().collect(Collectors.joining(","));
      throw new GenericException(
          Translator.getWithArgs("customer.relation.target_customer.exist", userNames));
    }
  }
  ...
  private CustomerRelation getCustomerRelation(
      CustomerRelationSaveRequest request, String customerId) {
