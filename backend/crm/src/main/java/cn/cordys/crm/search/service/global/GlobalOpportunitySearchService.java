...
@Service
public class GlobalOpportunitySearchService
    extends BaseSearchService<BasePageRequest, GlobalOpportunityResponse> {
  ...
  @Override
  public Pager<List<GlobalOpportunityResponse>> startSearchNoOption(
    ...
    if (CollectionUtils.isNotEmpty(userSearchConfigs)) {
      List<UserSearchConfig> opportunitySearchConfigs =
          userSearchConfigs.stream()
              .filter(
                  t ->
                      Strings.CI.equals(
                          t.getModuleType(), SearchModuleEnum.SEARCH_ADVANCED_OPPORTUNITY))
              .toList();
      ...
      for (UserSearchConfig userSearchConfig : opportunitySearchConfigs) {
        ...
        if (StringUtils.isBlank(userSearchConfig.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
                ...
                BusinessModuleField.OPPORTUNITY_OWNER.getBusinessKey())
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(),
                BusinessModuleField.OPPORTUNITY_PRODUCTS.getBusinessKey())
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(),
                BusinessModuleField.OPPORTUNITY_CUSTOMER_NAME.getBusinessKey())
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(),
                BusinessModuleField.OPPORTUNITY_NAME.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
        }
      ...
      if (CollectionUtils.isNotEmpty(list)) {
        FilterCondition customerCondition =
            getFilterCondition(
                "customerId",
                list,
    ...
    // 查询重复商机列表
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<GlobalOpportunityResponse> globalOpportunityResponses =
        extOpportunityMapper.globalSearchList(request, orgId);
    if (CollectionUtils.isEmpty(globalOpportunityResponses)) {
      return PageUtils.setPageInfo(page, List.of());
    }
    // 获取系统设置的脱敏字段
    List<SearchFieldMaskConfig> searchFieldMaskConfigs =
        getSearchFieldMaskConfigs(orgId, SearchModuleEnum.SEARCH_ADVANCED_OPPORTUNITY);
    List<GlobalOpportunityResponse> buildList =
        buildListData(
            globalOpportunityResponses,
            orgId,
            userId,
  ...
  public List<GlobalOpportunityResponse> buildListData(
      List<GlobalOpportunityResponse> list,
      String orgId,
      String userId,
      List<SearchFieldMaskConfig> searchFieldMaskConfigs,
    ...
    List<String> opportunityIds =
        list.stream().map(GlobalOpportunityResponse::getId).collect(Collectors.toList());
    ...
    List<String> ownerIds =
        list.stream().map(GlobalOpportunityResponse::getOwner).distinct().toList();
    ...
    Map<String, Opportunity> internalKeyValueMap =
        opportunities.stream().collect(Collectors.toMap(Opportunity::getId, t -> t));
    // 处理自定义字段选项数据
    ...
    list.forEach(
        opportunityListResponse -> {
          ...
          if (CollectionUtils.isNotEmpty(fieldIdSet)) {
            List<BaseModuleFieldValue> returnOpportunityFields =
                getBaseModuleFieldValues(
                    fieldIdSet,
                    opportunityListResponse.getId(),
                    opportunityFiledMap,
          ...
          List<String> productNames =
              getProductNames(opportunityListResponse.getProducts(), productNameMap);
            ...
            searchFieldMaskConfigMap.forEach(
                (fieldId, searchFieldMaskConfig) -> {
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "name")) {
                    opportunityListResponse.setName(
                        (String)
                            getInputFieldValue(
                                opportunityListResponse.getName(),
                                opportunityListResponse.getName().length()));
                  }
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "contactId")
                      && StringUtils.isNotBlank(opportunityListResponse.getContactName())) {
                    opportunityListResponse.setContactName(
                        (String)
                            getInputFieldValue(
                                opportunityListResponse.getContactName(),
                                opportunityListResponse.getContactName().length()));
                  }
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "customerId")
                      && StringUtils.isNotBlank(opportunityListResponse.getCustomerName())) {
                    opportunityListResponse.setCustomerName(
                        (String)
                            getInputFieldValue(
                                opportunityListResponse.getCustomerName(),
                                opportunityListResponse.getCustomerName().length()));
                  }
