...
public class ThirdDepartmentService {
  ...
  @Resource private WeComDepartmentService weComDepartmentService;

  @Resource private DingTalkDepartmentService dingTalkDepartmentService;

  @Resource private LarkDepartmentService larkDepartmentService;
  ...
  private void performSync(String operatorId, String orgId, String type) {
    ...
    var syncStatus =
        integrationConfigService.getSyncStatus(
    ...
    switch (deptType) {
      case WECOM -> {
        ...
        var departmentIds =
            departments.stream().map(ThirdDepartment::getId).map(Long::parseLong).toList();
        ...
        switch (deptType) {
          case WECOM -> {
            WecomThirdConfigRequest config =
                JSON.MAPPER.convertValue(thirdConfig.getConfig(), WecomThirdConfigRequest.class);
            yield tokenService.getAssessToken(config.getCorpId(), config.getAppSecret());
          }
          case DINGTALK -> {
            DingTalkThirdConfigRequest dingTalkConfig =
                JSON.MAPPER.convertValue(thirdConfig.getConfig(), DingTalkThirdConfigRequest.class);
            yield tokenService.getDingTalkToken(
                dingTalkConfig.getAgentId(), dingTalkConfig.getAppSecret());
          }
          case LARK -> {
            LarkThirdConfigRequest larkConfig =
                JSON.MAPPER.convertValue(thirdConfig.getConfig(), LarkThirdConfigRequest.class);
  ...
  /** 同步部门和用户数据 */
  private void syncDepartmentsAndUsers(
      String orgId,
      List<ThirdDepartment> departments,
      Map<String, List<ThirdUser>> departmentUserMap,
      boolean isUpdate,
  ...
  private void logSyncOperation(LogService logService, String orgId, String operatorId) {
    ...
    LogDTO logDTO =
        new LogDTO(
            orgId, operatorId, operatorId, LogType.SYNC, LogModule.SYSTEM_ORGANIZATION, detail);
