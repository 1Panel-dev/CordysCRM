...
public class PoolCustomerController {
  ...
  @Resource private CustomerPoolExportService customerPoolExportService;
  ...
  public List<CustomerPoolDTO> getPoolOptions() {
    return poolCustomerService.getPoolOptions(
  ...
  public void batchAssign(@Validated @RequestBody PoolBatchAssignRequest request) {
    poolCustomerService.batchAssign(
        request,
        request.getAssignUserId(),
        OrganizationContext.getOrganizationId(),
        SessionUtils.getUserId());
  ...
  public void batchDelete(@Validated @RequestBody PoolBatchRequest request) {
    poolCustomerService.batchDelete(
        request.getBatchIds(), SessionUtils.getUserId(), OrganizationContext.getOrganizationId());
  }
  ...
  public void batchUpdate(@Validated @RequestBody ResourceBatchEditRequest request) {
    poolCustomerService.batchUpdate(
        request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId());
  }
  ...
  public String customerPoolExportAll(@Validated @RequestBody CustomerExportRequest request) {
    ConditionFilterUtils.parseCondition(request);
    return customerPoolExportService.exportCrossPage(
        SessionUtils.getUserId(),
        request,
        OrganizationContext.getOrganizationId(),
        null,
  ...
  public String customerPoolExportSelect(@Validated @RequestBody ExportSelectRequest request) {
    return customerPoolExportService.exportCrossSelect(
        SessionUtils.getUserId(),
        request,
        OrganizationContext.getOrganizationId(),
        LocaleContextHolder.getLocale());
  ...
  public List<ChartResult> chart(@Validated @RequestBody PoolCustomerChartAnalysisRequest request) {
    return poolCustomerService.chart(
        request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), null);
  }
}
