...
public class PersonalCenterService {
  ...
  @Resource private OrganizationUserService organizationUserService;
  ...
  public void sendCode(SendEmailDTO emailDTO, String organizationId) {
    ...
    try {
      String emailContent = Translator.get("email_setting_content").replace("${code}", code);
  ...
  public void resetUserPassword(
      PersonalPasswordRequest personalPasswordRequest, String operatorId) {
  ...
  @OperationLog(
      ...
      type = LogType.UPDATE,
      operator = "{#userId}")
  public UserResponse updateInfo(
      PersonalInfoRequest personalInfoRequest, String userId, String orgId) {
  ...
  public PagerWithOption<List<FollowUpPlanListResponse>> getPlanList(
      FollowUpPlanPageRequest request, String userId, String organizationId) {
    ...
    List<String> resourceTypeList =
        buildAccessibleResourceTypes(permissions, isAdmin, enabledModules);
    ...
    List<FollowUpPlanListResponse> planList =
        extFollowUpPlanMapper.selectList(
            request, userId, organizationId, null, null, null, resourceTypeList);

    // 6. 构建完整数据和选项映射
    List<FollowUpPlanListResponse> enrichedList =
        followUpPlanService.buildListData(planList, organizationId);
    Map<String, List<OptionDTO>> optionMap =
        followUpPlanService.buildOptionMap(organizationId, planList, enrichedList);
  ...
  /** 构建用户可访问的资源类型列表 */
  private List<String> buildAccessibleResourceTypes(
      List<String> permissions, boolean isAdmin, List<String> enabledModules) {
