...
public class DingTalkDepartmentService {
  ...
  public ThirdOrgDataDTO convertToThirdOrgDataDTO(String accessToken) {
    ...
    thirdOrgDataDTO.setDepartments(
        Optional.ofNullable(response.getDepartments()).orElse(Collections.emptyList()).stream()
            .map(this::convertToThirdDepartment)
            .toList());
    Map<String, List<ThirdUser>> thirdUsersByDept =
        Optional.ofNullable(response.getUsers()).orElse(Collections.emptyMap()).entrySet().stream()
            .collect(
  ...
  private Optional<DingTalkDepartment> getDepartmentDetail(String accessToken, Long deptId) {
    ...
    try {
      ...
      DepartmentDetailsResponse response =
          JSON.parseObject(responseBody, DepartmentDetailsResponse.class);
  ...
  private List<DingTalkUser> getUsersByDepartment(String accessToken, Long deptId) {
    ...
    String url =
        HttpRequestUtil.urlTransfer(DingTalkApiPaths.DING_DEPARTMENT_USER_DETAIL_LIST, accessToken);
