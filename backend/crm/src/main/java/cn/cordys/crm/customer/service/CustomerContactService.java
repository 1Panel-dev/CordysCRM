...
public class CustomerContactService {
  ...
  @Resource private BaseMapper<Opportunity> opportunityMapper;
  ...
  @Resource private BaseMapper<CustomerContactField> customerContactFieldMapper;
  @Resource private BaseMapper<CustomerContactFieldBlob> customerContactFieldBlobMapper;
  @Resource private LogService logService;

  public PagerWithOption<List<CustomerContactListResponse>> list(
      CustomerContactPageRequest request,
      String userId,
      String orgId,
    ...
    List<CustomerContactListResponse> list =
        extCustomerContactMapper.list(request, userId, orgId, deptDataPermission, source);
  ...
  public Map<String, List<OptionDTO>> getListOptionMap(
      String orgId, List<CustomerContactListResponse> list) {
    ...
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            list, CustomerContactListResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            list, CustomerContactListResponse::getOwner, CustomerContactListResponse::getOwnerName);
    ...
    List<OptionDTO> customerFieldOption =
        moduleFormService.getBusinessFieldOption(
            list,
            CustomerContactListResponse::getCustomerId,
            CustomerContactListResponse::getCustomerName);
    optionMap.put(
  ...
  public List<CustomerContactListResponse> buildListData(
      List<CustomerContactListResponse> list, String orgId) {
    ...
    Map<String, List<BaseModuleFieldValue>> fieldValueMap =
        customerContactFieldService.setBusinessRefFieldValue(
            list,
            moduleFormService.getFlattenFormFields(FormKey.CONTACT.getKey(), orgId),
            caseCustomFiledMap);
    ...
    list.forEach(
        customerListResponse -> {
          ...
          List<BaseModuleFieldValue> customerFields =
              fieldValueMap.get(customerListResponse.getId());
  ...
  public CustomerContactGetResponse get(String id) {
    ...
    CustomerContactGetResponse customerContactGetResponse =
        BeanUtils.copyBean(new CustomerContactGetResponse(), customerContact);
    ...
    UserDeptDTO userDeptDTO =
        baseService.getUserDeptMapByUserId(
            customerContact.getOwner(), customerContact.getOrganizationId());
    if (userDeptDTO != null) {
    ...
    customerContactGetResponse =
        baseService.setCreateUpdateOwnerUserName(customerContactGetResponse);

    // 获取模块字段
    List<BaseModuleFieldValue> customerContactFields =
        customerContactFieldService.getModuleFieldValuesByResourceId(id);
    customerContactFields =
        customerContactFieldService
            .setBusinessRefFieldValue(
                List.of(customerContactGetResponse),
                moduleFormService.getFlattenFormFields(
                    FormKey.CONTACT.getKey(), customerContact.getOrganizationId()),
                new HashMap<>(Map.of(id, customerContactFields)))
            .get(id);
    ModuleFormConfigDTO customerContactFormConfig =
        getFormConfig(customerContact.getOrganizationId());

    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerContactFormConfig, customerContactFields);

    // 补充负责人选项
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            customerContactGetResponse,
            CustomerContactGetResponse::getOwner,
            CustomerContactGetResponse::getOwnerName);
    optionMap.put(BusinessModuleField.CUSTOMER_CONTACT_OWNER.getBusinessKey(), ownerFieldOption);
    ...
    List<OptionDTO> customerFieldOption =
        moduleFormService.getBusinessFieldOption(
            customerContactGetResponse,
            CustomerContactGetResponse::getCustomerId,
            CustomerContactGetResponse::getCustomerName);
    optionMap.put(
        BusinessModuleField.CUSTOMER_CONTACT_CUSTOMER.getBusinessKey(), customerFieldOption);
    ...
    // 附件信息
    customerContactGetResponse.setAttachmentMap(
        moduleFormService.getAttachmentMap(customerContactFormConfig, customerContactFields));
    return customerContactGetResponse;
  }

  @OperationLog(
      module = LogModule.CUSTOMER_CONTACT,
      type = LogType.ADD,
  ...
  public CustomerContact add(CustomerContactAddRequest request, String userId, String orgId) {
    ...
    // 保存自定义字段
    customerContactFieldService.saveModuleField(
        customerContact, orgId, userId, request.getModuleFields(), false);
    ...
    if (customer != null && StringUtils.isNotEmpty(customer.getOwner())) {
      ...
      commonNoticeSendService.sendNotice(
          ...
          NotificationConstants.Event.CUSTOMER_CONCAT_ADD,
          paramMap,
          userId,
          orgId,
  ...
  public CustomerContact update(CustomerContactUpdateRequest request, String userId, String orgId) {
    ...
    CustomerContact originCustomerContact =
        customerContactMapper.selectByPrimaryKey(customerContact.getId());
    // 获取模块字段
    List<BaseModuleFieldValue> originCustomerFields =
        customerContactFieldService.getModuleFieldValuesByResourceId(request.getId());

    if (BooleanUtils.isTrue(request.getAgentInvoke())) {
      customerContactFieldService.updateModuleFieldByAgent(
          customerContact, originCustomerFields, request.getModuleFields(), orgId, userId);
    } else {
      // 更新模块字段
    ...
    baseService.handleUpdateLog(
        originCustomerContact,
        customerContact,
        originCustomerFields,
        request.getModuleFields(),
        originCustomerContact.getId(),
        originCustomerContact.getName());
    return customerContact;
  }
  ...
  public CustomerContactListAllResponse listByCustomerId(
    ...
    if (deptDataPermission.getAll() || Strings.CS.equals(userId, InternalUser.ADMIN.getValue())) {
      // 全部数据权限，直接返回
      List<CustomerContactListResponse> list =
          extCustomerContactMapper.listByCustomerId(customerId);
      list = buildListData(list, orgId);
      Map<String, List<OptionDTO>> optionMap = getListOptionMap(orgId, list);
    ...
    List<CustomerCollaboration> collaborations =
        customerCollaborationService.selectByCustomerId(customerId);

    // 获取协作类型的协作的联系人
    Set<String> collaborationUserIds =
        collaborations.stream()
            .filter(
                collaboration ->
                    Strings.CS.equals(
                        collaboration.getCollaborationType(),
                        CustomerCollaborationType.COLLABORATION.name()))
    ...
    if (CollectionUtils.isNotEmpty(deptDataPermission.getDeptIds())) {
      UserDeptDTO customerOwnerDept =
          baseService.getUserDeptMapByUserId(customer.getOwner(), orgId);
      // 部门权限是否有该客户的权限
      boolean hasDeptCustomerPermission =
          customerOwnerDept != null
      ...
      if (hasDeptCustomerPermission) {
        Map<String, UserDeptDTO> userDeptMapByUserIds =
            baseService.getUserDeptMapByUserIds(collaborationUserIds, orgId);
        list =
            list.stream()
                .filter(
                    item -> {
                      ...
                      return userDeptDTO != null
                          && deptDataPermission.getDeptIds().contains(userDeptDTO.getDeptId());
                    })
                .collect(Collectors.toList());
      ...
      if (isCustomerOwner) {
        ...
        list =
            list.stream()
                .filter(
                    item ->
                        !collaborationUserIds.contains(item.getOwner())
                            || Strings.CS.equals(item.getOwner(), userId))
                .collect(Collectors.toList());
      } else if (isCollaborationUser) {
        // 没有权限，只是协作人，则只能看自己的
        list =
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.CUSTOMER_MANAGEMENT_CONTACT_READ, rolePermissions);
  }
   ...
   * @return 是否唯一
   */
  public boolean checkCustomerContactUnique(
      String contact, String phone, String customerId, String orgId) {
    LambdaQueryWrapper<ModuleForm> formQueryWrapper = new LambdaQueryWrapper<>();
    formQueryWrapper
        .eq(ModuleForm::getOrganizationId, orgId)
        .eq(ModuleForm::getFormKey, FormKey.CONTACT.getKey());
    ...
    List<String> fieldIds =
        moduleFieldMapper.selectListByLambda(queryWrapper).stream()
    ...
    for (ModuleFieldBlob blob : blobs) {
      ...
      boolean hasUnique =
          baseField.getRules().stream()
              .anyMatch(rule -> RuleValidatorConstants.UNIQUE.equals(rule.getKey()));
    ...
    ContactUniqueRequest request =
        ContactUniqueRequest.builder()
            .name(contact)
            .phone(phone)
  ...
  public void updateContactOwner(
      String customerId, String newOwner, String oldOwner, String orgId) {
   ...
   * @param orgId 组织ID
   */
  public void updatePoolContactOwner(
      String customerId, String newOwner, String oldOwner, String orgId) {
  ...
  public void downloadImportTpl(HttpServletResponse response, String currentOrg) {
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            response,
            moduleFormService.getCustomImportHeadsNoRef(FormKey.CONTACT.getKey(), currentOrg),
            Translator.get("contact.import_tpl.name"),
            Translator.get(SheetKey.DATA),
            Translator.get(SheetKey.COMMENT),
            new CustomTemplateWriteHandler(
                moduleFormService.getAllCustomImportFields(FormKey.CONTACT.getKey(), currentOrg)),
  ...
  public ImportResponse realImport(MultipartFile file, String currentOrg, String currentUser) {
    try {
      ...
      CustomImportAfterDoConsumer<CustomerContact, BaseResourceSubField> afterDo =
          (contacts, contactFields, contactFieldBlobs) -> {
            ...
            customerContactFieldMapper.batchInsert(
                contactFields.stream()
                    .map(field -> BeanUtils.copyBean(new CustomerContactField(), field))
                    .toList());
            customerContactFieldBlobMapper.batchInsert(
                contactFieldBlobs.stream()
                    .map(field -> BeanUtils.copyBean(new CustomerContactFieldBlob(), field))
                    .toList());
            // record logs
            logService.batchAdd(logs);
          };
      CustomFieldImportEventListener<CustomerContact> eventListener =
    ...
    try {
      ...
      CustomFieldCheckEventListener eventListener =
          new CustomFieldCheckEventListener(
              fields, "customer_contact", "customer_contact_field", currentOrg);
      FastExcelFactory.read(file.getInputStream(), eventListener)
          .headRowNumber(1)
          .ignoreEmptyRow(true)
          .sheet()
  ...
  public void batchUpdate(ResourceBatchEditRequest request, String userId, String organizationId) {
    BaseField field =
        customerContactFieldService.getAndCheckField(request.getFieldId(), organizationId);

    List<CustomerContact> originCustomerContacts =
        customerContactMapper.selectByIds(request.getIds());

    customerContactFieldService.batchUpdate(
        request,
        field,
        originCustomerContacts,
        CustomerContact.class,
        LogModule.CUSTOMER_CONTACT,
        extCustomerContactMapper::batchUpdate,
        userId,
        organizationId);
  }
  ...
  public List<ChartResult> chart(
    ...
    List<ChartResult> chartResults =
        extCustomerContactMapper.chart(chartAnalysisDbRequest, userId, orgId, deptDataPermission);
