...
public class MessageTaskController {
  @Resource private MessageTaskService messageTaskService;

  @PostMapping("save")
  @Operation(summary = "项目管理-消息管理-消息设置-保存消息设置")
  @RequiresPermissions(PermissionConstants.SYSTEM_NOTICE_UPDATE)
  public MessageTask saveMessage(
      @Validated({Created.class, Updated.class}) @RequestBody
          MessageTaskRequest messageTaskRequest) {
    return messageTaskService.saveMessageTask(
        messageTaskRequest, SessionUtils.getUserId(), OrganizationContext.getOrganizationId());
  }
  ...
  public void batchSaveMessage(
      @Validated({Created.class, Updated.class}) @RequestBody
          MessageTaskBatchRequest messageTaskBatchRequest) {
    messageTaskService.batchSaveMessageTask(
        messageTaskBatchRequest, OrganizationContext.getOrganizationId(), SessionUtils.getUserId());
  }
  ...
  public MessageTaskConfigWithNameDTO getMessageConfig(
      @Validated @RequestBody MessageTaskConfigRequest request) {
    return messageTaskService.getMessageConfig(
        request.getModule(), request.getEvent(), OrganizationContext.getOrganizationId());
  }
}
