...
public class LarkDepartmentService {
  ...
  public List<LarkDepartment> getAllSubDepartments(String tenantAccessToken, String departmentId) {
    ...
    do {
      String url =
          LarkApiPaths.LARK_CHILDREN_DEPARTMENT_URL.replace("{0}", departmentId)
              + "?fetch_child=true";
      if (pageToken != null) {
      ...
      LarkDepartmentResponse larkResponse =
          JSON.parseObject(response, LarkDepartmentResponse.class);
      if (larkResponse.getCode() == 0) {
        ...
        log.error(
            "Failed to get sub-departments from Lark: {}",
            larkResponse.getCode() + ":" + larkResponse.getMsg());
        throw new GenericException(
            "Failed to get sub-departments from Lark: " + larkResponse.getMsg());
      }
    } while (hasMore);
  ...
  public List<LarkUser> getDepartmentUsers(String tenantAccessToken, String departmentId) {
    ...
    do {
      ...
      if (larkUserResponse.getCode() == 0) {
        ...
        log.error(
            "Failed to get department users from Lark: {}",
            larkUserResponse.getCode() + ":" + larkUserResponse.getMsg());
        throw new GenericException(
            "Failed to get department users from Lark: " + larkUserResponse.getMsg());
      }
    } while (hasMore);
   ...
   * @return 部门ID与用户列表的映射
   */
  public Map<String, List<ThirdUser>> getDepartmentUserList(
      String tenantAccessToken, List<String> departmentIds) {
        ...
        departmentId -> {
          ...
          thirdUserMap.put(
              ...
              larkUsers.stream()
                  .filter(
                      larkUser ->
                          larkUser.getOrders().stream()
                              .anyMatch(
                                  order ->
                                      order.getIsPrimaryDept()
                                          && order.getDepartmentId().equals(departmentId)))
                  .map(
                      larkUser ->
                          ThirdUser.builder()
                              ...
                              .mobile(
                                  larkUser.getMobile() != null && larkUser.getMobile().length() > 11
                                      ? larkUser
                                          .getMobile()
                                          .substring(larkUser.getMobile().length() - 11)
                                      : larkUser.getMobile())
                              .position(larkUser.getWorkStation())
                              .gender(larkUser.getGender() == null ? null : larkUser.getGender())
                              .avatar(
                                  (larkUser.getAvatar() == null
                                          || StringUtils.isBlank(
                                              larkUser.getAvatar().getAvatar240()))
                                      ? null
                                      : larkUser.getAvatar().getAvatar240())
                              .isLeaderInDept(
                                  Strings.CI.equals(
                                      larkUser.getLeaderUserId(), larkUser.getUserId()))
                              .build())
  ...
  public LarkTenant getTenantInfo(String tenantAccessToken) {
    try {
      ...
      if (larkTenantResponse.getCode() == 0) {
        ...
        log.error(
            "Failed to get tenant info from Lark: {}",
            larkTenantResponse.getCode() + ":" + larkTenantResponse.getMsg());
        throw new GenericException(
            "Failed to get tenant info from Lark: " + larkTenantResponse.getMsg());
      }
    } catch (Exception e) {
