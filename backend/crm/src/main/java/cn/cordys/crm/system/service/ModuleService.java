...
public class ModuleService {
  ...
  @Resource private ExtModuleMapper extModuleMapper;
  ...
  @Resource private BaseMapper<OrganizationConfig> organizationConfigMapper;
    ...
    return modules.stream()
        .map(
            module -> {
              ...
              dto.setDisabled(
                  Strings.CS.equals(dto.getModuleKey(), "dashboard")
                      && !isDashboardEnabled(request.getOrganizationId()));
              return dto;
            })
  ...
  public List<OptionDTO> getFormList() {
    return Arrays.stream(FormKey.values())
        .map(formKey -> new OptionDTO(formKey.getKey(), Translator.get(formKey.getKey())))
        .toList();
  }
  ...
  public void initModule(String organizationId) {
    ...
    Arrays.stream(ModuleKey.values())
        .forEach(
            moduleConstant -> {
  ...
  private boolean isDashboardEnabled(String organizationId) {
    ...
    configQuery
        .eq(OrganizationConfig::getOrganizationId, organizationId)
        .eq(OrganizationConfig::getType, OrganizationConfigConstants.ConfigType.THIRD.name());

    // 直接获取第一个配置或返回null
    OrganizationConfig config =
        organizationConfigMapper.selectListByLambda(configQuery).stream().findFirst().orElse(null);
    ...
    detailQuery
        .eq(OrganizationConfigDetail::getConfigId, config.getId())
        .like(OrganizationConfigDetail::getType, "DE_BOARD");

    // 查找配置并判断是否启用
    return organizationConfigDetailMapper.selectListByLambda(detailQuery).stream()
        .findFirst()
