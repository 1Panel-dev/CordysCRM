...
public class WeComDepartmentService {
   ...
   * @return 部门ID与用户列表的映射
   */
  public Map<String, List<ThirdUser>> getDepartmentUser(
      String accessToken, List<Long> departmentIds) {
    ...
    for (Long departmentId : departmentIds) {
      ...
      List<ThirdUser> thirdUsers =
          response
              .getUserList()
              .stream() // 过滤如果weComUser有mainDepartment属性，过滤department中含有mainDepartment的用户，没有就不过滤
              .filter(
                  weComUser ->
                      weComUser.getMainDepartment() == null
                          || weComUser.getMainDepartment().equals(departmentId))
              .map(
                  weComUser ->
                      ThirdUser.builder()
                          ...
                          .isLeaderInDept(
                              weComUser
                                      .getIsLeaderInDept()
                                      .indexOf(Integer.parseInt(departmentId.toString()))
                                  > 0)
                          .build())
