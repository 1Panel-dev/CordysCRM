...
public class DataEaseClient {
  ...
  public SysVariableValueDTO createSysVariableValue(SysVariableValueCreateRequest request) {
    return post("sysVariable/value/create", request, SysVariableValueCreateResponse.class)
        .getData();
  }
  ...
  public <V extends DataEaseBaseResponse> V post(
      String path, Class<V> responseClass, String... uriVars) {
    return post(path, Map.of(), responseClass, uriVars);
  }

  public <V extends DataEaseBaseResponse> V post(
      String path, Object body, Class<V> responseClass, String... uriVars) {
    var url = getUrl(path);
    var response =
        restTemplate
            .exchange(url, HttpMethod.POST, getHttpEntity(body), responseClass, (Object[]) uriVars)
            .getBody();
  ...
  public <V extends DataEaseBaseResponse> V get(
      String path, Class<V> responseClass, Object... uriVars) {
    var url = getUrl(path);
    var response =
        restTemplate
            .exchange(url, HttpMethod.GET, getHttpEntity(), responseClass, uriVars)
            .getBody();
    validateResponse(response);
    return response;
  }
