...
public class DataHandleUtils {
  ...
  public DataHandleUtils(
      String orgId, Map<String, List<ThirdUser>> departmentUserMap, String type) {
    ...
    this.internalDepartment =
        departmentService.getInternalDepartment(orgId, ThirdConfigTypeConstants.INTERNAL.name());
  ...
  public void handleAddData(
      List<ThirdDepartment> thirdDepartments, String operatorId, String orgId, String type) {
    this.thirdDepartmentTree =
        ThirdDepartment.buildDepartmentTree(internalDepartment.getId(), thirdDepartments);
  ...
  private void handleTreeAddData(ThirdDepartment thirdDepartment, String operatorId) {
    ...
    if (CollectionUtils.isNotEmpty(thirdDepartment.getChildren())) {
      thirdDepartment.getChildren().stream()
          ...
          .forEach(department -> handleTreeAddData(department, operatorId));
    }
  }
  ...
  public void buildUser(
      ThirdDepartment thirdDepartment, List<ThirdUser> thirdUsers, String operatorId) {
    ...
    thirdUsers.forEach(
        thirdUser -> {
          ...
          if (CollectionUtils.isNotEmpty(organizationUsers)) {
            OrganizationUser organizationUser =
                organizationUsers.stream()
                    .filter(
                        user -> Strings.CI.equals(user.getResourceUserId(), thirdUser.getUserId()))
                    .findFirst()
                    .orElse(null);
          ...
          OrganizationUser orgUser =
              createOrganizationUser(id, thirdDepartment, thirdUser, operatorId, currentTime);
  ...
  public void handleUpdateData(List<ThirdDepartment> thirdDepartments, String operatorId) {
    ...
    // 微信全量用户
    List<ThirdUser> thirdUserList =
        departmentUserMap.values().stream().flatMap(List::stream).toList();

    // 需要禁用的用户（企业微信不存在而系统存在的用户）
    List<OrganizationUser> disableUserList =
        userList.stream()
            .filter(
                user ->
                    thirdUserList.stream()
                        .noneMatch(
                            thirdUser ->
                                Strings.CI.equalsAny(
                                    thirdUser.getUserId(), user.getResourceUserId())))
    ...
    List<DepartmentCommander> currentCommander =
        departmentService.getDepartmentCommander(currentUserList);

    if (CollectionUtils.isNotEmpty(currentDepartmentList)) {
      thirdDepartmentTree =
          ThirdDepartment.buildDepartmentTreeMultiple(
              internalDepartment.getId(), currentDepartmentList, thirdDepartments);
    }

    thirdDepartmentTree.forEach(
        thirdDepartment ->
            handleTreeUpdateData(
                thirdDepartment,
                operatorId,
  ...
  private void handleTreeUpdateData(
      ...
      List<DepartmentCommander> currentCommander) {
    buildUpdateData(
        thirdDepartment, operatorId, currentDepartmentList, currentUserList, currentCommander);
  ...
  /** 构建更新和新增用户数据 */
  private void buildUpdateAndAddUser(
      ThirdDepartment thirdDepartment,
      List<ThirdUser> thirdUsers,
      String operatorId,
      List<OrganizationUser> currentUserList,
      List<Department> currentDepartmentList,
      List<DepartmentCommander> currentCommander) {
    ...
    thirdUsers.forEach(
        thirdUser -> {
          ...
          String departmentId =
              (department != null) ? department.getId() : thirdDepartment.getCrmId();
  ...
  /** 同步构建更新和新增部门数据 */
  private void buildUpdateAndAddDepartment(
      ThirdDepartment thirdDepartment, String operatorId, List<Department> currentDepartmentList) {
    if (thirdDepartment.getIsRoot()) {
      // 更新根部门
      Department department = new Department();
      department.setId(internalDepartment.getId());
  ...
  /** 构建子部门数据 */
  private void buildChildrenDepartment(
      ThirdDepartment thirdDepartment, String operatorId, List<Department> currentDepartmentList) {
    long currentTime = System.currentTimeMillis();
    ...
    if (CollectionUtils.isEmpty(currentDepartmentList)) {
      ...
      if (existingDept != null) {
        ...
        updateExistingDepartment(
            existingDept, thirdDepartment, currentDepartmentList, operatorId, currentTime);
      } else {
        // 添加新部门
        addNewDepartment(thirdDepartment, operatorId, currentTime);
      }
  ...
  private OrganizationUser createOrganizationUser(
      String userId,
      ThirdDepartment thirdDepartment,
      ThirdUser thirdUser,
      String operatorId,
  ...
  private void addDepartmentCommanderIfNeeded(
      String userId,
      ThirdDepartment thirdDepartment,
      ThirdUser thirdUser,
      String operatorId,
  ...
  private OrganizationUser findExistingUser(
      List<OrganizationUser> currentUserList, ThirdUser thirdUser) {
    return Optional.ofNullable(currentUserList).orElse(Collections.emptyList()).stream()
        .filter(user -> Strings.CI.equalsAny(user.getResourceUserId(), thirdUser.getUserId()))
        .findFirst()
        .orElse(null);
  }

  private Department findExistingDepartment(
      List<Department> currentDepartmentList, ThirdDepartment thirdDepartment) {
    return Optional.ofNullable(currentDepartmentList).orElse(Collections.emptyList()).stream()
        .filter(dept -> Strings.CI.equalsAny(dept.getResourceId(), thirdDepartment.getId()))
        .findFirst()
        .orElse(null);
  }
  ...
  private void addCommanderIfNeeded(
      String userId,
      String departmentId,
      ThirdUser thirdUser,
      List<DepartmentCommander> currentCommander,
    ...
    if (thirdUser.getIsLeaderInDept()) {
      boolean commanderExists =
          Optional.ofNullable(currentCommander).orElse(Collections.emptyList()).stream()
              .anyMatch(
                  cmd ->
                      Strings.CI.equalsAny(cmd.getUserId(), userId)
  ...
  private void addNewUser(
    ...
    if (CollectionUtils.isNotEmpty(organizationUsers)) {
      OrganizationUser organizationUser =
          organizationUsers.stream()
              .filter(user -> Strings.CI.equals(user.getResourceUserId(), thirdUser.getUserId()))
              .findFirst()
              .orElse(null);
      if (organizationUser != null) {
        return;
  ...
  private void updateExistingDepartment(
    ...
    if (crmParentDep != null) {
      ...
      updateDept.setParentId(
          parentDep == null ? thirdDepartment.getCrmParentId() : parentDep.getId());
    }
    updateDepartments.add(updateDept);
  }

  private void addNewDepartment(
      ThirdDepartment thirdDepartment, String operatorId, long timestamp) {
    Department department = new Department();
    department.setId(thirdDepartment.getCrmId());
    department.setName(thirdDepartment.getName());
    department.setOrganizationId(orgId);
