...
public class SystemModuleLogService extends BaseModuleLogService {
    ...
    differences.forEach(
        differ -> {
          if (isLinkFormKey(differ.getColumn())
              && differ.getColumn().split(LINK_KEY_SPILT).length == 2) {
            String[] splitKey = differ.getColumn().split(LINK_KEY_SPILT);
            differ.setColumnName(
                Translator.get("log." + splitKey[0] + ".link")
                    + "-"
                    + Translator.get(splitKey[1].toLowerCase()));
          ...
          if (Strings.CI.equalsAny(
              differ.getColumn(), "rate", "stage", "afootRollBack", "endRollBack")) {
  ...
  private void handleModuleMainNav(JsonDifferenceDTO differ) {
    ...
    differ.setOldValueName(
        parseFieldList(differ.getOldValue()).stream()
            .map(String::valueOf)
            .map(Translator::get)
            .toList());

    differ.setNewValueName(
        parseFieldList(differ.getNewValue()).stream()
            .map(String::valueOf)
            .map(Translator::get)
            .toList());
  }
  ...
  private List<String> toNamePairs(Map<String, String> pairs, Map<String, String> fieldMap) {
    return pairs.entrySet().stream()
        .map(
            e ->
                fieldMap.getOrDefault(e.getKey(), e.getKey())
                    + "-"
