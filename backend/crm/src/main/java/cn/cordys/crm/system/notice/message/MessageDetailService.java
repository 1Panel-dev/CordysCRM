...
public class MessageDetailService {
   ...
   */
  public List<MessageDetailDTO> searchMessageByTypeAndOrgId(
      String module, boolean useTemplate, String template, String organizationId) {
    try {
  ...
  private List<MessageDetailDTO> getMessageDetailDTOs(
      String module, boolean useTemplate, String template, String organizationId) {
    List<MessageDetailDTO> messageDetails = new ArrayList<>();
    List<MessageTask> messageTaskLists =
        extMessageTaskMapper.getEnableMessageTaskByReceiveTypeAndTaskType(module, organizationId);
    ...
    return messageDetails.stream()
        .sorted(
            Comparator.comparing(
                    MessageDetailDTO::getCreateTime, Comparator.nullsLast(Long::compareTo))
                .reversed())
        .distinct()
        .collect(Collectors.toList());
  }

  private void getMessageDetailDTOs(
      boolean useTemplate,
      String customTemplate,
      List<MessageDetailDTO> messageDetails,
      List<MessageTask> messageTaskLists) {
    // 消息通知任务以消息类型事件接收类型唯一进行分组
    Map<String, List<MessageTask>> messageTaskGroup =
        messageTaskLists.stream()
            .collect(Collectors.groupingBy(t -> (t.getTaskType() + t.getEvent())));
    messageTaskGroup.forEach(
        (messageTaskId, messageTaskList) -> {
          // 获取同一任务所有的接收人
