...
public class CustomerContactController {
  ...
  @Resource private CustomerContactExportService customerContactExportService;
  ...
  public ModuleFormConfigDTO getModuleFormConfig() {
    return moduleFormCacheService.getBusinessFormConfig(
        FormKey.CONTACT.getKey(), OrganizationContext.getOrganizationId());
  }
  ...
  public List<ChartResult> chart(@Validated @RequestBody ChartAnalysisRequest request) {
    DeptDataPermissionDTO deptDataPermission =
        dataScopeService.getDeptDataPermission(
            ...
            request.getViewId(),
            PermissionConstants.CUSTOMER_MANAGEMENT_CONTACT_READ);
    return customerContactService.chart(
        request,
        SessionUtils.getUserId(),
        OrganizationContext.getOrganizationId(),
        deptDataPermission);
  ...
  public CustomerContactListAllResponse list(@Validated @PathVariable String customerId) {
    DeptDataPermissionDTO deptDataPermission =
        dataScopeService.getDeptDataPermission(
            ...
            OrganizationContext.getOrganizationId(),
            PermissionConstants.CUSTOMER_MANAGEMENT_READ);
    return customerContactService.listByCustomerId(
        customerId,
        SessionUtils.getUserId(),
        OrganizationContext.getOrganizationId(),
        deptDataPermission);
  ...
  @RequiresPermissions(
      value = {
        ...
        PermissionConstants.CUSTOMER_MANAGEMENT_CONTACT_DELETE
      },
      logical = Logical.OR)
  ...
  public ResourceTabEnableDTO getTabEnableConfig() {
    return customerContactService.getTabEnableConfig(
  ...
  public String customerContactExportAll(
      @Validated @RequestBody CustomerContactExportRequest request) {
    ConditionFilterUtils.parseCondition(request);
    DeptDataPermissionDTO deptDataPermission =
        dataScopeService.getDeptDataPermission(
            SessionUtils.getUserId(),
            ...
            request.getViewId(),
            PermissionConstants.CUSTOMER_MANAGEMENT_CONTACT_READ);
    return customerContactExportService.export(
        SessionUtils.getUserId(),
        request,
        OrganizationContext.getOrganizationId(),
        deptDataPermission,
  ...
  public String customerContactExportSelect(@Validated @RequestBody ExportSelectRequest request) {
    return customerContactExportService.exportSelect(
        SessionUtils.getUserId(),
        request,
        OrganizationContext.getOrganizationId(),
        LocaleContextHolder.getLocale());
  ...
  public ImportResponse realImport(@RequestPart(value = "file") MultipartFile file) {
    return customerContactService.realImport(
  ...
  public void batchUpdate(@Validated @RequestBody ResourceBatchEditRequest request) {
    customerContactService.batchUpdate(
        request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId());
  }
}
