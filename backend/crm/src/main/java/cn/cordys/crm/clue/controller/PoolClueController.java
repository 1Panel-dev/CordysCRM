...
public class PoolClueController {
  ...
  @Resource private CluePoolExportService cluePoolExportService;
  ...
  public List<CluePoolDTO> getPoolOptions() {
    return poolClueService.getPoolOptions(
  ...
  public PagerWithOption<List<ClueListResponse>> list(
    ...
    return clueService.list(
        request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), null, false);
  }
  ...
  public void assign(@Validated @RequestBody PoolClueAssignRequest request) {
    poolClueService.assign(
        request.getClueId(),
        request.getAssignUserId(),
  ...
  public String exportAll(@Validated @RequestBody ClueExportRequest request) {
    ConditionFilterUtils.parseCondition(request);
    return cluePoolExportService.exportCrossPage(
        request,
        SessionUtils.getUserId(),
        OrganizationContext.getOrganizationId(),
        null,
  ...
  public String exportSelect(@Validated @RequestBody ExportSelectRequest request) {
    return cluePoolExportService.exportSelect(
        request,
        SessionUtils.getUserId(),
        OrganizationContext.getOrganizationId(),
        LocaleContextHolder.getLocale());
  ...
  public List<ChartResult> chart(@Validated @RequestBody PoolClueChartAnalysisRequest request) {
    return cluePoolExportService.chart(
        request, SessionUtils.getUserId(), OrganizationContext.getOrganizationId(), null);
  }
}
