 ...
 * 跟进计划提醒监听器
 *
 * <p>该监听器负责监听执行事件，当触发时检查并提醒到期的跟进计划。 支持客户、商机和线索三种类型的跟进计划提醒。
 */
@Component
@Slf4j
  ...
  public void followUpPlanRemind() {
    ...
    if (CollectionUtils.isNotEmpty(planList)) {
      ...
      if (CollectionUtils.isNotEmpty(opportunityIds)) {
        opportunityMap =
            extOpportunityMapper.getOpportunityOptionsByIds(opportunityIds).stream()
                .collect(Collectors.toMap(OptionDTO::getId, OptionDTO::getName));
      }
      ...
      if (CollectionUtils.isNotEmpty(clueIds)) {
        clueMap =
            extClueMapper.selectOptionByIds(clueIds).stream()
                .collect(Collectors.toMap(OptionDTO::getId, OptionDTO::getName));
      }
