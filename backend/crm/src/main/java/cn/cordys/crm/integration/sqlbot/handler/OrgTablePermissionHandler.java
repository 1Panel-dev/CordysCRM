...
public class OrgTablePermissionHandler implements TablePermissionHandler {
  ...
  protected String getDefaultFieldSql(FieldDTO sqlBotField, String prefix) {
    ...
    } else if (Strings.CS.endsWith(fieldName, "_user")
        || Strings.CS.equalsAny(fieldName, "follower")) {
  ...
  private String getUserFieldSql(String fieldName, String prefix) {
    if (StringUtils.isBlank(prefix)) {
      return String.format(
          "(select su.name from sys_user su where su.id = c.%s limit 1) as %s",
          fieldName, fieldName);
    } else {
      return String.format(
          "(select su.name from sys_user su where su.id = %s.%s limit 1) as %s",
  ...
  private String getTimeFieldSql(String fieldName, String prefix) {
    if (StringUtils.isBlank(prefix)) {
      return MessageFormat.format(
          "DATE_FORMAT(FROM_UNIXTIME(c.{0} / 1000),''%Y-%m-%d %H:%i:%s'') as {1}",
          fieldName, fieldName);
    } else {
      return MessageFormat.format(
          "DATE_FORMAT(FROM_UNIXTIME({0}.{1} / 1000),''%Y-%m-%d %H:%i:%s'') as {2}",
          prefix, fieldName, fieldName);
    }
  }
  ...
  private String getDateFieldSql(String fieldName, String prefix) {
    if (StringUtils.isBlank(prefix)) {
      return MessageFormat.format(
          "DATE_FORMAT(FROM_UNIXTIME(c.{0} / 1000),''%Y-%m-%d'') as {1}", fieldName, fieldName);
    } else {
      return MessageFormat.format(
          "DATE_FORMAT(FROM_UNIXTIME({0}.{1} / 1000),''%Y-%m-%d'') as {2}",
          prefix, fieldName, fieldName);
    }
  }
   ...
   * @return
   */
  protected List<FieldDTO> filterSystemFields(
      List<FieldDTO> fields, Set<String> disabledFieldNames) {
  ...
  /**
   * 获取选项字段查询的SQL 示例： CASE c.stage WHEN 'SUCCESS' THEN '成功' WHEN 'FAIL' THEN '失败' ELSE '' END AS
   * stage_name
   *
   * @param stream
   * @param fieldName
   * @return
   */
  protected <T> String getSystemOptionFileSql(
      Stream<T> stream,
    ...
    stream.forEach(
        clueStatus ->
            sqlBuilder.append(
                String.format(
                    " when '%s' then '%s'",
                    getKeyFunc.apply(clueStatus), getNameFunc.apply(clueStatus))));
