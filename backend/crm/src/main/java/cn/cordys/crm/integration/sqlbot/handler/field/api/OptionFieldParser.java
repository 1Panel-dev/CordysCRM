...
public abstract class OptionFieldParser<T extends BaseField> implements ModuleFieldParser<T> {
  ...
  /**
   * 将选项列表转换为SQL查询语句 例如： SELECT '400电话' AS label, '1' AS value UNION ALL SELECT '官网咨询', '2'
   *
   * @param options
   * @return
   */
  ...
  protected String parseOptionFieldSql(
      String fieldValueTable, BaseField field, List<OptionProp> options) {
    if (CollectionUtils.isEmpty(options)) {
  ...
  protected String getOptionLabelStr(List<OptionProp> options) {
    return "，可选值：("
        + options.stream().map(OptionProp::getLabel).collect(Collectors.joining(","))
        + ")";
  }
