...
public class NoticeSendService {
  ...
  public void send(String module, NoticeModel noticeModel) {
    ...
    try {
      ...
      List<MessageDetailDTO> messageDetails =
          messageDetailService.searchMessageByTypeAndOrgId(
              module, useTemplate, template, organizationId);

      messageDetails.stream()
          .filter(
              messageDetail -> Strings.CS.equals(messageDetail.getEvent(), noticeModel.getEvent()))
  ...
  public void sendNotification(MessageDetailDTO messageDetail, NoticeModel noticeModel) {
    ...
    try {
      ...
      if (clonedMessageDetail.isDingTalkEnable()) {
        DingTalkNoticeSender dingTalkNoticeSender =
            CommonBeanFactory.getBean(DingTalkNoticeSender.class);
  ...
  public void sendOther(String module, NoticeModel noticeModel, boolean excludeSelf) {
    setLanguage(noticeModel.getParamMap().get("Language"));
    boolean useTemplate =
        Boolean.parseBoolean((String) noticeModel.getParamMap().get("useTemplate"));
    String template = (String) noticeModel.getParamMap().get("template");
    ...
    try {
      ...
      List<MessageDetailDTO> messageDetails =
          messageDetailService
              ...
              .filter(
                  messageDetail ->
                      Strings.CS.equals(messageDetail.getEvent(), noticeModel.getEvent()))
              .toList();
