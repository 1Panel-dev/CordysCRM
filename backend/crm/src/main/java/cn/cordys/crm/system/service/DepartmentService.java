...
public class DepartmentService extends MoveNodeService {
  ...
  public void rename(DepartmentRenameRequest request, String userId, String orgId) {
    ...
    checkDepartmentName(
        request.getName(),
        originalDepartment.getParentId(),
        originalDepartment.getOrganizationId());
    ...
    // 添加日志上下文
    String resourceName =
        Optional.ofNullable(department.getName()).orElse(originalDepartment.getName());
    OperationLogContext.setContext(
        LogContextInfo.builder()
  ...
  public void setCommander(DepartmentCommanderRequest request, String userId) {
    ...
    OperationLogContext.setContext(
        LogContextInfo.builder()
            ...
            .resourceName(department.getName())
            .originalValue(DepartmentSetCommanderLog.builder().commander(originCommander).build())
            .modifiedValue(
                DepartmentSetCommanderLog.builder().commander(request.getCommanderId()).build())
            .build());
  }
  ...
  public boolean deleteCheck(List<String> ids, String orgId) {
    ...
    if (CollectionUtils.isNotEmpty(departmentList)) {
      departmentList.forEach(
          department -> {
            if (Strings.CI.equalsAny(
                    department.getResource(), ThirdConfigTypeConstants.INTERNAL.name())
   ...
   * @return
   */
  public Map<String, String> createDepartment(
      List<String> departmentPath,
      String orgId,
      List<BaseTreeNode> departmentTree,
      String operatorId,
      Map<String, String> departmentMap,
      List<LogDTO> logs) {
    departmentPath.forEach(
        path -> {
          path = "/" + path;
  ...
  private void createDepByPathIterator(
      Iterator<String> itemIterator,
      String currentDepPath,
      BaseTreeNode departmentTreeNode,
      Map<String, String> departmentMap,
      String orgId,
      String operatorId,
      List<LogDTO> logs) {
  ...
  private void createDepByPath(
      Iterator<String> itemIterator,
      String departmentName,
      BaseTreeNode parentDep,
      String orgId,
      String currentDepPath,
      Map<String, String> departmentMap,
      String operatorId,
  ...
  private String insertNode(
      String departmentName, String parentId, String orgId, String operatorId, List<LogDTO> logs) {
    String id = IDGenerator.nextStr();
    Department department = new Department();
    department.setId(id);
  ...
  public List<DepartmentCommander> getDepartmentCommander(List<OrganizationUser> userList) {
    ...
    List<String> userIds = userList.stream().map(OrganizationUser::getUserId).toList();
