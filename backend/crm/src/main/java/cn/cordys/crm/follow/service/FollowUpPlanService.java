...
public class FollowUpPlanService extends BaseFollowUpService {
  ...
  @Resource private FollowUpPlanFieldService followUpPlanFieldService;
  ...
  public FollowUpPlan add(FollowUpPlanAddRequest request, String userId, String orgId) {
    ...
    // 保存自定义字段
    followUpPlanFieldService.saveModuleField(
        followUpPlan, orgId, userId, request.getModuleFields(), false);
   ...
   */
  @OperationLog(
      module = LogModule.FOLLOW_UP_PLAN,
      type = LogType.UPDATE,
  ...
  public FollowUpPlan update(FollowUpPlanUpdateRequest request, String userId, String orgId) {
    ...
    Optional.ofNullable(followUpPlan)
        .ifPresentOrElse(
            plan -> {
              // 更新跟进计划
              ...
              List<BaseModuleFieldValue> originCustomerFields =
                  followUpPlanFieldService.getModuleFieldValuesByResourceId(request.getId());
              // 更新模块字段
              ...
              baseService.handleUpdateLog(
                  ...
                  followUpPlan.getId(),
                  Translator.get("update_follow_up_plan"));
            },
            () -> {
  ...
  private FollowUpPlan newPlan(
      FollowUpPlan plan, FollowUpPlanUpdateRequest request, String userId) {
  ...
  public PagerWithOption<List<FollowUpPlanListResponse>> list(
    ...
    List<FollowUpPlanListResponse> list =
        extFollowUpPlanMapper.selectList(
            request, userId, orgId, resourceType, type, customerData, null);
   ...
   * @return 跟进计划汇总列表
   */
  public PagerWithOption<List<FollowUpPlanListResponse>> totalList(
      PlanHomePageRequest request,
      String userId,
      String orgId,
    ...
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<FollowUpPlanListResponse> list =
        extFollowUpPlanMapper.selectTotalList(
  ...
  public Map<String, List<OptionDTO>> buildOptionMap(
      String orgId, List<FollowUpPlanListResponse> list, List<FollowUpPlanListResponse> buildList) {
    // 处理自定义字段选项数据
    ModuleFormConfigDTO customerFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.FOLLOW_PLAN.getKey(), orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(list, FollowUpPlanListResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList, FollowUpPlanListResponse::getOwner, FollowUpPlanListResponse::getOwnerName);
    ...
    List<OptionDTO> contactFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList,
            FollowUpPlanListResponse::getContactId,
            FollowUpPlanListResponse::getContactName);
    optionMap.put(BusinessModuleField.OPPORTUNITY_CONTACT.getBusinessKey(), contactFieldOption);
    return optionMap;
  }

  public List<FollowUpPlanListResponse> buildListData(
      List<FollowUpPlanListResponse> list, String orgId) {
    ...
    Map<String, List<BaseModuleFieldValue>> resourceFieldMap =
        followUpPlanFieldService.getResourceFieldMap(ids, true);

    List<String> createUserIds =
        list.stream().map(FollowUpPlanListResponse::getCreateUser).toList();
    List<String> updateUserIds =
        list.stream().map(FollowUpPlanListResponse::getUpdateUser).toList();
    ...
    List<String> opportunityIds =
        list.stream().map(FollowUpPlanListResponse::getOpportunityId).toList();
    ...
    list.forEach(
        planResponse -> {
          // 获取自定义字段
          List<BaseModuleFieldValue> planCustomerFields =
              resourceFieldMap.get(planResponse.getId());
  ...
  public FollowUpPlanDetailResponse get(String id, String orgId) {
    ...
    FollowUpPlanDetailResponse response =
        BeanUtils.copyBean(new FollowUpPlanDetailResponse(), followUpPlan);
    ...
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            List.of(response), FollowUpPlanDetailResponse::getModuleFields);
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerFormConfig, moduleFieldValues);
    ...
    List<OptionDTO> customerOption =
        moduleFormService.getBusinessFieldOption(
            buildList,
            FollowUpPlanListResponse::getCustomerId,
            FollowUpPlanListResponse::getCustomerName);
    ...
    List<OptionDTO> opportunityOption =
        moduleFormService.getBusinessFieldOption(
            buildList,
            FollowUpPlanListResponse::getOpportunityId,
            FollowUpPlanListResponse::getOpportunityName);
    optionMap.put(BusinessModuleField.FOLLOW_PLAN_OPPORTUNITY.getBusinessKey(), opportunityOption);

    // 线索
    List<OptionDTO> clueOption =
        moduleFormService.getBusinessFieldOption(
            buildList, FollowUpPlanListResponse::getClueId, FollowUpPlanListResponse::getClueName);
  ...
  public void cancelPlan(String id, String operator) {
    ...
    if (followUpPlan.getStatus().equals(FollowUpPlanStatusType.COMPLETED.name())
        && followUpPlan.getConverted()) {
      return;
    }
  ...
  public void deleteByCustomerIds(List<String> customerIds) {
    ...
    List<String> ids = getByCustomerIds(customerIds).stream().map(FollowUpPlan::getId).toList();
    deleteByIds(ids);
  }
  ...
  public void deleteByClueIds(List<String> clueIds) {
    ...
    List<String> ids = getByClueIds(clueIds).stream().map(FollowUpPlan::getId).toList();
    deleteByIds(ids);
  }
