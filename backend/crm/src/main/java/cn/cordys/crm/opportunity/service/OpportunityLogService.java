  ...
  public List<JsonDifferenceDTO> handleLogField(List<JsonDifferenceDTO> differences, String orgId) {
    ...
    for (JsonDifferenceDTO differ : differences) {
      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.OPPORTUNITY_OWNER.getBusinessKey())) {
        setUserFieldName(differ);
        continue;
      }

      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.OPPORTUNITY_PRODUCTS.getBusinessKey())) {
        setProductName(differ);
        continue;
      }

      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.OPPORTUNITY_CONTACT.getBusinessKey())) {
        setContactFieldName(differ);
        continue;
      }

      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.OPPORTUNITY_CUSTOMER_NAME.getBusinessKey())) {
      ...
      if (Strings.CS.equals(differ.getColumnName(), Translator.get("log.stage"))) {
        if (differ.getOldValue() != null) {
          differ.setOldValueName(
              Translator.get(
                  differ.getOldValueName().toString(), differ.getOldValueName().toString()));
        }
        if (differ.getNewValue() != null) {
          differ.setNewValueName(
              Translator.get(
                  differ.getNewValueName().toString(), differ.getNewValueName().toString()));
        }
      }
      ...
      if (Strings.CS.equals(differ.getColumn(), "status")) {
        ...
        if (differ.getOldValue() != null) {
          differ.setOldValueName(
              Boolean.parseBoolean(differ.getOldValueName().toString())
                  ? Translator.get("log.opportunity.status.true")
                  : Translator.get("log.opportunity.status.false"));
        }
        if (differ.getNewValue() != null) {
          differ.setNewValueName(
              Boolean.parseBoolean(differ.getNewValueName().toString())
                  ? Translator.get("log.opportunity.status.true")
                  : Translator.get("log.opportunity.status.false"));
        }
      }
