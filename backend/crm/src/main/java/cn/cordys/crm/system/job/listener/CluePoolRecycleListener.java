...
/** 线索池回收监听器 负责监听执行事件并将符合条件的线索回收到线索池 */
@Component
@Slf4j
public class CluePoolRecycleListener implements ApplicationListener<ExecuteEvent> {
  ...
  public void recycle() {
    ...
    // 获取所有者到线索池的最佳匹配映射
    Map<List<String>, CluePool> ownersPoolMap =
        cluePoolService.getOwnersBestMatchPoolMap(enabledPools);
    List<String> recycleOwnersIds = ownersPoolMap.keySet().stream().flatMap(List::stream).toList();
  ...
  private List<Clue> getCluesForRecycle(List<String> ownerIds) {
    ...
    clueQueryWrapper.in(Clue::getOwner, ownerIds).eq(Clue::getInSharedPool, false);
