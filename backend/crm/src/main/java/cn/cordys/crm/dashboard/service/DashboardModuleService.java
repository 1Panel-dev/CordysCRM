...
public class DashboardModuleService extends MoveNodeService {
  ...
  @Resource private ExtDashboardModuleMapper extDashboardModuleMapper;
  @Resource private BaseMapper<DashboardModule> dashboardModuleMapper;
  ...
  private static void buildIdNodeMap(
      List<BaseTreeNode> nodeList, Map<String, BaseTreeNode> idNodeMap) {
  ...
  public DashboardModule addFileModule(
      DashboardModuleAddRequest request, String orgId, String userId) {
    // 同一层级文件名称唯一
  ...
  public void rename(DashboardModuleRenameRequest request, String userId) {
    ...
    checkFileName(
        ...
        originalDashboardModule.getOrganizationId(),
        originalDashboardModule.getId());
    ...
    // 添加日志上下文
    String resourceName =
        Optional.ofNullable(dashboardModule.getName()).orElse(originalDashboardModule.getName());
    OperationLogContext.setContext(
        LogContextInfo.builder()
  ...
  public void delete(List<String> ids, String userId, String orgId) {
    ...
    // 添加日志上下文
    dashboardModules.forEach(
        dashboardModule -> {
          LogDTO logDTO =
              new LogDTO(
                  dashboardModule.getOrganizationId(),
                  dashboardModule.getId(),
                  userId,
                  LogType.DELETE,
                  LogModule.DASHBOARD,
                  dashboardModule.getName());
  ...
  public List<DashboardTreeNode> getTree(String userId, String orgId) {
    ...
    List<DashboardTreeNode> dashboardNode =
        extDashboardMapper.selectDashboardNode(departmentIds, orgId, userId);

    Set<String> myCollects = new HashSet<>(extDashboardCollectionMapper.getByUserId(userId));
    dashboardNode.forEach(
        node -> {
  ...
  public Map<String, Long> moduleCount(String userId, String orgId) {
    ...
    Map<String, Integer> moduleCountMap =
        dashboardNode.stream()
            .collect(
                Collectors.groupingBy(BaseTreeNode::getParentId, Collectors.summingInt(node -> 1)));
  ...
  private int calculateDashboardCount(
      BaseTreeNode node, Map<String, Long> countMap, Map<String, Integer> moduleCountMap) {
    int count = 0;
  ...
  public void moveNode(NodeMoveRequest request, String userId) {
    ...
    if (dashboardModuleMapper.countByExample(dashboardModule) == 0) {
      DashboardModule moveModule =
          dashboardModuleMapper.selectByPrimaryKey(request.getDragNodeId());
      moveModule.setParentId(nodeSortDTO.getParent().getId());
      checkFileName(
          moveModule.getName(), moveModule.getParentId(), moveModule.getOrganizationId(), null);
