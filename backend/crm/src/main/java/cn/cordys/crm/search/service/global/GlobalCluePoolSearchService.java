...
@Service
public class GlobalCluePoolSearchService
    extends BaseSearchService<BasePageRequest, GlobalCluePoolResponse> {
  ...
  @Override
  public Pager<List<GlobalCluePoolResponse>> startSearchNoOption(
      BasePageRequest request, String orgId, String userId) {
    // 获取查询关键字
    ...
    if (CollectionUtils.isNotEmpty(userSearchConfigs)) {
      List<UserSearchConfig> cluePoolSearchConfigs =
          userSearchConfigs.stream()
              .filter(
                  t ->
                      Strings.CI.equals(
                          t.getModuleType(), SearchModuleEnum.SEARCH_ADVANCED_CLUE_POOL))
              .toList();
      ...
      for (UserSearchConfig userSearchConfig : cluePoolSearchConfigs) {
        ...
        if (StringUtils.isBlank(userSearchConfig.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
                ...
                userSearchConfig.getBusinessKey(), BusinessModuleField.CLUE_NAME.getBusinessKey())
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(),
                BusinessModuleField.CLUE_PRODUCTS.getBusinessKey())
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(),
                BusinessModuleField.CLUE_CONTACT_PHONE.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
        }
      ...
      // 设置默认查询属性
      FilterCondition nameCondition =
          getFilterCondition(
              "name",
              keyword,
              FilterCondition.CombineConditionOperator.CONTAINS.toString(),
      ...
      FilterCondition phoneCondition =
          getFilterCondition(
              "phone",
              phoneList,
              FilterCondition.CombineConditionOperator.IN.toString(),
              FieldType.DATA_SOURCE.toString());
    ...
    // 查询重复商机列表
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<GlobalCluePoolResponse> globalCluePoolResponses =
        extClueMapper.globalPoolSearchList(request, orgId);
    if (CollectionUtils.isEmpty(globalCluePoolResponses)) {
      return PageUtils.setPageInfo(page, List.of());
    }
    // 获取系统设置的脱敏字段
    List<SearchFieldMaskConfig> searchFieldMaskConfigs =
        getSearchFieldMaskConfigs(orgId, SearchModuleEnum.SEARCH_ADVANCED_CLUE_POOL);
    List<GlobalCluePoolResponse> buildList =
        buildListData(
            globalCluePoolResponses,
            orgId,
            userId,
            searchFieldMaskConfigs,
            fieldIdSet,
            internalKeyMap);
    return PageUtils.setPageInfo(page, buildList);
  }

  public List<GlobalCluePoolResponse> buildListData(
      List<GlobalCluePoolResponse> list,
      String orgId,
      String userId,
      List<SearchFieldMaskConfig> searchFieldMaskConfigs,
    ...
    List<String> clueIds =
        list.stream().map(GlobalCluePoolResponse::getId).collect(Collectors.toList());
    Map<String, List<BaseModuleFieldValue>> clueFiledMap =
    ...
    Map<String, Clue> internalKeyValueMap =
        clues.stream().collect(Collectors.toMap(Clue::getId, t -> t));
    // 处理自定义字段选项数据
    ModuleFormConfigDTO clueFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.CLUE.getKey(), orgId);
    Map<String, SearchFieldMaskConfig> searchFieldMaskConfigMap =
        searchFieldMaskConfigs.stream()
            .collect(Collectors.toMap(SearchFieldMaskConfig::getFieldId, t -> t));
    list.forEach(
        globalClueResponse -> {
          // 判断该数据是否有权限
          ...
          if (CollectionUtils.isNotEmpty(fieldIdSet)) {
            List<BaseModuleFieldValue> returnCluePoolFields =
                getBaseModuleFieldValues(
                    fieldIdSet,
                    globalClueResponse.getId(),
                    clueFiledMap,
                    clueFormConfig,
                    searchFieldMaskConfigMap,
                    hasPermission);
          ...
          List<BaseModuleFieldValue> baseModuleFieldValues =
              buildInternalField(
                  internalKeyMap, searchFieldMaskConfigMap, hasPermission, clue, Clue.class);
          ...
          List<String> productNames =
              getProductNames(globalClueResponse.getProducts(), productNameMap);
          globalClueResponse.setProducts(productNames);
          if (!hasPermission) {
            searchFieldMaskConfigMap.forEach(
                (fieldId, searchFieldMaskConfig) -> {
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "name")) {
                    globalClueResponse.setName(
                        (String)
                            getInputFieldValue(
                                globalClueResponse.getName(),
                                globalClueResponse.getName().length()));
                  }
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "products")) {
                    List<String> maskProductNames =
                        productNames.stream()
                            .map(t -> (String) getInputFieldValue(t, t.length()))
                            .toList();
                    globalClueResponse.setProducts(maskProductNames);
                  }
                  if (Strings.CI.equals(searchFieldMaskConfig.getBusinessKey(), "phone")
                      && StringUtils.isNotBlank(globalClueResponse.getPhone())) {
                    globalClueResponse.setPhone(
                        (String)
                            getPhoneFieldValue(
                                globalClueResponse.getPhone(),
                                globalClueResponse.getPhone().length()));
                  }
                });
          }
  ...
  private boolean getHasPermission(
      String userId,
      String orgId,
      GlobalCluePoolResponse clueListResponse,
      Map<String, String> userPoolMap) {
    ...
    boolean hasPermission =
        dataScopeService.hasDataPermission(
            userId, orgId, new ArrayList<>(), PermissionConstants.CLUE_MANAGEMENT_POOL_READ);
