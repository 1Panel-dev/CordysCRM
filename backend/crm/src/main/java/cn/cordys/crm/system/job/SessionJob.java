...
public class SessionJob {
  ...
  @Resource private RedisIndexedSessionRepository redisIndexedSessionRepository;
   ...
   * 定时清理没有绑定用户的会话。
   *
   * <p>该方法每晚 0 点 2 分执行，扫描 Redis 中的会话数据，删除没有绑定用户信息的会话。 此外，还会处理一些特殊情况，如 Redisson 设置了过期时间为 -1
   * 时，手动设置过期时间。
   *
   * <p>该方法使用 {@link QuartzScheduled} 注解定时执行，并使用 {@link ScanOptions} 扫描 Redis 中的会话。
   *
   * <p>spring.session.timeout=30d server.servlet.session.timeout=30d
   */
      ...
      while (scan.hasNext()) {
        ...
        if (!exists) {
          ...
          if (expire != null && expire == -1) {
            redisIndexedSessionRepository
                .getSessionRedisOperations()
                .expire(key, Duration.of(30, ChronoUnit.SECONDS));
          }
        }
      }
      // 清理缓存
