 ...
 * 商机规则监听器
 *
 * <p>此监听器负责监听定时执行事件，根据预设规则自动处理商机状态。 当触发执行事件时，会根据启用的自动化规则检查所有相关商机， 并根据规则条件自动关闭符合条件的商机。
 */
@Component
@Slf4j
public class OpportunityRuleListener implements ApplicationListener<ExecuteEvent> {
  ...
  @Resource private OpportunityRuleService opportunityRuleService;
   ...
   * 执行商机关闭规则任务
   *
   * <p>此方法执行以下步骤： 1. 查询所有已启用且设置为自动执行的商机规则 2. 获取规则对应的最佳匹配所有者映射关系 3. 查询相关所有者的所有商机 4. 根据规则检查每个商机是否需要关闭
   * 5. 自动关闭符合条件的商机
   */
  public void execute() {
    log.info("开始回收商机资源");
    ...
    // 获取所有者与规则的最佳匹配映射
    Map<List<String>, OpportunityRule> ownersBestMatchRuleMap =
        opportunityRuleService.getOwnersBestMatchRuleMap(rules);
    List<String> handleOwnerIds =
        ownersBestMatchRuleMap.keySet().stream().flatMap(List::stream).toList();
    ...
    List<OpportunityStageConfig> stageConfigList =
        extOpportunityStageConfigMapper.getAllStageConfigList();

    Map<String, String> orgFailStageConfigMap =
        stageConfigList.stream()
            .filter(
                config ->
                    Strings.CI.equals(config.getType(), OpportunityStageType.END.name())
                        && Strings.CI.equals(config.getRate(), "0"))
            .collect(
                Collectors.toMap(
                    OpportunityStageConfig::getOrganizationId, OpportunityStageConfig::getId));

    // 根据规则处理每个商机
    opportunities.forEach(
        opportunity ->
            ownersBestMatchRuleMap.forEach(
                (ownerIds, rule) -> {
                  if (ownerIds.contains(opportunity.getOwner())) {
                    ...
                    if (closed) {
                      ...
                      opportunity.setStage(
                          orgFailStageConfigMap.get(opportunity.getOrganizationId()));
