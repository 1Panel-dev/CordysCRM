...
public class UserSearchConfigService {
  ...
  @Resource private BaseMapper<UserSearchConfig> userSearchConfigMapper;
  ...
  public void save(UserSearchConfigAddRequest request, String userId, String orgId) {
    boolean allEmpty =
        request.getSearchFields().values().stream()
            ...
            (key, value) -> {
              ...
              switch (key) {
                ...
                case SearchModuleEnum.SEARCH_ADVANCED_OPPORTUNITY:
                  saveSearchFields(
                      value, userId, orgId, FormKey.OPPORTUNITY.getKey(), key, request);
                  break;
                default:
                  break;
              }
  ...
  private void deleteUserSearchConfig(String userId, String orgId) {
    ...
    queryWrapper
        .eq(UserSearchConfig::getUserId, userId)
    ...
    if (CollectionUtils.isNotEmpty(fieldIds)) {
      ...
      for (String fieldId : fieldIds) {
        ...
        if (baseField.getType().equals("DATA_SOURCE_MULTIPLE")) {
          userSearchConfig.setDataSourceType(
              ((DatasourceMultipleField) baseField).getDataSourceType());
        }
   ...
   */
  private void buildContactFields(
      String formKey, String userId, String orgId, List<UserSearchConfig> defaultSearchConfigs) {
    ModuleFormConfigDTO businessFormConfig =
        moduleFormCacheService.getBusinessFormConfig(formKey, orgId);
    businessFormConfig
        .getFields()
        .forEach(
            field -> {
              if (Strings.CI.equals(
                  field.getInternalKey(), DefaultSearchFieldEnum.CONTACT_CUSTOMER)) {
                buildDto(
                    field,
                    userId,
                    orgId,
                    SearchModuleEnum.SEARCH_ADVANCED_CONTACT,
            ...
            field -> {
              if (Strings.CI.equals(field.getInternalKey(), DefaultSearchFieldEnum.CLUE_NAME)) {
                buildDto(
                    ...
                    userId,
                    orgId,
                    SearchModuleEnum.SEARCH_ADVANCED_CLUE,
                    defaultSearchConfigs);
                buildDto(
                    field,
                    userId,
              ...
              if (Strings.CI.equals(
                  field.getInternalKey(), DefaultSearchFieldEnum.CLUE_CONTACT_PHONE)) {
                buildDto(
                    field,
                    userId,
                    orgId,
            ...
            field -> {
              if (Strings.CI.equals(field.getInternalKey(), DefaultSearchFieldEnum.CUSTOMER_NAME)) {
                buildDto(
                    ...
                    userId,
                    orgId,
                    SearchModuleEnum.SEARCH_ADVANCED_CUSTOMER,
                    defaultSearchConfigs);
                buildDto(
                    field,
                    userId,
   ...
   * @param defaultSearchConfigs
   */
  private void buildOpportunityFields(
      String formKey, String userId, String orgId, List<UserSearchConfig> defaultSearchConfigs) {
    ModuleFormConfigDTO businessFormConfig =
        moduleFormCacheService.getBusinessFormConfig(formKey, orgId);
    businessFormConfig
        ...
        .forEach(
            field -> {
              if (Strings.CI.equals(
                  field.getInternalKey(), DefaultSearchFieldEnum.OPPORTUNITY_NAME)) {
                buildDto(
                    field,
                    userId,
                    orgId,
                    SearchModuleEnum.SEARCH_ADVANCED_OPPORTUNITY,
                    defaultSearchConfigs);
              }
              if (Strings.CI.equals(
                  field.getInternalKey(), DefaultSearchFieldEnum.OPPORTUNITY_CUSTOMER)) {
                buildDto(
                    field,
                    userId,
                    orgId,
  ...
  private void buildDto(
      ...
      String orgId,
      String moduleType,
      List<UserSearchConfig> defaultSearchConfigs) {
    UserSearchConfig userSearchConfig = new UserSearchConfig();
    userSearchConfig.setId(IDGenerator.nextStr());
        ...
        searchConfigs.stream()
            .collect(
                Collectors.groupingBy(
                    ...
                    Collectors.mapping(UserSearchConfig::getFieldId, Collectors.toList())));
