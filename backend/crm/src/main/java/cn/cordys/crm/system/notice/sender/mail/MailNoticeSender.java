...
public class MailNoticeSender extends AbstractNoticeSender {
  ...
  public void sendMail(
      String context, NoticeModel noticeModel, String organizationId, String subjectText) {
    ...
    List<String> userIds =
        receivers.stream().map(Receiver::getUserId).distinct().collect(Collectors.toList());
    String[] users =
        super.getUsers(userIds, organizationId).stream()
            .map(User::getEmail)
            .distinct()
            .toArray(String[]::new);
