...
public class OpportunityRuleService {
  ...
  @Resource private BaseMapper<OpportunityRule> opportunityRuleMapper;
  @Resource private ExtOpportunityRuleMapper extOpportunityRuleMapper;
  ...
  public List<OpportunityRuleDTO> page(BasePageRequest request, String organizationId) {
    ...
    List<String> userIds =
        rules.stream()
            .flatMap(rule -> Stream.of(rule.getCreateUser(), rule.getUpdateUser()))
            .toList();
    ...
    rules.forEach(
        rule -> {
          delOldTime(rule);
          rule.setMembers(
              userExtendService.getScope(JSON.parseArray(rule.getScopeId(), String.class)));
          rule.setOwners(
              userExtendService.getScope(JSON.parseArray(rule.getOwnerId(), String.class)));
  ...
  private void delOldTime(OpportunityRuleDTO rule) {
    List<RuleConditionDTO> ruleConditionDTOS =
        JSON.parseArray(rule.getCondition(), RuleConditionDTO.class);
  ...
  public void update(
      OpportunityRuleUpdateRequest request, String currentUserId, String organizationId) {
    ...
    OperationLogContext.setContext(
        LogContextInfo.builder()
            .resourceName(
                Translator.get("module.opportunity.rule.setting") + ": " + oldRule.getName())
  ...
  public void delete(String id) {
    ...
    // 设置操作对象
    OperationLogContext.setResourceName(
        Translator.get("module.opportunity.rule.setting") + ": " + rule.getName());
  }
   ...
   * @return 默认规则
   */
  public Map<String, OpportunityRule> getOwnersDefaultRuleMap(
  ...
  public Map<List<String>, OpportunityRule> getOwnersBestMatchRuleMap(List<OpportunityRule> rules) {
    ...
    for (OpportunityRule rule : rules) {
      ...
      List<String> ownerIds =
          userExtendService.getScopeOwnerIds(scopeIds, rule.getOrganizationId());
