...
public abstract class DataScopeTablePermissionHandler extends ModuleFieldTablePermissionHandler {
  ...
  public void handleTable(
      TableDTO table, TableHandleParam tableHandleParam, ModuleFormConfigDTO formConfig) {
  ...
  protected String getProductsFieldSql() {
    return "(select JSON_ARRAYAGG(p.name) from product p where c.products like concat('%', p.id,"
               + " '%')) as products";
  }

  protected String getTableSql(
      List<FieldDTO> sqlBotFields,
      TableHandleParam tableHandleParam,
      List<BaseField> moduleFields) {
    String fieldsSql =
        parseFieldsSql(tableHandleParam.getTableInfo().getTableName() + "_field", moduleFields);
    sqlBotFields =
        sqlBotFields.stream()
    ...
    if (Strings.CS.equals(tableHandleParam.getUserId(), InternalUser.ADMIN.getValue())
        || dataPermission.getAll()) {
      return dataScopeSql;
    } else if (CollectionUtils.isNotEmpty(dataPermission.getDeptIds())) {
      String deptIdStr = getInConditionStr(dataPermission.getDeptIds());
      return dataScopeSql
          + String.format(
              " and (sou.department_id in (%s) or sou.user_id = '%s')",
              deptIdStr, tableHandleParam.getUserId());
    } else {
