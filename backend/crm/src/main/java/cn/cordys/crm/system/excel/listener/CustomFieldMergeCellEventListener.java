...
public class CustomFieldMergeCellEventListener extends AnalysisEventListener<Map<Integer, String>> {
  ...
  /** 合并单元格信息 (按行号分组, 方便查询) */
  @Getter private final Map<Integer, List<CellExtra>> mergeCellMap = new HashMap<>();

  @Getter private final Map<Integer, Map<Integer, String>> mergeRowDataMap = new HashMap<>();
  ...
  public void extra(CellExtra extra, AnalysisContext context) {
    if (extra.getType() == CellExtraTypeEnum.MERGE && extra.getRowIndex() >= maxHeadRow) {
      ...
      for (int r = firstRow; r <= lastRow; r++) {
        mergeRowDataMap.computeIfAbsent(r, k -> new HashMap<>(8)).put(colIndex, val);
      }
    }
  }
