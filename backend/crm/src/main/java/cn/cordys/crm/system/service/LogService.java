  ...
  public void add(LogDTO log) {
    ...
    if (extra != null) {
      ...
      if (StringUtils.isBlank(log.getResourceId())
          && ObjectUtils.isNotEmpty(extra.getResourceId())) {
        log.setResourceId(extra.getResourceId());
      }

      // 如果注解中没有设置 resourceName ，则使用 extra 中的 resourceName
      if (StringUtils.isBlank(log.getResourceName())
          && ObjectUtils.isNotEmpty(extra.getResourceName())) {
