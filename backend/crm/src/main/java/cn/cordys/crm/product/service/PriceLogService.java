...
public class PriceLogService extends BaseModuleLogService {
  ...
  public List<JsonDifferenceDTO> handleLogField(List<JsonDifferenceDTO> differences, String orgId) {
    ...
    for (JsonDifferenceDTO differ : differences) {
      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.PRICE_STATUS.getBusinessKey())) {
  ...
  private void setPriceStatus(JsonDifferenceDTO differ, String orgId) {
    ModuleFormConfigDTO priceConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.PRICE.getKey(), orgId);
    Optional<BaseField> statusField =
        priceConfig.getFields().stream()
            .filter(
                f ->
                    Strings.CS.equals(
                        BusinessModuleField.PRICE_STATUS.getKey(), f.getInternalKey()))
            .findFirst();
    ...
    differ.setNewValueName(
        options.stream()
            .filter(
                optionProp ->
                    Strings.CS.equals(optionProp.getValue(), differ.getNewValue().toString()))
            .map(OptionProp::getLabel));
    differ.setOldValueName(
        options.stream()
            .filter(
                optionProp ->
                    Strings.CS.equals(optionProp.getValue(), differ.getOldValue().toString()))
            .map(OptionProp::getLabel));
  }
}
