...
public class ProductPriceService {
  ...
  @Resource private BaseMapper<ProductPrice> productPriceMapper;
  ...
  @Resource private ProductPriceFieldService productPriceFieldService;
  @Resource private ExtProductPriceMapper extProductPriceMapper;
  @Resource private LogService logService;
  @Resource private BaseMapper<OpportunityQuotationField> opportunityFieldMapper;
  ...
  public PagerWithOption<List<ProductPriceResponse>> list(
      ProductPricePageRequest request, String currentOrg) {
    ...
    // 处理自定义字段选项
    ModuleFormConfigDTO priceFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.PRICE.getKey(), currentOrg);
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(results, ProductPriceResponse::getModuleFields);
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(priceFormConfig, moduleFieldValues);
   ...
   */
  @OperationLog(
      module = LogModule.PRODUCT_PRICE_MANAGEMENT,
      type = LogType.ADD,
  ...
  public ProductPrice add(ProductPriceAddRequest request, String currentUser, String currentOrg) {
    ...
    productPriceFieldService.saveModuleField(
        productPrice, currentOrg, currentUser, request.getModuleFields(), false);
    productPriceMapper.insert(productPrice);
    // 处理日志上下文
    ModuleFormConfigDTO priceFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.PRICE.getKey(), currentOrg);
    baseService.handleAddLogWithSubTable(
        productPrice, request.getModuleFields(), Translator.get("products_info"), priceFormConfig);
    return productPrice;
  }
  ...
  public ProductPrice update(
      ProductPriceEditRequest request, String currentUser, String currentOrg) {
    ...
    List<BaseModuleFieldValue> originFields =
        productPriceFieldService.getModuleFieldValuesByResourceId(request.getId());
    ProductPrice productPrice = BeanUtils.copyBean(new ProductPrice(), request);
    productPrice.setUpdateTime(System.currentTimeMillis());
    productPrice.setUpdateUser(currentUser);
    // 设置子表格字段值
  ...
  public ProductPriceGetResponse get(String id) {
    ...
    List<BaseModuleFieldValue> fieldValues =
        productPriceFieldService.getModuleFieldValuesByResourceId(id);
    ModuleFormConfigDTO priceFormConf =
        moduleFormCacheService.getBusinessFormConfig(
            FormKey.PRICE.getKey(), price.getOrganizationId());
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(priceFormConf, fieldValues);
    priceDetail.setOptionMap(optionMap);
    moduleFormService.processBusinessFieldValues(priceDetail, fieldValues, priceFormConf);
    priceDetail.setAttachmentMap(
        moduleFormService.getAttachmentMap(priceFormConf, priceDetail.getModuleFields()));
  ...
  public void delete(String id) {
    // 1. 查询价格表，不存在则抛错
    ProductPrice price =
        Optional.ofNullable(productPriceMapper.selectByPrimaryKey(id))
            .orElseThrow(() -> new GenericException(Translator.get("product.price.not.exist")));

    // 2. 检查是否被引用（报价单等）
    List<OpportunityQuotationField> opportunityFields =
        opportunityFieldMapper.selectListByLambda(
            new LambdaQueryWrapper<OpportunityQuotationField>()
                .eq(OpportunityQuotationField::getFieldValue, id));

    List<OpportunityQuotationFieldBlob> opportunityBlobFields =
        opportunityQuotationFieldBlobBaseMapper.selectListByLambda(
    ...
    if (!opportunityFields.isEmpty() || !opportunityBlobFields.isEmpty()) {
      throw new GenericException(Translator.get("product.price.in.use.cannot.delete"));
    }
  ...
  public ProductPrice copy(String id, String currentUser, String currentOrg) {
    ...
    price.setName(
        price.getName()
            + "_copy_"
            + RandomStringUtils.random(6, 0, 0, true, true, null, new Random()));
    ...
    List<BaseModuleFieldValue> fvs =
        productPriceFieldService.getModuleFieldValuesByResourceId(price.getId());
    baseService.handleAddLogWithSubTable(
        price, fvs, Translator.get("products_info"), priceFormConfig);
  ...
  public void batchUpdate(ResourceBatchEditRequest request, String currentUser, String currentOrg) {
    ...
    productPriceFieldService.batchUpdate(
        request,
        field,
        prices,
        ProductPrice.class,
        LogModule.PRODUCT_PRICE_MANAGEMENT,
        extProductPriceMapper::batchUpdate,
        currentUser,
        currentOrg);
  }
    ...
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            ...
            new CustomTemplateWriteHandler(
                moduleFormService.getAllCustomImportFields(FormKey.PRICE.getKey(), currentOrg)),
  ...
  public List<ProductPriceResponse> buildList(List<ProductPriceResponse> listData) {
    ...
    Map<String, List<BaseModuleFieldValue>> dataFieldMap =
        productPriceFieldService.getResourceFieldMap(
   ...
   * @return 列表数据
   */
  public List<ProductPriceResponse> processList(
      List<ProductPriceResponse> listData, ModuleFormConfigDTO priceFormConf) {
    // 查询列表数据的自定义字段
    Map<String, List<BaseModuleFieldValue>> dataFieldMap =
        productPriceFieldService.getResourceFieldMap(
            listData.stream().map(ProductPriceResponse::getId).toList(), true);
    ...
    listData.forEach(
        item -> {
          ...
          moduleFormService.processBusinessFieldValues(
              item, dataFieldMap.get(item.getId()), priceFormConf);
        });
    return baseService.setCreateAndUpdateUserName(listData);
  }
  ...
  public void editPos(PosRequest request) {
    ServiceUtils.updatePosFieldByAsc(
        request,
        ProductPrice.class,
        null,
        null,
   ...
   */
  private void copyPriceFields(
      String sourceId, String targetId, String currentOrg, String currentUser) {
    ...
    if (CollectionUtils.isNotEmpty(sourceFields)) {
      List<ProductPriceField> targetFields =
          sourceFields.stream()
              .peek(
                  field -> {
    ...
    List<ProductPriceFieldBlob> sourceBlobs =
        productPriceFieldBlobMapper.selectListByLambda(blobQuery);

    Map<String, String> attachmentIdMap = new HashMap<>(8);
    List<String> attachmentFieldIds =
        moduleFieldExtService.getFieldIdsOfForm(FormKey.PRICE.getKey(), currentOrg);

    if (CollectionUtils.isNotEmpty(sourceBlobs)) {
      List<ProductPriceFieldBlob> targetBlobs =
          sourceBlobs.stream()
              .peek(
                  blob -> {
                    ...
                    if (attachmentFieldIds.contains(blob.getFieldId())
                        && blob.getFieldValue() != null) {

                      List<String> attachmentIds =
                          JSON.parseArray(blob.getFieldValue().toString(), String.class);
