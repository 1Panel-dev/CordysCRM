...
public class BusinessTitleService {
  ...
  @Resource private BaseMapper<BusinessTitle> businessTitleMapper;
  @Resource private BaseMapper<ContractInvoice> contractInvoiceMapper;
  @Resource private ExtBusinessTitleMapper extBusinessTitleMapper;
  @Resource private LogService logService;
  @Resource private BaseService baseService;
  @Resource private BaseMapper<BusinessTitleConfig> businessTitleConfigMapper;
  @Resource private IntegrationConfigService integrationConfigService;
   ...
   */
  @OperationLog(
      module = LogModule.CONTRACT_BUSINESS_TITLE,
      type = LogType.ADD,
  ...
  public boolean checkHasInvoice(String id) {
    ...
    List<ContractInvoice> contractInvoices =
        contractInvoiceMapper.selectListByLambda(invoiceLambdaQueryWrapper);
   ...
   * @return
   */
  public Pager<List<BusinessTitleListResponse>> list(
      BusinessTitlePageRequest request, String userId, String orgId) {
  ...
  public BusinessTitleListResponse get(String id) {
    ...
    BusinessTitleListResponse businessTitleListResponse =
        BeanUtils.copyBean(new BusinessTitleListResponse(), businessTitle);
  ...
  private LogDTO getApprovalLogDTO(
    ...
    LogDTO logDTO =
        new LogDTO(
            orgId, id, userId, LogType.APPROVAL, LogModule.CONTRACT_BUSINESS_TITLE, response);
  ...
  public void downloadImportTpl(HttpServletResponse response, String orgId) {
    ...
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            response,
            heads,
            Translator.get("business_title_import_tpl.name"),
            Translator.get(SheetKey.DATA),
            Translator.get(SheetKey.COMMENT),
            new BusinessTitleTemplateWriteHandler(heads, getBusinessTitleConfig(orgId)),
  ...
  private Map<String, Boolean> getBusinessTitleConfig(String orgId) {
    ...
    List<BusinessTitleConfig> businessTitleConfigs =
        businessTitleConfigMapper.selectListByLambda(configWrapper);
    return businessTitleConfigs.stream()
        .collect(Collectors.toMap(BusinessTitleConfig::getField, BusinessTitleConfig::getRequired));
  }
  ...
  private ImportResponse checkImportExcel(MultipartFile file, String orgId) {
    try {
      ...
      BusinessTitleCheckEventListener eventListener =
          new BusinessTitleCheckEventListener(
              clazz, getBusinessTitleConfig(orgId), orgId, getTemplateHead());
  ...
  public ImportResponse realImport(MultipartFile file, String userId, String orgId) {
    ...
    try {
      Class<?> clazz = new UserExcelDataFactory().getExcelDataByLocal();
      Consumer<List<BusinessTitle>> afterDto =
          (businessTitles) -> {
            List<LogDTO> logs = new ArrayList<>();
            businessTitles.forEach(
                title -> {
                  ...
                  logs.add(
                      new LogDTO(
                          ...
                          userId,
                          LogType.ADD,
                          LogModule.CONTRACT_BUSINESS_TITLE,
                          title.getName()));
                });
      ...
      var eventListener =
          new BusinessTitleImportEventListener(
              clazz,
              BusinessTitle.class,
              getBusinessTitleConfig(orgId),
              orgId,
              userId,
              afterDto,
              getTemplateHead());
  ...
  public BusinessTitle thirdQuery(String keyword, String orgId) {
    ...
    ThirdConfigBaseDTO<?> thirdConfig =
        integrationConfigService.getThirdConfigForPublic(
            ThirdConfigTypeConstants.QCC.name(), orgId);
    if (thirdConfig.getVerify()) {
      QccThirdConfigRequest qccConfig =
          JSON.MAPPER.convertValue(thirdConfig.getConfig(), QccThirdConfigRequest.class);
  ...
  private BusinessTitle getTitleInfo(String keyword, QccThirdConfigRequest qccConfig) {
    ...
    try {
      String url =
          HttpRequestUtil.urlTransfer(
              qccConfig.getQccAddress().concat(QccApiPaths.ENTERPRISE_INFO_VERIFY_API),
              qccConfig.getQccAccessKey(),
              keyword);
  ...
  private void getNameList(
      String keyword, QccThirdConfigRequest qccConfig, String pageIndex, Pager<List<String>> page) {
    Map<String, String> headers = buildHeaders(qccConfig);
    try {
      String url =
          HttpRequestUtil.urlTransfer(
              qccConfig.getQccAddress().concat(QccApiPaths.FUZZY_SEARCH_LIST_API),
              qccConfig.getQccAccessKey(),
              keyword,
              pageIndex);
      String json = HttpRequestUtil.sendGetRequest(url, headers);
  ...
  private Map<String, String> buildHeaders(QccThirdConfigRequest qccConfig) {
    ...
    String token =
        EncryptUtils.md5(qccConfig.getQccAccessKey() + time + qccConfig.getQccSecretKey())
            .toUpperCase();
