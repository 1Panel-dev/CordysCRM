...
public class OpportunityQuotationService {
  ...
  @Resource private OpportunityQuotationFieldService opportunityQuotationFieldService;
  ...
  @Resource private ExtOpportunityQuotationMapper extOpportunityQuotationMapper;
  @Resource private SqlSessionFactory sqlSessionFactory;
  @Resource private BaseMapper<OpportunityQuotation> opportunityQuotationMapper;
  @Resource private BaseMapper<OpportunityQuotationSnapshot> snapshotBaseMapper;
  ...
  @Resource private BaseMapper<OpportunityQuotationApproval> approvalBaseMapper;
  @Resource private BaseMapper<Opportunity> opportunityBaseMapper;

  private static final ObjectMapper mapper = new ObjectMapper();

  /**
   * 新增商机报价单 新增报价单会自动将报价单状态设置为“提审”，此时需要保存报价单值快照，报价单表单设置快照
   *
   * @param request 新增请求参数
   * @return 商机报价单实体
   */
  @OperationLog(
      module = LogModule.OPPORTUNITY_QUOTATION,
      type = LogType.ADD,
      resourceName = "{#request.name}",
      operator = "{#userId}")
  public OpportunityQuotation add(
      OpportunityQuotationAddRequest request, String orgId, String userId) {
    ...
    ModuleFormConfigDTO saveModuleFormConfigDTO =
        JSON.parseObject(JSON.toJSONString(moduleFormConfigDTO), ModuleFormConfigDTO.class);
    ...
    opportunityQuotationFieldService.saveModuleField(
        opportunityQuotation, orgId, userId, moduleFields, false);
    opportunityQuotationMapper.insert(opportunityQuotation);

    // 保存表单配置快照
    List<BaseModuleFieldValue> resolveFieldValues =
        moduleFormService.resolveSnapshotFields(
            moduleFields,
            moduleFormConfigDTO,
            opportunityQuotationFieldService,
            opportunityQuotation.getId());
    OpportunityQuotationGetResponse response =
        getOpportunityQuotationGetResponse(
            opportunityQuotation, resolveFieldValues, moduleFormConfigDTO);

    baseService.handleAddLogWithSubTable(
        opportunityQuotation, moduleFields, Translator.get("products_info"), moduleFormConfigDTO);
  ...
  private void setAmount(
      List<Map<String, Object>> products, OpportunityQuotation opportunityQuotation) {
  ...
  private void saveSnapshot(
    ...
    response.setModuleFields(
        response.getModuleFields().stream()
            .filter(
                field ->
                    (field.getFieldValue() != null
                        && StringUtils.isNotBlank(field.getFieldValue().toString())
                        && !"[]".equals(field.getFieldValue().toString())))
            .toList());
   ...
   * @return 报价单详情
   */
  private OpportunityQuotationGetResponse getOpportunityQuotationGetResponse(
      OpportunityQuotation opportunityQuotation,
      List<BaseModuleFieldValue> moduleFields,
      ModuleFormConfigDTO moduleFormConfigDTO) {
    OpportunityQuotationGetResponse response =
        BeanUtils.copyBean(new OpportunityQuotationGetResponse(), opportunityQuotation);
    List<BaseModuleFieldValue> fvs =
        opportunityQuotationFieldService
            .setBusinessRefFieldValue(
                List.of(response),
                moduleFormService.getFlattenFormFields(
                    FormKey.QUOTATION.getKey(), opportunityQuotation.getOrganizationId()),
                new HashMap<>(Map.of(response.getId(), moduleFields)))
            .get(response.getId());
    ...
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(moduleFormConfigDTO, fvs);
    if (opportunity != null) {
      optionMap.put(
          "opportunityId", List.of(new OptionDTO(opportunity.getId(), opportunity.getName())));
      response.setOpportunityName(opportunity.getName());
    }
    response.setOptionMap(optionMap);
    Map<String, List<Attachment>> attachmentMap =
        moduleFormService.getAttachmentMap(moduleFormConfigDTO, moduleFields);
  ...
  public OpportunityQuotationGetResponse getSnapshot(String id) {
    ...
    OpportunityQuotationSnapshot snapshot =
        snapshotBaseMapper.selectListByLambda(wrapper).stream().findFirst().orElse(null);
    if (snapshot != null) {
      response =
          JSON.parseObject(snapshot.getQuotationValue(), OpportunityQuotationGetResponse.class);
    }
    return response;
  }
  ...
  public OpportunityQuotationGetResponse get(String id) {
    ...
    List<BaseModuleFieldValue> moduleFieldValues =
        opportunityQuotationFieldService.getModuleFieldValuesByResourceId(id);
    List<BaseModuleFieldValue> resolveFieldValues =
        moduleFormService.resolveSnapshotFields(
            moduleFieldValues,
            moduleFormConfigDTO,
    ...
    List<BaseModuleFieldValue> fvs =
        opportunityQuotationFieldService
            .setBusinessRefFieldValue(
                ...
                moduleFormService.getFlattenFormFields(
                    FormKey.QUOTATION.getKey(), opportunityQuotation.getOrganizationId()),
                new HashMap<>(Map.of(response.getId(), resolveFieldValues)))
            .get(response.getId());
    response.setModuleFields(fvs);
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(moduleFormConfigDTO, fvs);
    ...
    response.setOptionMap(optionMap);
    Map<String, List<Attachment>> attachmentMap =
        moduleFormService.getAttachmentMap(moduleFormConfigDTO, response.getModuleFields());
    response.setAttachmentMap(attachmentMap);
    baseService.setCreateAndUpdateUserName(response);
    UserDeptDTO userDeptDTO =
        baseService.getUserDeptMapByUserId(
            opportunityQuotation.getCreateUser(), opportunityQuotation.getOrganizationId());
    if (userDeptDTO != null) {
  ...
  public String revoke(String id, String userId) {
    ...
    if (!Strings.CI.equals(opportunityQuotation.getCreateUser(), userId)
        || !Strings.CI.equals(
            opportunityQuotation.getApprovalStatus(), ApprovalState.APPROVING.toString())) {
  ...
  public String voidQuotation(String id, String userId, String orgId) {
    OpportunityQuotation oldOpportunityQuotation =
        opportunityQuotationMapper.selectByPrimaryKey(id);
    if (oldOpportunityQuotation == null) {
      throw new GenericException(Translator.get("opportunity.quotation.not.exist"));
    }
    String oldApprovalStatus = oldOpportunityQuotation.getApprovalStatus();
    OpportunityQuotation opportunityQuotation =
        updateApprovalState(oldOpportunityQuotation, LogType.VOIDED, userId);
    if (opportunityQuotation == null) {
      throw new GenericException(Translator.get("opportunity.quotation.not.exist"));
    }
    ...
    // 增加通知
    sendNotice(
        Translator.get("opportunity.quotation.status.voided"),
        opportunityQuotation,
        userId,
        orgId,
        NotificationConstants.Event.BUSINESS_QUOTATION_APPROVAL);
    return opportunityQuotation.getApprovalStatus();
  }
  ...
  private void checkQuotationLinked(String id, String key) {
    ...
    List<ContractFieldBlob> contractFieldBlobList =
        contractFieldBlobMapper.selectListByLambda(wrapperBlob);

    if (CollectionUtils.isNotEmpty(contractFieldList)
        || CollectionUtils.isNotEmpty(contractFieldBlobList)) {
  ...
  public String approve(OpportunityQuotationEditRequest request, String userId, String orgId) {
    ...
    List<String> approvalStatusList =
        Arrays.stream(ApprovalState.values())
            .map(ApprovalState::getId)
            .filter(status -> ApprovalState.APPROVED.toString().equals(status))
            .toList();
    String noticeKey =
        approvalStatusList.contains(request.getApprovalStatus())
            ? Translator.get("opportunity.quotation.status.approved")
            : Translator.get("opportunity.quotation.status.unapproved");
    OpportunityQuotation oldOpportunityQuotation =
        opportunityQuotationMapper.selectByPrimaryKey(request.getId());
    if (oldOpportunityQuotation == null) {
      throw new GenericException(Translator.get("opportunity.quotation.not.exist"));
    }
    String oldApprovalStatus = oldOpportunityQuotation.getApprovalStatus();
    OpportunityQuotation opportunityQuotation =
        updateApprovalState(oldOpportunityQuotation, request.getApprovalStatus(), userId);
    if (opportunityQuotation == null) {
  ...
  private void updateSnapshot(
      String id, String approvalStatus, ModuleFormConfigDTO moduleFormConfigDTO) {
    ...
    List<OpportunityQuotationSnapshot> opportunityQuotationSnapshots =
        snapshotBaseMapper.selectListByLambda(delWrapper);
    OpportunityQuotationSnapshot first = opportunityQuotationSnapshots.getFirst();
    if (first != null) {
      OpportunityQuotationGetResponse response =
          JSON.parseObject(first.getQuotationValue(), OpportunityQuotationGetResponse.class);
   ...
   * @return 报价单
   */
  private OpportunityQuotation updateApprovalState(
      OpportunityQuotation opportunityQuotation, String approvalStatus, String userId) {
    if ((Strings.CI.equals(approvalStatus, ApprovalState.APPROVED.toString())
            || Strings.CI.equals(approvalStatus, ApprovalState.UNAPPROVED.toString()))
        && !Strings.CI.equals(
            opportunityQuotation.getApprovalStatus(), ApprovalState.APPROVING.toString())) {
      return null;
    }
  ...
  private void sendNotice(
    ...
    commonNoticeSendService.sendNotice(
        ...
        userId,
        orgId,
        List.of(opportunityQuotation.getCreateUser()),
        true);
  }
   ...
   * @param opportunityQuotation 报价单实体
   */
  private void saveSateChangeLog(
      String orgId,
      String state,
      String userId,
      String logType,
  ...
  public void delete(String id, String userId, String organizationId) {
    ...
    sendNotice(
        ...
        userId,
        organizationId,
        NotificationConstants.Event.BUSINESS_QUOTATION_DELETED);
  }
   ...
   * @return 商机报价单列表
   */
  public PagerWithOption<List<OpportunityQuotationListResponse>> list(
      OpportunityQuotationPageRequest request,
    ...
    List<OpportunityQuotationListResponse> list =
        extOpportunityQuotationMapper.list(
            request, organizationId, userId, deptDataPermission, source);
    ...
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            results, OpportunityQuotationListResponse::getModuleFields);
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(moduleFormConfigDTO, moduleFieldValues);
   ...
   * @return 列表数据
   */
  private List<OpportunityQuotationListResponse> buildList(
      List<OpportunityQuotationListResponse> listData, String organizationId) {
    // 查询列表数据的自定义字段
    Map<String, List<BaseModuleFieldValue>> dataFieldMap =
        opportunityQuotationFieldService.getResourceFieldMap(
            listData.stream()
                .map(OpportunityQuotationListResponse::getId)
                .collect(Collectors.toList()),
            true);
    Map<String, List<BaseModuleFieldValue>> resolvefieldValueMap =
        opportunityQuotationFieldService.setBusinessRefFieldValue(
            listData,
            moduleFormService.getFlattenFormFields(FormKey.QUOTATION.getKey(), organizationId),
            dataFieldMap);

    // 列表项设置自定义字段&&用户名
    List<String> createUserIds =
        listData.stream().map(OpportunityQuotationListResponse::getCreateUser).toList();
    Map<String, UserDeptDTO> userDeptMap =
        baseService.getUserDeptMapByUserIds(createUserIds, organizationId);
    listData.forEach(
        item -> {
  ...
  @OperationLog(
      ...
      resourceName = "{#request.name}",
      operator = "{#userId}")
  public OpportunityQuotation update(
      OpportunityQuotationEditRequest request, String userId, String orgId) {
    ...
    OpportunityQuotation opportunityQuotation =
        BeanUtils.copyBean(new OpportunityQuotation(), request);
    opportunityQuotation.setUpdateTime(System.currentTimeMillis());
    opportunityQuotation.setUpdateUser(userId);
    ...
    saveSnapshot(opportunityQuotation, saveModuleFormConfigDTO, response);
    // 处理日志上下文
    baseService.handleUpdateLogWithSubTable(
        oldOpportunityQuotation,
        opportunityQuotation,
        originFields,
        moduleFields,
        id,
        opportunityQuotation.getName(),
        Translator.get("products_info"),
        moduleFormConfigDTO);
   ...
   */
  private void checkQuotationInfo(
      List<BaseModuleFieldValue> moduleFields,
      ModuleFormConfigDTO moduleFormConfigDTO,
  ...
  private void updateQuotationApproval(String userId, String quotationId, String approvalStatus) {
    ...
    List<OpportunityQuotationApproval> approvalList =
        approvalBaseMapper.selectListByLambda(wrapper);
   ...
   * @param approvalStatus 审批状态
   */
  private void updateQuotationApprovalState(
      String userId, OpportunityQuotationApproval quotationApproval, String approvalStatus) {
  ...
  private void updateFields(
    ...
    opportunityQuotationFieldService.saveModuleField(
        opportunityQuotation, orgId, userId, fields, true);
  }
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.OPPORTUNITY_QUOTATION_READ, rolePermissions);
  }
   ...
   * @return 审批状态
   */
  public BatchAffectSkipResponse batchApprove(
      OpportunityQuotationBatchRequest request, String userId, String orgId) {
    ...
    ExtOpportunityQuotationMapper batchUpdateMapper =
        sqlSession.getMapper(ExtOpportunityQuotationMapper.class);
    ExtOpportunityQuotationSnapshotMapper extOpportunityQuotationSnapshotMapper =
        sqlSession.getMapper(ExtOpportunityQuotationSnapshotMapper.class);
    ...
    list.stream()
        .filter(
            item -> {
              if (!Strings.CI.equals(
                  item.getApprovalStatus(), ApprovalState.APPROVING.toString())) {
        ...
        .forEach(
            item -> {
              ...
              logs.add(log);
              batchUpdateMapper.updateApprovalStatus(
                  item.getId(), approvalStatus, userId, System.currentTimeMillis());
            });
    ...
    List<OpportunityQuotationSnapshot> opportunityQuotationSnapshots =
        snapshotBaseMapper.selectListByLambda(snapshotWrapper);
    ...
    list.forEach(
        item ->
            sendNotice(
                (Strings.CI.equals(approvalStatus, ApprovalState.APPROVED.toString())
                    ? Translator.get("opportunity.quotation.status.approved")
                    : Translator.get("opportunity.quotation.status.unapproved")),
                item,
                userId,
                orgId,
                NotificationConstants.Event.BUSINESS_QUOTATION_APPROVAL));
    return BatchAffectSkipResponse.builder()
        .success(list.size() - skipCount.get())
        .fail(0)
        .skip(skipCount.get())
        .build();
  }
   ...
   * @return 批量作废响应参数
   */
  public BatchAffectReasonResponse batchVoidQuotation(
      OpportunityQuotationBatchRequest request, String userId, String organizationId) {
    List<String> ids = request.getIds();
    LambdaQueryWrapper<OpportunityQuotation> wrapper = new LambdaQueryWrapper<>();
    wrapper.in(OpportunityQuotation::getId, ids);
    List<OpportunityQuotation> list = opportunityQuotationMapper.selectListByLambda(wrapper);
    if (CollectionUtils.isEmpty(list)) {
      return BatchAffectReasonResponse.builder()
          ...
          .fail(0)
          .skip(0)
          .errorMessages(Translator.get("opportunity.quotation.not.exist"))
          .build();
    }
    ...
    if (CollectionUtils.isEmpty(validateList)) {
      return BatchAffectReasonResponse.builder()
          ...
          .fail(list.size())
          .skip(0)
          .errorMessages(Translator.get("opportunity.quotation.batch.no.voided"))
          .build();
    }
    ...
    if (CollectionUtils.isNotEmpty(successIds)) {
      ...
      successIds.forEach(
          id ->
              batchUpdateMapper.updateApprovalStatus(
                  id, LogType.VOIDED, userId, System.currentTimeMillis()));
      // 批量修改报价单快照
      for (OpportunityQuotationSnapshot snapshot : opportunityQuotationSnapshots) {
        OpportunityQuotationGetResponse response =
            JSON.parseObject(snapshot.getQuotationValue(), OpportunityQuotationGetResponse.class);
    ...
    return BatchAffectReasonResponse.builder()
        ...
        .fail(list.size() - validateList.size())
        .skip(skipCount.get())
        .errorMessages(Translator.get("opportunity.quotation.batch.no.voided"))
        .build();
  }
  ...
  private List<OpportunityQuotation> validateVoidQuotation(List<OpportunityQuotation> list) {
    ...
    contractFields.forEach(
        cf -> existingIdSet.addAll(extractIdsFromFieldValue(cf.getFieldValue())));
    contractFieldBlobs.forEach(
        cfb -> existingIdSet.addAll(extractIdsFromFieldValue(cfb.getFieldValue())));
    return list.stream().filter(item -> !existingIdSet.contains(item.getId())).toList();
  }
  ...
  public ModuleFormConfigDTO getFormSnapshot(String id, String orgId) {
    ...
    if (snapshot != null) {
      moduleFormConfigDTO =
          JSON.parseObject(snapshot.getQuotationProp(), ModuleFormConfigDTO.class);
    } else {
      moduleFormConfigDTO =
          moduleFormCacheService.getBusinessFormConfig(FormKey.QUOTATION.getKey(), orgId);
    }
  ...
  public String getQuotationName(String id) {
    OpportunityQuotation opportunityQuotation = opportunityQuotationMapper.selectByPrimaryKey(id);
    return Optional.ofNullable(opportunityQuotation)
        .map(OpportunityQuotation::getName)
        .orElse(null);
  }
  ...
  public String getQuotationNameByIds(List<String> ids) {
    ...
    if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(opportunityQuotations)) {
      List<String> names =
          opportunityQuotations.stream().map(OpportunityQuotation::getName).toList();
