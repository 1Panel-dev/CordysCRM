...
public class OrganizationUserService {
  ...
  @Resource private ExtOrganizationUserMapper extOrganizationUserMapper;
  @Resource private ExtUserMapper extUserMapper;
  @Resource private RoleService roleService;
  @Resource private BaseMapper<User> userMapper;
  @Resource private BaseMapper<OrganizationUser> organizationUserMapper;
  @Resource private BaseMapper<UserRole> userRoleMapper;
  @Resource private ExtUserRoleMapper extUserRoleMapper;
  @Resource private LogService logService;
  @Resource private ExtDepartmentCommanderMapper extDepartmentCommanderMapper;
  @Resource private ExtDepartmentMapper extDepartmentMapper;
  @Resource private BaseMapper<DepartmentCommander> departmentCommanderMapper;
  @Resource private BaseMapper<UserExtend> userExtendMapper;
  @Resource private ExtUserExtendMapper extUserExtendMapper;
  @Resource private BaseMapper<Department> departmentMapper;
  ...
  @Resource private ExtUserViewMapper extUserViewMapper;
  @Resource private ExtUserViewConditionMapper extUserViewConditionMapper;
  @Resource private ExtUserSearchConfigMapper extUserSearchConfigMapper;
  @Resource private PermissionCache permissionCache;
  @Resource private ExtNotificationMapper extNotificationMapper;
  ...
  private String buildOrderByFieldClause(List<String> departmentIds) {
    ...
    departmentIds =
        departmentIds.stream().filter(id -> id.chars().allMatch(Character::isDigit)).toList();
  ...
  private void handleData(List<UserPageResponse> list, String departmentId) {
    if (CollectionUtils.isNotEmpty(list)) {
      ...
      Map<String, List<UserRoleConvert>> userRoleMap =
          userRoles.stream().collect(Collectors.groupingBy(UserRoleConvert::getUserId));
      // 创建人 更新人
      ...
      Map<String, String> userMap =
          options.stream().collect(Collectors.toMap(OptionDTO::getId, OptionDTO::getName));
      // 直属上级
      List<String> supervisorIds =
          list.stream().map(UserPageResponse::getSupervisorId).distinct().toList();
      List<OptionDTO> supervisors = extUserMapper.selectUserOptionByIds(supervisorIds);
      Map<String, String> supervisorMap =
          supervisors.stream().collect(Collectors.toMap(OptionDTO::getId, OptionDTO::getName));
      // 部门
      List<String> departmentIds =
          list.stream().map(UserPageResponse::getDepartmentId).distinct().toList();
      List<Department> departmentList =
          departmentMapper.selectByIds(departmentIds.toArray(new String[0]));
      Map<String, String> departmentMap =
          departmentList.stream().collect(Collectors.toMap(Department::getId, Department::getName));
      // 部门负责人
      List<String> commanderIds =
          extDepartmentCommanderMapper.selectCommander(
              departmentId, list.getFirst().getOrganizationId());
   ...
   */
  @OperationLog(
      module = LogModule.SYSTEM_ORGANIZATION,
      type = LogType.ADD,
      operator = "{#operatorId}")
  ...
  private void addUserInfo(
      UserAddRequest request, String organizationId, String operatorId, String userId) {
  ...
  private User addUserBaseData(UserAddRequest request, String operatorId, String id) {
    ...
    user.setId(id);
    user.setPassword(
        CodingUtils.md5(request.getPhone().substring(request.getPhone().length() - 6)));
  ...
  public UserResponse getUserDetail(String id) {
    ...
    // 获取用户角色
    List<UserRoleConvert> userRoles =
        extUserMapper.getUserRole(List.of(userDetail.getUserId()), userDetail.getOrganizationId());
    userRoles.forEach(role -> role.setName(roleService.translateInternalRole(role.getName())));
  ...
  private void updateUserRole(
      List<String> roleIds, UserResponse oldUser, String operatorId, String orgId) {
    Optional.ofNullable(roleIds)
        .ifPresent(ids -> extUserRoleMapper.deleteUserRoleByUserId(oldUser.getUserId()));
    if (CollectionUtils.isNotEmpty(roleIds)) {
  ...
  public void enable(UserBatchEnableRequest request, String operatorId, String orgId) {
    ...
    SubListUtils.dealForSubList(
        ...
        50,
        ids -> {
          List<OptionDTO> orgUsers =
              extOrganizationUserMapper.selectEnableOrgUser(ids, !request.isEnable());
  ...
  public void save(
      List<User> users,
      List<UserExtend> userExtends,
      List<OrganizationUser> organizationUsers,
      List<DepartmentCommander> departmentCommanders) {
  ...
  public void disableUsers(List<OrganizationUser> userList, String operatorId) {
    ...
    ExtOrganizationUserMapper extOrganizationUserMapper =
        sqlSession.getMapper(ExtOrganizationUserMapper.class);
   ...
   * @param organizationId
   */
  public void batchEditUser(
      UserBatchEditRequest request, String operatorId, String organizationId) {
  ...
  public void downloadExcelTemplate(HttpServletResponse response) {
    ...
    new EasyExcelExporter()
        .exportByCustomWriteHandler(
            ...
            heads,
            data,
            Translator.get("user_import_template_name"),
            Translator.get("user_import_template_sheet"),
            handler);
  }
  ...
  public void saveImportData(
      List<UserExcelData> list,
      List<BaseTreeNode> departmentTree,
      Map<String, String> departmentMap,
      String operatorId,
      String orgId) {
    // 部门
    ...
    List<LogDTO> logs = new ArrayList<>();
    // 构建部门树
    Map<String, String> departmentPathMap =
        departmentService.createDepartment(
            departmentPath, orgId, departmentTree, operatorId, departmentMap, logs);
    List<String> nameList =
        list.stream().map(UserExcelData::getSupervisor).filter(StringUtils::isNotBlank).toList();
  ...
  private void buildUser(
      UserExcelData userData,
      String operatorId,
      List<User> userList,
      Map<String, String> departmentPathMap,
      String orgId,
      List<OrganizationUser> organizationUsers,
      List<UserImportDTO> supervisorList,
      List<LogDTO> logs) {
    User user = new User();
    ...
    user.setPassword(
        CodingUtils.md5(userData.getPhone().substring(userData.getPhone().length() - 6)));
    user.setLanguage(Locale.SIMPLIFIED_CHINESE.toString());
    ...
    organizationUser.setSupervisorId(
        handleSupervisor(
            supervisorList, organizationUser.getDepartmentId(), userData.getSupervisor()));
   ...
   * @return
   */
  private String handleSupervisor(
      List<UserImportDTO> supervisorList, String departmentId, String name) {
    if (StringUtils.isBlank(name)) {
      return null;
    }
    List<UserImportDTO> departmentUsers =
        supervisorList.stream()
            .filter(
                supervisor ->
                    Strings.CI.equals(supervisor.getName(), name)
                        && Strings.CI.equals(supervisor.getDepartmentId(), departmentId))
            .toList();
    if (CollectionUtils.isNotEmpty(departmentUsers)) {
      ...
      List<DepartmentCommander> commanders =
          extDepartmentCommanderMapper.selectCommanderByUsers(departmentId, userIds);
    ...
    List<UserImportDTO> unDepartmentUsers =
        supervisorList.stream()
            .filter(supervisor -> Strings.CI.equals(supervisor.getName(), name))
            .toList();
  ...
  public List<OptionDTO> getUserOptions(String orgId) {
    ...
    String defaultOrder =
        CollectionUtils.isEmpty(allDpIds) ? StringUtils.EMPTY : buildOrderByFieldClause(allDpIds);
  ...
  public void updateUserName(UserUpdateName request, String operatorId) {
    ...
    extOrganizationUserMapper.updateUserByUserId(
        user.getId(), System.currentTimeMillis(), operatorId);
