...
public class ContractPaymentRecordService {
  ...
  @Resource private ModuleFieldExtService moduleFieldExtService;
  ...
  @Resource private BaseMapper<ContractPaymentPlan> contractPaymentPlanMapper;
  ...
  @Resource private ExtContractPaymentRecordMapper extContractPaymentRecordMapper;
  @Resource private ContractPaymentRecordFieldService contractPaymentRecordFieldService;
   ...
   * @return 回款记录列表
   */
  public PagerWithOption<List<ContractPaymentRecordResponse>> list(
      ContractPaymentRecordPageRequest request,
      String currentUser,
      String currentOrg,
      DeptDataPermissionDTO deptDataPermission) {
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<ContractPaymentRecordResponse> list =
        extContractPaymentRecordMapper.list(request, currentUser, currentOrg, deptDataPermission);
  ...
  @OperationLog(
      module = LogModule.CONTRACT_PAYMENT_RECORD,
      type = LogType.ADD,
      resourceName = "{#request.name}",
      operator = "{#currentUser}")
  public ContractPaymentRecord add(
      ContractPaymentRecordAddRequest request, String currentUser, String currentOrg) {
    ...
    List<String> rules =
        moduleFieldExtService.getSerialFieldRulesByKey(
            FormKey.CONTRACT_PAYMENT_RECORD.getKey(),
            currentOrg,
            BusinessModuleField.CONTRACT_PAYMENT_RECORD_NO.getKey());
    if (CollectionUtils.isNotEmpty(rules)) {
      paymentRecord.setNo(
          serialNumGenerator.generateByRules(
              rules, currentOrg, FormKey.CONTRACT_PAYMENT_RECORD.getKey()));
    }
    ...
    // 保存自定义字段值&回款记录
    contractPaymentRecordFieldService.saveModuleField(
        paymentRecord, currentOrg, currentUser, request.getModuleFields(), false);
  ...
  @OperationLog(
      ...
      type = LogType.UPDATE,
      operator = "{#currentUser}")
  public ContractPaymentRecord update(
      ContractPaymentRecordUpdateRequest request, String currentUser, String currentOrg) {
    ContractPaymentRecord oldRecord =
        contractPaymentRecordMapper.selectByPrimaryKey(request.getId());
    if (oldRecord == null) {
      throw new GenericException(Translator.get("record.not.exist"));
    }
    checkContractPaymentAmount(
        request.getContractId(), request.getRecordAmount(), oldRecord.getId());
    ContractPaymentRecord contractPaymentRecord =
        BeanUtils.copyBean(new ContractPaymentRecord(), request);
    ...
    List<BaseModuleFieldValue> oldFvs =
        contractPaymentRecordFieldService.getModuleFieldValuesByResourceId(request.getId());
  ...
  public ContractPaymentRecordGetResponse getWithDataPermissionCheck(
      String id, String currentUser, String currentOrg) {
  ...
  public ContractPaymentRecordGetResponse get(String id, String currentOrg) {
    ...
    ContractPaymentRecordGetResponse recordDetail =
        BeanUtils.copyBean(new ContractPaymentRecordGetResponse(), paymentRecord);
    ...
    ContractPaymentPlan contractPaymentPlan =
        contractPaymentPlanMapper.selectByPrimaryKey(recordDetail.getPaymentPlanId());
    // 自定义字段值 & 选项值
    List<BaseModuleFieldValue> fvs =
        contractPaymentRecordFieldService.getModuleFieldValuesByResourceId(id);
    fvs =
        contractPaymentRecordFieldService
            .setBusinessRefFieldValue(
                List.of(recordDetail),
                moduleFormService.getFlattenFormFields(
                    FormKey.CONTRACT_PAYMENT_RECORD.getKey(), paymentRecord.getOrganizationId()),
                new HashMap<>(Map.of(id, fvs)))
            .get(id);
    ModuleFormConfigDTO recordFormConf =
    ...
    optionMap.put(
        BusinessModuleField.CONTRACT_PAYMENT_RECORD_OWNER.getBusinessKey(),
        moduleFormService.getBusinessFieldOption(
            List.of(recordDetail),
            ContractPaymentRecordGetResponse::getOwner,
            ContractPaymentRecordGetResponse::getOwnerName));
    if (contract != null) {
      recordDetail.setContractName(contract.getName());
      optionMap.put(
          BusinessModuleField.CONTRACT_PAYMENT_RECORD_CONTRACT.getBusinessKey(),
          moduleFormService.getBusinessFieldOption(
              List.of(recordDetail),
              ContractPaymentRecordGetResponse::getContractId,
              ContractPaymentRecordGetResponse::getContractName));
    }
    if (contractPaymentPlan != null) {
      recordDetail.setPaymentPlanName(contractPaymentPlan.getName());
      optionMap.put(
          BusinessModuleField.CONTRACT_PAYMENT_RECORD_PLAN.getBusinessKey(),
          moduleFormService.getBusinessFieldOption(
              List.of(recordDetail),
              ContractPaymentRecordGetResponse::getPaymentPlanId,
              ContractPaymentRecordGetResponse::getPaymentPlanName));
    }
    recordDetail.setModuleFields(fvs);
    recordDetail.setOptionMap(optionMap);
    recordDetail.setAttachmentMap(
        moduleFormService.getAttachmentMap(recordFormConf, recordDetail.getModuleFields()));

    if (recordDetail.getOwner() != null) {
      UserDeptDTO userDeptDTO =
          baseService.getUserDeptMapByUserId(recordDetail.getOwner(), currentOrg);
      if (userDeptDTO != null) {
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.CONTRACT_PAYMENT_RECORD_READ, rolePermissions);
  }
    ...
    try {
      ...
      CustomFieldCheckEventListener eventListener =
          new CustomFieldCheckEventListener(
              fields, "contract_payment_record", "contract_payment_record_field", currentOrg);
  ...
  public ImportResponse realImport(MultipartFile file, String currentOrg, String currentUser) {
    try {
      List<BaseField> fields =
          moduleFormService.getAllFields(FormKey.CONTRACT_PAYMENT_RECORD.getKey(), currentOrg);
      Optional<BaseField> serialOptional =
          fields.stream()
              .filter(
                  field ->
                      Strings.CI.equals(
                          field.getInternalKey(),
                          BusinessModuleField.CONTRACT_PAYMENT_RECORD_NO.getKey()))
              .findAny();
      CustomImportAfterDoConsumer<ContractPaymentRecord, BaseResourceSubField> afterDo =
          (records, recordFields, recordFieldBlobs) -> {
            List<LogDTO> logs = new ArrayList<>();
            records.forEach(
                record -> {
                  if (serialOptional.isPresent()) {
                    List<String> serialNumberRules =
                        ((SerialNumberField) serialOptional.get()).getSerialNumberRules();
                    record.setNo(
                        serialNumGenerator.generateByRules(
                            serialNumberRules,
                            currentOrg,
                  ...
                  logs.add(
                      new LogDTO(
                          ...
                          currentUser,
                          LogType.ADD,
                          LogModule.CONTRACT_PAYMENT_RECORD,
                          record.getName()));
                });
            contractPaymentRecordMapper.batchInsert(records);
            contractPaymentRecordFieldMapper.batchInsert(
                recordFields.stream()
                    .map(field -> BeanUtils.copyBean(new ContractPaymentRecordField(), field))
                    .toList());
            contractPaymentRecordFieldBlobMapper.batchInsert(
                recordFieldBlobs.stream()
                    .map(field -> BeanUtils.copyBean(new ContractPaymentRecordFieldBlob(), field))
                    .toList());
            // record logs
            logService.batchAdd(logs);
          };
      CustomFieldImportEventListener<ContractPaymentRecord> eventListener =
          new CustomFieldImportEventListener<>(
              fields,
              ContractPaymentRecord.class,
              currentOrg,
              currentUser,
   ...
   * @return 汇总结果
   */
  public CustomerPaymentRecordStatisticResponse sumCustomerPaymentAmount(
      String customerId,
      String userId,
      String organizationId,
      DeptDataPermissionDTO deptDataPermission) {
    return extContractPaymentRecordMapper.sumCustomerRecordAmount(
        customerId, userId, organizationId, deptDataPermission);
  }
  ...
  private void updateModuleField(
    ...
    contractPaymentRecordFieldService.saveModuleField(
        contractPaymentRecord, currentOrg, currentUser, moduleFields, true);
  }
   ...
   * @return 列表扩展数据
   */
  public List<ContractPaymentRecordResponse> buildListExtra(
      List<ContractPaymentRecordResponse> list, String currentOrg) {
    if (CollectionUtils.isEmpty(list)) {
      return list;
    }
    List<String> recordIds =
        list.stream().map(ContractPaymentRecordResponse::getId).collect(Collectors.toList());
    List<String> refPlanIds =
        list.stream().map(ContractPaymentRecordResponse::getPaymentPlanId).toList();
    List<ContractPaymentPlan> contractPaymentPlans =
        contractPaymentPlanMapper.selectByIds(refPlanIds);
    Map<String, String> paymentPlanMap =
        contractPaymentPlans.stream()
            .collect(Collectors.toMap(ContractPaymentPlan::getId, ContractPaymentPlan::getName));
    Map<String, List<BaseModuleFieldValue>> resourceFieldMap =
        contractPaymentRecordFieldService.getResourceFieldMap(recordIds, true);
    ...
    List<String> ownerIds =
        list.stream().map(ContractPaymentRecordResponse::getOwner).distinct().toList();
    List<String> createUserIds =
        list.stream().map(ContractPaymentRecordResponse::getCreateUser).distinct().toList();
    List<String> updateUserIds =
        list.stream().map(ContractPaymentRecordResponse::getUpdateUser).distinct().toList();
    List<String> userIds =
        Stream.of(ownerIds, createUserIds, updateUserIds)
            .flatMap(Collection::stream)
            .distinct()
            .toList();
    Map<String, String> userNameMap = baseService.getUserNameMap(userIds);
    Map<String, UserDeptDTO> userDeptMap =
        baseService.getUserDeptMapByUserIds(ownerIds, currentOrg);
    list.forEach(
        item -> {
          item.setModuleFields(resolvefieldValueMap.get(item.getId()));
          item.setCreateUserName(
              baseService.getAndCheckOptionName(userNameMap.get(item.getCreateUser())));
          item.setUpdateUserName(
              baseService.getAndCheckOptionName(userNameMap.get(item.getUpdateUser())));
  ...
  private Map<String, List<OptionDTO>> buildOptionMap(
    ...
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(
            formConfig,
            moduleFormService.getBaseModuleFieldValues(
                list, ContractPaymentRecordResponse::getModuleFields));
    ...
    optionMap.put(
        ...
        moduleFormService.getBusinessFieldOption(
            list,
            ContractPaymentRecordResponse::getOwner,
            ContractPaymentRecordResponse::getOwnerName));
    optionMap.put(
        BusinessModuleField.CONTRACT_PAYMENT_RECORD_CONTRACT.getBusinessKey(),
        moduleFormService.getBusinessFieldOption(
            list,
            ContractPaymentRecordResponse::getContractId,
            ContractPaymentRecordResponse::getContractName));
    optionMap.put(
        BusinessModuleField.CONTRACT_PAYMENT_RECORD_PLAN.getBusinessKey(),
        moduleFormService.getBusinessFieldOption(
            list,
            ContractPaymentRecordResponse::getPaymentPlanId,
            ContractPaymentRecordResponse::getPaymentPlanName));
   ...
   */
  private void checkContractPaymentAmount(
      String contractId, BigDecimal payAmount, String excludeRecordId) {
    ...
    List<ContractPaymentRecord> contractPaymentRecords =
        contractPaymentRecordMapper.selectListByLambda(recordLambdaQueryWrapper);
    BigDecimal alreadyPay =
        contractPaymentRecords.stream()
    ...
    if ((alreadyPay.add(payAmount)).compareTo(contract.getAmount()) > 0) {
      throw new GenericException(
          Translator.getWithArgs(
              "record.amount.exceed", contract.getAmount().subtract(alreadyPay)));
    }
  }
