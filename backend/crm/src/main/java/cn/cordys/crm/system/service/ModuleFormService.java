...
public class ModuleFormService {
  ...
  @Resource private BaseMapper<ModuleForm> moduleFormMapper;
  @Resource private BaseMapper<ModuleFormBlob> moduleFormBlobMapper;
  @Resource private BaseMapper<ModuleField> moduleFieldMapper;
  @Resource private BaseMapper<ModuleFieldBlob> moduleFieldBlobMapper;
  @Resource private ExtModuleFieldMapper extModuleFieldMapper;
  @Resource private UserExtendService userExtendService;
  @Resource private DepartmentService departmentService;
  @Resource private BaseMapper<Attachment> attachmentMapper;
  @Resource private SerialNumGenerator serialNumGenerator;
  @Lazy @Resource private FieldSourceServiceProvider fieldSourceServiceProvider;
   ...
   */
  @OperationLog(
      module = LogModule.SYSTEM_MODULE,
      type = LogType.UPDATE,
      resourceId = "{#saveParam.formKey}")
  public ModuleFormConfigDTO save(
      ModuleFormSaveRequest saveParam, String currentUserId, String currentOrgId) {
    // 处理表单
    LambdaQueryWrapper<ModuleForm> queryWrapper = new LambdaQueryWrapper<>();
    queryWrapper
        .eq(ModuleForm::getFormKey, saveParam.getFormKey())
    ...
    OperationLogContext.setContext(
        LogContextInfo.builder()
            .resourceName(
                Translator.get(saveParam.getFormKey()) + Translator.get("module.form.setting"))
  ...
  private void resetSerial(
      List<ModuleField> oldFields, List<BaseField> saveFields, String formKey, String orgId) {
    Optional<ModuleField> serialOld =
        oldFields.stream()
            .filter(f -> Strings.CS.equals(f.getType(), FieldType.SERIAL_NUMBER.name()))
            .findAny();
    Optional<BaseField> serialNew = saveFields.stream().filter(BaseField::isSerialNumber).findAny();
    if (serialOld.isEmpty()) {
      return;
    }
    ModuleFieldBlob oldFieldBlob =
        moduleFieldBlobMapper.selectByPrimaryKey(serialOld.get().getId());
    List<String> oldRules =
        JSON.parseObject(oldFieldBlob.getProp(), SerialNumberField.class).getSerialNumberRules();
  ...
  private void saveFields(List<BaseField> saveFields, String saveFormId, String currentUserId) {
    ...
    List<String> showFields =
        saveFields.stream()
            .filter(
                f ->
                    f instanceof DatasourceField sourceField
                        && CollectionUtils.isNotEmpty(sourceField.getShowFields()))
            .flatMap(sf -> ((DatasourceField) sf).getShowFields().stream())
            .distinct()
            .toList();
    List<BaseField> refFields =
        saveFields.stream()
            .filter(f -> showFields.contains(f.getId()))
    ...
    saveFields.forEach(
        field -> {
          ...
          if (field instanceof DatasourceField sourceField
              && CollectionUtils.isNotEmpty(sourceField.getShowFields())) {
            List<BaseField> sourceRefFields =
                refFields.stream()
                    .filter(f -> sourceField.getShowFields().contains(f.getId()))
                    .toList();
    ...
    if (linkProp != null && !linkProp.isEmpty()) {
      linkProp.forEach(
          (key, value) ->
              value.forEach(
                  scenario ->
                      parseLinkFieldMap.put(
                          key + "-" + scenario.getKey(), scenario.getLinkFields())));
    }
  ...
  public List<BaseField> getAllFields(String formId) {
    ...
    if (CollectionUtils.isNotEmpty(fields)) {
      ...
      Map<String, String> fieldBlobMap =
          fieldBlobs.stream()
              .collect(Collectors.toMap(ModuleFieldBlob::getId, ModuleFieldBlob::getProp));
      fields.forEach(
          field -> {
            BaseField baseField =
                JSON.parseObject(fieldBlobMap.get(field.getId()), BaseField.class);
   ...
   * @return 字段选项集合
   */
  public Map<String, List<OptionDTO>> getOptionMap(
      ModuleFormConfigDTO formConfig, List<BaseModuleFieldValue> allDataFields) {
    ...
    allFields.forEach(
        field -> {
          if (Strings.CS.equalsAny(field.getType(), FieldType.RADIO.name())
              && field instanceof RadioField radioField) {
            optionMap.put(
                getOptionKey(showFields, field), optionPropToDto(radioField.getOptions()));
          }
          if (Strings.CS.equalsAny(field.getType(), FieldType.CHECKBOX.name())
              && field instanceof CheckBoxField checkBoxField) {
            optionMap.put(
                getOptionKey(showFields, field), optionPropToDto(checkBoxField.getOptions()));
          }
          if (Strings.CS.equalsAny(
              field.getType(), FieldType.SELECT.name(), FieldType.SELECT_MULTIPLE.name())) {
            if (field instanceof HasOption optionField) {
              optionMap.put(
                  getOptionKey(showFields, field), optionPropToDto(optionField.getOptions()));
            }
          }
          if (Strings.CS.equalsAny(
              field.getType(),
          ...
          if (Strings.CS.equalsAny(
              field.getType(), FieldType.MEMBER.name(), FieldType.MEMBER_MULTIPLE.name())) {
            idTypeMap.put(getOptionKey(showFields, field), FieldType.MEMBER.name());
          }
          if (Strings.CS.equalsAny(
              field.getType(), FieldType.DEPARTMENT.name(), FieldType.DEPARTMENT_MULTIPLE.name())) {
          ...
          .forEach(
              fv -> {
                ...
                if (v instanceof List) {
                  typeIdsMap
                      .get(fv.getFieldId())
                      .addAll(JSON.parseArray(JSON.toJSONString(v), String.class));
                } else {
    ...
    typeIdsMap.forEach(
        (fieldId, ids) -> {
          if (CollectionUtils.isNotEmpty(ids)
              && TYPE_SOURCE_MAP.containsKey(idTypeMap.get(fieldId))) {
            List<OptionDTO> options =
                extModuleFieldMapper.getSourceOptionsByIds(
                    TYPE_SOURCE_MAP.get(idTypeMap.get(fieldId)), ids);
  ...
  public List<BaseField> flattenFormAllFields(ModuleFormConfigDTO formConfig) {
    ...
    formConfig.getFields().stream()
        ...
        .map(f -> ((SubField) f).getSubFields())
        .forEach(toFlattenFields::addAll);
  ...
  public List<BaseModuleFieldValue> flattenSubFieldValues(
      ModuleFormConfigDTO formConfig, List<BaseModuleFieldValue> allFieldValues) {
    // 类型为子表的字段
    List<BaseField> subFields =
        formConfig.getFields().stream().filter(f -> f instanceof SubField).toList();
    ...
    allFieldValues.stream()
        .filter(fv -> subIdSet.contains(fv.getFieldId()))
        .forEach(
            fv -> {
              ...
              Map<String, List<String>> subFieldIdValueMap =
                  new HashMap<>(subRowList.getFirst().size());
              subRowList.forEach(
                  subRow ->
                      subRow.forEach(
                          (k, v) -> {
  ...
  public Map<String, List<Attachment>> getAttachmentMap(
      ModuleFormConfigDTO formConfig, List<BaseModuleFieldValue> allDataFields) {
    List<String> attachmentFieldIds =
        formConfig.getFields().stream()
    ...
    allDataFields.stream()
        .filter(
            field ->
                attachmentFieldIds.contains(field.getFieldId()) && field.getFieldValue() != null)
        .forEach(
            field -> {
    ...
    List<String> attachmentIds =
        fieldAttachmentIds.values().stream().flatMap(List::stream).distinct().toList();
    ...
    List<User> users =
        userExtendService.getUserOptionByIds(ListUtils.union(createUserIds, updateUserIds));
    ...
    Map<String, Attachment> attachmentMap =
        attachments.stream().collect(Collectors.toMap(Attachment::getId, Function.identity()));
   ...
   * @param formConfig 表单配置
   */
  public void processBusinessFieldValues(
      Object resource, List<BaseModuleFieldValue> fieldValues, ModuleFormConfigDTO formConfig) {
    List<BaseField> subFields =
        formConfig.getFields().stream()
            .filter(f -> f instanceof SubField && StringUtils.isNotEmpty(f.getBusinessKey()))
            .toList();

    Map<String, String> subFieldBusinessMap =
        subFields.stream().collect(Collectors.toMap(BaseField::getId, BaseField::getBusinessKey));
    List<BaseModuleFieldValue> businessFieldValues =
        fieldValues.stream()
            .filter(fv -> subFieldBusinessMap.containsKey(fv.getFieldId()))
            .toList();

    businessFieldValues.forEach(
        bfv -> {
          String businessKey = subFieldBusinessMap.get(bfv.getFieldId());
          Field field =
              ReflectionUtils.findField(
                  resource.getClass(), f -> Strings.CS.equals(f.getName(), businessKey));
          if (field == null) {
    ...
    Field moduleFields =
        ReflectionUtils.findField(
            resource.getClass(), f -> Strings.CS.equals(f.getName(), "moduleFields"));
  ...
  public void setFieldRefOption(BaseField field) {
    ...
    if (StringUtils.isEmpty(of.getOptionSource())
        || Strings.CS.equals(of.getOptionSource(), OPTION_DEFAULT_SOURCE)) {
      ...
      if (fieldBlob != null) {
        ...
        if (refField instanceof HasOption refOption) {
          of.setOptions(
              CollectionUtils.isNotEmpty(refOption.getOptions())
                  ? refOption.getOptions()
                  : refOption.getCustomOptions());
        }
      }
    }
  }
  ...
  public void setFieldBusinessParam(BaseField field) {
    Set<String> businessTitleKeySet =
        Arrays.stream(BusinessTitleConstants.values())
    ...
    // 获取特殊的业务字段
    Map<String, BusinessModuleField> businessModuleFieldMap =
        Arrays.stream(BusinessModuleField.values())
  ...
  public void reloadPropOfSubRefFields(BaseField field) {
    if (field instanceof SubField subField) {
      ...
      Map<String, String> reloadFieldMap =
          reloadFieldBlobs.stream()
              .collect(Collectors.toMap(ModuleFieldBlob::getId, ModuleFieldBlob::getProp));
      List<BaseField> subFields = getSubFieldsBySourceType(FieldSourceType.PRICE.name());
      Map<String, BaseField> subFieldMap =
          subFields.stream()
              .collect(Collectors.toMap(BaseField::getId, Function.identity(), (p, n) -> p));
      ...
      while (it.hasNext()) {
        ...
        if (!reloadFieldMap.containsKey(oldField.getId())
            && !subFieldMap.containsKey(oldField.getId())) {
          continue;
        }
        BaseField refField;
        if (reloadFieldMap.containsKey(oldField.getId())) {
    ...
    fields.forEach(
        field -> {
          ...
          if (field instanceof DatasourceField sourceField
            ...
            Map<String, BaseField> reloadFieldMap =
                reloadFieldBlobs.stream()
                    .collect(
                        Collectors.toMap(
                            ModuleFieldBlob::getId,
            ...
            if (sourceField.getRefFields() == null) {
              ...
              refFieldIds =
                  sourceField.getRefFields().stream()
                      .map(BaseField::getId)
                      .collect(Collectors.toSet());
            }
                ...
                .forEach(
                    oldRefField -> {
                      if (!reloadFieldMap.containsKey(oldRefField.getId())
                          && !subFieldMap.containsKey(oldRefField.getId())) {
                        return;
                      }
                      BaseField refField;
                      if (reloadFieldMap.containsKey(oldRefField.getId())) {
  ...
  private List<BaseField> getSubFieldsBySourceType(String sourceType) {
    ...
    if (Strings.CS.equals(sourceType, FieldSourceType.QUOTATION.name())) {
      fieldWrapper.eq(
          ModuleField::getInternalKey, BusinessModuleField.QUOTATION_PRODUCT_TABLE.getKey());
    } else if (Strings.CS.equals(sourceType, FieldSourceType.PRICE.name())) {
      fieldWrapper.eq(
          ModuleField::getInternalKey, BusinessModuleField.PRICE_PRODUCT_TABLE.getKey());
    } else if (Strings.CS.equals(sourceType, FieldSourceType.CONTRACT.name())) {
      fieldWrapper.eq(
          ModuleField::getInternalKey, BusinessModuleField.CONTRACT_PRODUCT_TABLE.getKey());
    } else {
      return new ArrayList<>();
    }
  ...
  public void initExtFieldsByVer(String version) {
    try {
      ...
      Map<String, List<Map<String, Object>>> fieldMap =
          JSON.parseObject(fieldResource.getInputStream(), Map.class);
      fieldMap.forEach(
          (formKey, formFields) -> {
            boolean existExtVerField =
                formFields.stream()
                    .anyMatch(
                        f ->
                            f.containsKey(UPGRADE_EXT_FIELD)
                                && Strings.CS.equals(version, f.get(UPGRADE_EXT_FIELD).toString()));
            if (!existExtVerField) {
              return;
            }
            ModuleForm example = new ModuleForm();
            ...
            List<Map<String, Object>> extFields =
                formFields.stream()
                    .filter(
                        f ->
                            f.containsKey(UPGRADE_EXT_FIELD)
            ...
            extFields.forEach(
                initField -> {
                  ModuleField field =
                      supplyFieldInfo(
                          initField, form.getId(), pos.getAndIncrement(), new HashMap<>(2));
              ...
              key -> {
                ...
                initFields.forEach(
                    initField -> {
                      ...
                      ModuleField field =
                          supplyFieldInfo(
                              initField, formId, pos.getAndIncrement(), controlKeyPreMap);
                      initField.put("id", field.getId());
                      fields.add(field);
                      if (initField.containsKey(CONTROL_RULES_KEY)) {
                        List<ControlRuleProp> controlRules =
                            JSON.parseArray(
                                JSON.toJSONString(initField.get(CONTROL_RULES_KEY)),
                                ControlRuleProp.class);
                        controlRules.forEach(
                            controlRule -> {
                      ...
                      if (initField.containsKey(SUB_FIELDS)) {
                        List<BaseField> subFields =
                            JSON.parseArray(
                                JSON.toJSONString(initField.get(SUB_FIELDS)), BaseField.class);
  ...
  private void handleShowFieldsInit(Map<String, Object> initField, List<ModuleField> initFields) {
    if (initField.containsKey(SHOW_FIELD_KEY)) {
      ...
      List<ModuleField> showFields =
          moduleFieldMapper.selectListByLambda(
              new LambdaQueryWrapper<ModuleField>().in(ModuleField::getInternalKey, showFieldKeys));
      ...
      if (CollectionUtils.isNotEmpty(showFieldKeys)) {
        Set<String> internalKeys =
            showFields.stream().map(ModuleField::getInternalKey).collect(Collectors.toSet());

        // 添加表单字段
        List<String> showFieldResult =
            new ArrayList<>(showFields.stream().map(ModuleField::getId).toList());
        // 添加表单中没有的系统字段
        List<String> systemFieldKeys =
            showFieldKeys.stream().filter(fieldKey -> !internalKeys.contains(fieldKey)).toList();
   ...
   * @return 字段
   */
  private ModuleField supplyFieldInfo(
      Map<String, Object> fieldMap, String formId, Long pos, Map<String, String> controlKeyPreMap) {
    ModuleField field = new ModuleField();
    field.setId(
        controlKeyPreMap.containsKey(field.getInternalKey())
            ? controlKeyPreMap.get(field.getInternalKey())
            : IDGenerator.nextStr());
  ...
  private void handleInitialOption(BaseField field) {
    if (field instanceof MemberField memberField) {
      memberField.setInitialOptions(
          userExtendService.getUserOptionById(memberField.getDefaultValue()));
    }
    if (field instanceof MemberMultipleField memberMultipleField) {
      memberMultipleField.setInitialOptions(
          userExtendService.getUserOptionByIds(memberMultipleField.getDefaultValue()));
    }
    if (field instanceof DepartmentField departmentField) {
      departmentField.setInitialOptions(
          departmentService.getDepartmentOptionsById(departmentField.getDefaultValue()));
    }
    if (field instanceof DepartmentMultipleField departmentMultipleField) {
      departmentMultipleField.setInitialOptions(
          departmentService.getDepartmentOptionsByIds(departmentMultipleField.getDefaultValue()));
    }
  }
   ...
   * @param <T> 实体
   */
  public <T> List<OptionDTO> getBusinessFieldOption(
      List<T> list, Function<T, String> getOptionIdFunc, Function<T, String> getOptionNameFunc) {
    return list.stream()
        .map(
            item -> {
              OptionDTO optionDTO = new OptionDTO();
  ...
  public <T> List<OptionDTO> getBusinessFieldOption(
      T item, Function<T, String> getOptionIdFunc, Function<T, String> getOptionNameFunc) {
    return getBusinessFieldOption(List.of(item), getOptionIdFunc, getOptionNameFunc);
  }

  public <T> List<BaseModuleFieldValue> getBaseModuleFieldValues(
      List<T> list, Function<T, List<BaseModuleFieldValue>> getModuleFieldFunc) {
    // 处理自定义字段选项数据
    return list.stream()
   ...
   * @return 自定义导入表头集合
   */
  public List<List<String>> getAllExportHeads(
      List<ExportHeadDTO> exportHeads, String formKey, String currentOrg) {
    ...
    exportHeads.forEach(
        exportHead -> {
          ...
          if (field instanceof SubField subField
            ...
            Map<String, String> subFieldMap =
                subField.getSubFields().stream()
                    .collect(
                        Collectors.toMap(
                            f ->
                                StringUtils.isNotBlank(f.getResourceFieldId())
                                    ? f.getId()
                                    : f.idOrBusinessKey(),
                            BaseField::getName,
   ...
   * @return 导出字段ID集合
   */
  public List<String> getExportMergeHeads(
      String formKey, String currentOrg, List<ExportHeadDTO> exportHeads) {
    Map<String, BaseField> fieldConfigMap = getFieldConfigMapByName(formKey, currentOrg);
  ...
  public boolean supportSubHead(List<BaseField> headFields) {
    ...
    return headFields.stream()
        .anyMatch(
            field ->
                field instanceof SubField subField
                    && CollectionUtils.isNotEmpty(subField.getSubFields()));
  }
  ...
  private void preCheckForFieldSave(ModuleFormSaveRequest saveParam) {
    boolean businessDeleted =
        BusinessModuleField.isBusinessDeleted(saveParam.getFormKey(), saveParam.getFields());
    if (businessDeleted) {
    ...
    Optional<BaseField> repeatOptional =
        saveParam.getFields().stream()
            .filter(
                field -> {
                  if (field instanceof HasOption optionField) {
    ...
    if (repeatOptional.isPresent()) {
      ...
      throw new GenericException(
          Translator.getWithArgs("module.form.fields.option.repeat", field.getName()));
    }
  }
  ...
  public boolean hasFieldUniqueCheck(String formKey, String orgId, String internalKey) {
    ...
    Optional<BaseField> internalField =
        allFields.stream()
   ...
   * @return 填充结果
   */
  public <T, S> FormLinkFill<T> fillFormLinkValue(
      T target,
      S source,
      ModuleFormConfigDTO targetFormConfig,
      String orgId,
    ...
    // 未找到联动场景，直接返回目标对象
    Optional<LinkScenario> scenarioOptional =
        linkProp.get(sourceFormKey).stream()
            .filter(scenario -> Strings.CS.equals(scenario.getKey(), scenarioKey))
            .findFirst();
    ...
    // 目标表单字段
    Map<String, BaseField> targetFieldMap =
        targetFields.stream().collect(Collectors.toMap(BaseField::getId, Function.identity()));
    // 来源表单字段
    Map<String, BaseField> sourceFieldMap =
        sourceFields.stream().collect(Collectors.toMap(BaseField::getId, Function.identity()));
    // 填充数据
    Class<?> targetClass = target.getClass();
    Class<?> sourceClass = source.getClass();
    List<BaseModuleFieldValue> targetFieldVals = new ArrayList<>();
    @SuppressWarnings("unchecked")
    List<BaseModuleFieldValue> sourceFieldVals =
        (List<BaseModuleFieldValue>) sourceClass.getMethod("getModuleFields").invoke(source);
  ...
  private Object text2Val(List<OptionProp> options, Object text) {
    ...
    Map<String, String> optionMap =
        options.stream().collect(Collectors.toMap(OptionProp::getLabel, OptionProp::getValue));
    if (text instanceof List) {
      return ((List<?>) text)
          .stream().map(v -> optionMap.get(v.toString())).filter(Objects::nonNull).toList();
    } else {
   ...
   */
  private TransformSourceApplyDTO applySourceValue(
      BaseField sourceField,
      Class<?> sourceClass,
      Object source,
      List<BaseModuleFieldValue> sourceFieldVals)
      throws Exception {
    ...
    if (StringUtils.isNotEmpty(sourceField.getBusinessKey())) {
      // 业务字段取值
      tmpVal =
          sourceClass.getMethod(capitalizeGetParam(sourceField.getBusinessKey())).invoke(source);
    } else {
      // 自定义字段取值
      Optional<BaseModuleFieldValue> find =
          sourceFieldVals.stream()
              .filter(fieldVal -> Strings.CS.equals(sourceField.getId(), fieldVal.getFieldId()))
              .findFirst();
   ...
   */
  private void putTargetFieldVal(
      BaseField targetField,
      TransformSourceApplyDTO putVal,
      Class<?> targetClass,
      Object target,
      List<BaseModuleFieldValue> targetFieldVals)
      throws Exception {
    ...
    if (StringUtils.isNotEmpty(targetField.getBusinessKey())) {
      ...
      Method method =
          targetClass.getMethod(
              capitalizeSetParam(targetField.getBusinessKey()),
              targetClass.getDeclaredField(targetField.getBusinessKey()).getType());
  ...
  private Object displayOfType(BaseField sourceField, Object actualVal) {
    ...
    AbstractModuleFieldResolver customFieldResolver =
        ModuleFieldResolverFactory.getResolver(sourceField.getType());
    // 将数据库中的字符串值,转换为对应的对象值
    return customFieldResolver.transformToValue(
        sourceField,
        actualVal instanceof List ? JSON.toJSONString(actualVal) : actualVal.toString());
  }
  ...
  public Object resolveTargetPutVal(BaseField targetField, TransformSourceApplyDTO sourceVal) {
    ...
    if (targetField instanceof InputField || targetField instanceof TextAreaField) {
      ...
      return displayVal instanceof List
          ? String.join(",", (List<String>) displayVal)
          : displayVal.toString();
    }
    if (targetField instanceof InputMultipleField) {
      ...
      return new ArrayList<>(
          textMultipleResolver.getCorrectFormatInput(
              sourceVal.getDisplayVal() instanceof List
                  ? (List<String>) sourceVal.getDisplayVal()
                  : List.of(sourceVal.getDisplayVal().toString().split(","))));
    }
  ...
  public void processOldLinkData() {
    ...
    wrapper.in(
        ModuleForm::getFormKey, List.of(FormKey.CUSTOMER.getKey(), FormKey.OPPORTUNITY.getKey()));
    List<ModuleForm> forms = moduleFormMapper.selectListByLambda(wrapper);
    Map<String, String> formKeyMap =
        forms.stream().collect(Collectors.toMap(ModuleForm::getId, ModuleForm::getFormKey));
    List<ModuleFormBlob> moduleFormBlobs =
        moduleFormBlobMapper.selectByIds(formKeyMap.keySet().stream().toList());
    for (ModuleFormBlob formBlob : moduleFormBlobs) {
      Map<String, Object> propMap = JSON.parseMap(formBlob.getProp());
      Object linkProp = propMap.get("linkProp");
      ...
      if (linkProp == null) {
        if (Strings.CS.equals(formKey, FormKey.CUSTOMER.getKey())) {
          ...
          dataMap.put(
              ...
              List.of(
                  LinkScenario.builder()
                      .key(LinkScenarioKey.CLUE_TO_OPPORTUNITY.name())
                      .linkFields(new ArrayList<>())
                      .build()));
          dataMap.put(
              ...
              List.of(
                  LinkScenario.builder()
                      .key(LinkScenarioKey.CUSTOMER_TO_OPPORTUNITY.name())
                      .linkFields(new ArrayList<>())
                      .build()));
        }
      } else {
        ...
        for (Map.Entry<String, Object> entry : linkPropMap.entrySet()) {
          ...
          List<LinkField> fieldList =
              fields.stream()
                  ...
                  .toList();
          String scenarioKey =
              (Strings.CS.equals(formKey, FormKey.CUSTOMER.getKey())
                      && Strings.CS.equals(entry.getKey(), FormKey.CLUE.getKey())
                  ? LinkScenarioKey.CLUE_TO_CUSTOMER.name()
                  : (Strings.CS.equals(formKey, FormKey.OPPORTUNITY.getKey())
                          && Strings.CS.equals(entry.getKey(), FormKey.CLUE.getKey())
                      ? LinkScenarioKey.CLUE_TO_OPPORTUNITY.name()
                      : LinkScenarioKey.CUSTOMER_TO_OPPORTUNITY.name()));
          LinkScenario linkScenario =
              LinkScenario.builder().key(scenarioKey).linkFields(fieldList).build();
  ...
  public void initFormScenarioProp() {
    ...
    dataMap.put(
        ...
        List.of(
            LinkScenario.builder()
                .key(LinkScenarioKey.CLUE_TO_RECORD.name())
                .linkFields(new ArrayList<>())
                .build()));
    dataMap.put(
        FormKey.CUSTOMER.getKey(),
        List.of(
            LinkScenario.builder()
                .key(LinkScenarioKey.CUSTOMER_TO_RECORD.name())
                .linkFields(new ArrayList<>())
                .build()));
    dataMap.put(
        ...
        List.of(
            LinkScenario.builder()
                .key(LinkScenarioKey.OPPORTUNITY_TO_RECORD.name())
                .linkFields(new ArrayList<>())
                .build()));
    if (linkProp != null) {
      Map<String, Object> linkPropMap = (Map<String, Object>) linkProp;
      List<Map<String, Object>> fields =
          (List<Map<String, Object>>) linkPropMap.get(FormKey.FOLLOW_PLAN.getKey());
      List<LinkField> fieldList =
          fields.stream()
              .map(
                  field -> {
                    LinkField linkField =
                        JSON.parseObject(JSON.toJSONString(field), LinkField.class);
      ...
      dataMap.put(
          ...
          List.of(
              LinkScenario.builder()
                  .key(LinkScenarioKey.PLAN_TO_RECORD.name())
                  .linkFields(fieldList)
                  .build()));
    } else {
      dataMap.put(
          FormKey.FOLLOW_PLAN.getKey(),
  ...
  public List<SimpleField> getMcpFields(String formKey, String organizationId) {
    ...
    return businessFormConfig.getFields().stream()
        .filter(BaseField::canImport)
        .map(
            field -> {
   ...
   * @return 处理后的自定义字段值
   */
  public <T extends BaseResourceField, V extends BaseResourceField>
      List<BaseModuleFieldValue> resolveSnapshotFields(
          List<BaseModuleFieldValue> fieldValues,
          ModuleFormConfigDTO formConfig,
          BaseResourceFieldService<T, V> baseResourceFieldService,
          String resourceId) {
    // 1. 扁平化所有字段
    ...
    // 2. 处理数据源引用字段
    List<BaseModuleFieldValue> resolveRefFvs =
        resolveRefFields(resolveFvs, flattenFields, baseResourceFieldService);
   ...
   */
  @SuppressWarnings("unchecked")
  private <F extends BaseResourceField, B extends BaseResourceField>
      List<BaseModuleFieldValue> resolveRefFields(
          List<BaseModuleFieldValue> resolveFvs,
          List<BaseField> flattenFields,
          BaseResourceFieldService<F, B> baseResourceFieldService) {
    // 1. 数据源字段配置映射 && 字段配置映射
    final Map<String, BaseField> sourceConfigMap =
        flattenFields.stream()
            .filter(
                f ->
                    f instanceof DatasourceField ds
                        && CollectionUtils.isNotEmpty(ds.getShowFields()))
            .collect(Collectors.toMap(BaseField::idOrBusinessKey, f -> f, (f1, f2) -> f1));

    final Map<String, BaseField> fieldMap =
        flattenFields.stream().collect(Collectors.toMap(BaseField::getId, f -> f, (f1, f2) -> f1));
        ...
        fv -> {
          ...
          if (sourceConfigMap.containsKey(fieldId)) {
            ...
            final FieldSourceType sourceType =
                FieldSourceType.valueOf(sourceField.getDataSourceType());
            final Object detail =
                fieldSourceServiceProvider.safeGetById(sourceType, fv.getFieldValue().toString());
            if (detail == null) {
              return;
            }
                ...
                .forEach(
                    refId -> {
                      ...
                      if (showFieldConf != null) {
                        final Object val =
                            baseResourceFieldService.getFieldValueOfDetailMap(
                                showFieldConf, detailMap);
          ...
          if (fieldMap.containsKey(fieldId) && fieldMap.get(fieldId).isSubField()) {
            final List<Map<String, Object>> subValues =
                (List<Map<String, Object>>) fv.getFieldValue();
            subValues.forEach(
                sfv -> {
                  ...
                  sfv.forEach(
                      (k, v) -> {
                        ...
                        final DatasourceField sourceField =
                            (DatasourceField) sourceConfigMap.get(k);
                        final FieldSourceType sourceType =
                            FieldSourceType.valueOf(sourceField.getDataSourceType());
                        final Object detail =
                            fieldSourceServiceProvider.safeGetById(sourceType, v.toString());
                        if (detail == null) {
                          return;
                        }

                        final Map<String, Object> detailMap =
                            JSON.MAPPER.convertValue(detail, Map.class);
                        ...
                        sourceField
                            ...
                            .forEach(
                                id -> {
                                  ...
                                  if (StringUtils.isNotEmpty(showFieldConf.getSubTableFieldId())
                                      && sfv.containsKey(PRICE_SUB_ROW_KEY)) {
                                    Object matchVal =
                                        baseResourceFieldService.matchSubFieldValueOfDetailMap(
                                            showFieldConf.idOrBusinessKey(),
                                            detailMap,
                                            BusinessModuleField.PRICE_PRODUCT_TABLE
                                                .getBusinessKey(),
   ...
   * @return 处理后的字段值
   */
  private List<BaseModuleFieldValue> resolveSubKeyToId(
      List<BaseModuleFieldValue> fvs, List<BaseField> flattenFields) {
    // 获取替换后的子表格值
    final Map<String, BaseField> subFieldConfigMap =
        flattenFields.stream()
            .filter(f -> f instanceof SubField)
            .collect(Collectors.toMap(BaseField::idOrBusinessKey, f -> f));

    final List<BaseModuleFieldValue> subFieldValues =
        fvs.stream()
            .filter(
                fv ->
                    subFieldConfigMap.containsKey(fv.getFieldId())
                        && subFieldConfigMap.get(fv.getFieldId()).isSubField())
            .map(
                fv ->
                    new BaseModuleFieldValue(
                        subFieldConfigMap.get(fv.getFieldId()).getId(), fv.getFieldValue()))
            .toList();
  ...
  private Map<String, BaseField> getFieldConfigMapByName(String formKey, String orgId) {
    ...
    return allFields.stream()
        .collect(Collectors.toMap(BaseField::getName, Function.identity(), (p, n) -> p));
  }
}
