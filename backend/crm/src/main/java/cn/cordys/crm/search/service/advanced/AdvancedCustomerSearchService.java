...
@Service
public class AdvancedCustomerSearchService
    extends BaseSearchService<CustomerPageRequest, AdvancedCustomerResponse> {
  ...
  @Override
  public PagerWithOption<List<AdvancedCustomerResponse>> startSearch(
      CustomerPageRequest request, String orgId, String userId) {
    ...
    List<AdvancedCustomerResponse> customers =
        extCustomerMapper.checkRepeatCustomer(request, orgId, userId);
    if (CollectionUtils.isEmpty(customers)) {
      return PageUtils.setPageInfoWithOption(page, List.of(), Map.of());
    }
    List<AdvancedCustomerResponse> buildList =
        buildCustomerList(orgId, userId, isAdmin, customers, enabledModules);
    Map<String, List<OptionDTO>> optionMap = buildCustomerOptionMap(orgId, customers, buildList);
    return PageUtils.setPageInfoWithOption(page, buildList, optionMap);
  }

  private List<AdvancedCustomerResponse> buildCustomerList(
    ...
    // 获取商机和线索的重复数量映射
    Map<String, String> opportunityCounts =
        getOpportunityCounts(permissions, isAdmin, customers, enabledModules);
    ...
    List<String> customerIds =
        customers.stream().map(AdvancedCustomerResponse::getId).collect(Collectors.toList());

    Map<String, List<BaseModuleFieldValue>> caseCustomFiledMap =
        customerFieldService.getResourceFieldMap(customerIds, true);

    List<String> ownerIds =
        customers.stream().map(AdvancedCustomerResponse::getOwner).distinct().toList();

    List<String> followerIds =
        customers.stream().map(AdvancedCustomerResponse::getFollower).distinct().toList();
    List<String> createUserIds =
        customers.stream().map(AdvancedCustomerResponse::getCreateUser).distinct().toList();
    List<String> updateUserIds =
        customers.stream().map(AdvancedCustomerResponse::getUpdateUser).distinct().toList();
    List<String> userIds =
    ...
    Map<String, UserDeptDTO> userDeptMap =
        baseService.getUserDeptMapByUserIds(ownerIds, organizationId);

    // 获取负责人默认公海信息
    Map<String, CustomerPool> ownersDefaultPoolMap =
        customerPoolService.getOwnersDefaultPoolMap(ownerIds, organizationId);
    List<String> poolIds =
    ...
    DictConfigDTO dictConf =
        dictService.getDictConf(DictModule.CUSTOMER_POOL_RS.name(), organizationId);
    ...
    return customers.stream()
        .peek(
            customer -> {
              ...
              String followerName =
                  baseService.getAndCheckOptionName(userNameMap.get(customer.getFollower()));
              customer.setFollowerName(followerName);
              String createUserName =
                  baseService.getAndCheckOptionName(userNameMap.get(customer.getCreateUser()));
              customer.setCreateUserName(createUserName);
              String updateUserName =
                  baseService.getAndCheckOptionName(userNameMap.get(customer.getUpdateUser()));
              customer.setUpdateUserName(updateUserName);
              customer.setOwnerName(userNameMap.get(customer.getOwner()));
              String reasonName =
                  baseService.getAndCheckOptionName(dictMap.get(customer.getReasonId()));
              customer.setReasonName(reasonName);
            })
        .toList();
  }

  public Map<String, List<OptionDTO>> buildCustomerOptionMap(
      String orgId, List<AdvancedCustomerResponse> list, List<AdvancedCustomerResponse> buildList) {
    // 处理自定义字段选项数据
    ModuleFormConfigDTO customerFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.CUSTOMER.getKey(), orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(list, AdvancedCustomerResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList, AdvancedCustomerResponse::getOwner, AdvancedCustomerResponse::getOwnerName);
   ...
   */
  private Map<String, String> getOpportunityCounts(
    ...
    // 获取客户ID列表并查询商机数量
    List<String> customerIds = customers.stream().map(AdvancedCustomerResponse::getId).toList();
   ...
   */
  private Map<String, String> getClueCounts(
      List<String> permissions,
      boolean isAdmin,
      List<AdvancedCustomerResponse> customers,
      List<String> enabledModules) {
    // 没有线索读取权限且不是管理员返回空map
    if (!permissions.contains(PermissionConstants.CLUE_MANAGEMENT_READ)
        && !permissions.contains(PermissionConstants.CLUE_MANAGEMENT_POOL_READ)
        && !isAdmin) {
      return Collections.emptyMap();
    }
    // 线索模块未启用返回空map
    ...
    // 获取客户名称列表并查询线索数量
    List<String> customerNames = customers.stream().map(AdvancedCustomerResponse::getName).toList();
  ...
  public List<AdvancedClueResponse> getRepeatClueDetail(
      RepeatCustomerDetailPageRequest request, String organizationId) {
    List<AdvancedClueResponse> repeatClueList =
        extClueMapper.getRepeatClueList(request.getName(), organizationId);
  ...
  public List<OpportunityRepeatResponse> getRepeatOpportunityDetail(
    ...
    List<String> productIds =
        responses.stream().flatMap(response -> response.getProducts().stream()).distinct().toList();
