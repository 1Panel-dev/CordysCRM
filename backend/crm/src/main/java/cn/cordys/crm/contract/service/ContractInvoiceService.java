...
public class ContractInvoiceService {
  ...
  @Resource private ContractInvoiceFieldService invoiceFieldService;
  @Resource private BaseMapper<ContractInvoice> invoiceMapper;
  @Resource private BaseService baseService;
  @Resource private ModuleFormService moduleFormService;
  @Resource private BaseMapper<ContractInvoiceSnapshot> snapshotBaseMapper;
  @Resource private ExtContractInvoiceMapper extContractInvoiceMapper;
  @Resource private BaseMapper<ContractInvoice> contractInvoiceMapper;
  ...
  @Resource private BusinessTitleService businessTitleService;
   ...
   * @return
   */
  public PagerWithOption<List<ContractInvoiceListResponse>> list(
      ContractInvoicePageRequest request,
      String userId,
      String orgId,
      DeptDataPermissionDTO deptDataPermission) {
    Page<Object> page = PageHelper.startPage(request.getCurrent(), request.getPageSize());
    List<ContractInvoiceListResponse> list =
        extContractInvoiceMapper.list(request, orgId, userId, deptDataPermission);
  ...
  private Map<String, List<OptionDTO>> buildOptionMap(
      List<ContractInvoiceListResponse> list,
      List<ContractInvoiceListResponse> buildList,
      String orgId) {
    ...
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(
            list, ContractInvoiceListResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList,
            ContractInvoiceListResponse::getOwner,
            ContractInvoiceListResponse::getOwnerName);
    optionMap.put(BusinessModuleField.INVOICE_OWNER.getBusinessKey(), ownerFieldOption);
    // 补充工商抬头选项
    List<OptionDTO> businessTitleFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList,
            ContractInvoiceListResponse::getBusinessTitleId,
            ContractInvoiceListResponse::getBusinessTitleName);
    optionMap.put(
        BusinessModuleField.INVOICE_BUSINESS_TITLE_ID.getBusinessKey(), businessTitleFieldOption);
    return optionMap;
  }
  ...
  public ContractInvoice add(ContractInvoiceAddRequest request, String operatorId, String orgId) {
    ...
    BigDecimal contractInvoiceValidAmount =
        extContractInvoiceMapper.calculateContractInvoiceValidAmount(
            request.getContractId(), operatorId, orgId, null);
    if (request.getAmount() != null
        && contract != null
        && request.getAmount().compareTo(contract.getAmount().subtract(contractInvoiceValidAmount))
            > 0) {
      // 校验发票金额
    ...
    List<BaseModuleFieldValue> resolveFieldValues =
        moduleFormService.resolveSnapshotFields(
            moduleFields, moduleFormConfigDTO, invoiceFieldService, invoice.getId());
   ...
   */
  @OperationLog(
      module = LogModule.CONTRACT_INVOICE,
      type = LogType.UPDATE,
  ...
  public ContractInvoice update(ContractInvoiceUpdateRequest request, String userId, String orgId) {
    ...
    String contractId =
        request.getContractId() == null
            ? originContractInvoice.getContractId()
            : request.getContractId();
    Contract contract = contractMapper.selectByPrimaryKey(contractId);
    BigDecimal contractInvoiceValidAmount =
        extContractInvoiceMapper.calculateContractInvoiceValidAmount(
            request.getContractId(), userId, orgId, request.getId());
    if (request.getAmount() != null
        && contract != null
        && request.getAmount().compareTo(contract.getAmount().subtract(contractInvoiceValidAmount))
            > 0) {
    ...
    dataScopeService.checkDataPermission(
        userId,
        orgId,
        originContractInvoice.getOwner(),
        PermissionConstants.CONTRACT_INVOICE_UPDATE);
    ModuleFormConfigDTO saveModuleFormConfigDTO =
        JSON.parseObject(JSON.toJSONString(moduleFormConfigDTO), ModuleFormConfigDTO.class);
    Optional.of(originContractInvoice)
        .ifPresentOrElse(
            item -> {
              List<BaseModuleFieldValue> originFields =
                  invoiceFieldService.getModuleFieldValuesByResourceId(request.getId());
              ContractInvoice invoice = BeanUtils.copyBean(new ContractInvoice(), request);
              invoice.setUpdateTime(System.currentTimeMillis());
              ...
              List<ContractInvoiceSnapshot> invoiceSnapshots =
              ...
              if (CollectionUtils.isNotEmpty(invoiceSnapshots)) {
                ...
                if (first != null) {
                  ContractInvoiceGetResponse response =
                      JSON.parseObject(first.getInvoiceValue(), ContractInvoiceGetResponse.class);
              ...
              invoice.setOrganizationId(orgId);
              ContractInvoiceGetResponse response =
                  get(invoice, resolveFieldValues, moduleFormConfigDTO);
              saveSnapshot(invoice, saveModuleFormConfigDTO, response);

              // 处理日志上下文
              baseService.handleUpdateLog(
                  originContractInvoice,
                  invoice,
                  originFields,
                  moduleFields,
                  request.getId(),
  ...
  public void delete(String id, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, invoice.getOwner(), PermissionConstants.CONTRACT_INVOICE_DELETE);
  ...
  public ContractInvoiceGetResponse getWithDataPermissionCheck(
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, getResponse.getOwner(), PermissionConstants.CONTRACT_INVOICE_READ);
    return getResponse;
  }

  public ContractInvoiceGetResponse getSnapshotWithDataPermissionCheck(
      String id, String userId, String orgId) {
  ...
  private ContractInvoiceGetResponse get(
      ContractInvoice contractInvoice,
      List<BaseModuleFieldValue> contractInvoiceFields,
      ModuleFormConfigDTO contractInvoiceFormConfig) {
    ContractInvoiceGetResponse contractInvoiceGetResponse =
        BeanUtils.copyBean(new ContractInvoiceGetResponse(), contractInvoice);
    contractInvoiceGetResponse =
        baseService.setCreateUpdateOwnerUserName(contractInvoiceGetResponse);

    // 获取模块字段
    moduleFormService.processBusinessFieldValues(
        contractInvoiceGetResponse, contractInvoiceFields, contractInvoiceFormConfig);
    contractInvoiceFields =
        invoiceFieldService
            .setBusinessRefFieldValue(
                List.of(contractInvoiceGetResponse),
                moduleFormService.getFlattenFormFields(
                    FormKey.INVOICE.getKey(), contractInvoice.getOrganizationId()),
                new HashMap<>(Map.of(contractInvoice.getId(), contractInvoiceFields)))
            .get(contractInvoice.getId());

    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(contractInvoiceFormConfig, contractInvoiceFields);

    // 补充负责人选项
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            contractInvoiceGetResponse,
            ContractInvoiceGetResponse::getOwner,
            ContractInvoiceGetResponse::getOwnerName);
    optionMap.put(BusinessModuleField.INVOICE_OWNER.getBusinessKey(), ownerFieldOption);
    ...
    if (contract != null) {
      ...
      optionMap.put(
          BusinessModuleField.INVOICE_CONTRACT_ID.getBusinessKey(),
          Collections.singletonList(new OptionDTO(contract.getId(), contract.getName())));
    }

    BusinessTitle businessTitle =
        businessTitleService.selectById(contractInvoiceGetResponse.getBusinessTitleId());
    if (businessTitle != null) {
      contractInvoiceGetResponse.setBusinessTitleName(businessTitle.getName());
      optionMap.put(
          BusinessModuleField.INVOICE_BUSINESS_TITLE_ID.getBusinessKey(),
          Collections.singletonList(new OptionDTO(businessTitle.getId(), businessTitle.getName())));
    }
    ...
    if (contractInvoiceGetResponse.getOwner() != null) {
      UserDeptDTO userDeptDTO =
          baseService.getUserDeptMapByUserId(
              contractInvoiceGetResponse.getOwner(), contractInvoice.getOrganizationId());
      if (userDeptDTO != null) {
    ...
    // 附件信息
    contractInvoiceGetResponse.setAttachmentMap(
        moduleFormService.getAttachmentMap(contractInvoiceFormConfig, contractInvoiceFields));
  ...
  public ContractInvoiceGetResponse get(String id) {
    ...
    // 获取模块字段
    ModuleFormConfigDTO contractInvoiceFormConfig =
        getFormConfig(contractInvoice.getOrganizationId());
    List<BaseModuleFieldValue> contractInvoiceFields =
        invoiceFieldService.getModuleFieldValuesByResourceId(id);
  ...
  public ContractInvoiceGetResponse getSnapshot(String id) {
    LambdaQueryWrapper<ContractInvoiceSnapshot> wrapper = new LambdaQueryWrapper<>();
    wrapper.eq(ContractInvoiceSnapshot::getInvoiceId, id);
    ContractInvoiceSnapshot snapshot =
  ...
  public List<ContractInvoiceListResponse> buildList(
      List<ContractInvoiceListResponse> list, String orgId) {
    if (CollectionUtils.isEmpty(list)) {
      return list;
    }

    List<String> invoiceIds =
        list.stream().map(ContractInvoiceListResponse::getId).collect(Collectors.toList());

    List<String> businessTitleIds =
        list.stream()
            .map(ContractInvoiceListResponse::getBusinessTitleId)
            .collect(Collectors.toList());

    Map<String, List<BaseModuleFieldValue>> invoiceFiledMap =
        invoiceFieldService.getResourceFieldMap(invoiceIds, true);
    Map<String, List<BaseModuleFieldValue>> resolvefieldValueMap =
        invoiceFieldService.setBusinessRefFieldValue(
            list,
            moduleFormService.getFlattenFormFields(FormKey.INVOICE.getKey(), orgId),
            invoiceFiledMap);
    Map<String, String> businessTitleNameMap =
        businessTitleService.selectByIds(businessTitleIds).stream()
            .collect(Collectors.toMap(BusinessTitle::getId, BusinessTitle::getName));

    List<String> ownerIds =
        list.stream().map(ContractInvoiceListResponse::getOwner).distinct().toList();
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String orgId) {
    ...
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.CONTRACT_INVOICE_READ, rolePermissions);
  }
  ...
  public void batchDelete(List<String> ids, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, owners, PermissionConstants.CONTRACT_INVOICE_DELETE);
    ...
    //                        NotificationConstants.Event.CUSTOMER_DELETED, invoice.getName(),
    // userId,
  ...
  private List<String> getOwners(List<ContractInvoice> invoices) {
    return invoices.stream().map(ContractInvoice::getOwner).distinct().toList();
  }
   ...
   * @param userId
   */
  public void approvalContractInvoice(
      ContractInvoiceApprovalRequest request, String userId, String orgId) {
    ContractInvoice invoice = invoiceMapper.selectByPrimaryKey(request.getId());
    if (invoice == null) {
      throw new GenericException(Translator.get("invoice.not.exist"));
    }
    dataScopeService.checkDataPermission(
        userId, orgId, invoice.getOwner(), PermissionConstants.CONTRACT_INVOICE_APPROVAL);
    ...
    LogDTO logDTO =
        getApprovalLogDTO(
            orgId, request.getId(), userId, invoice.getName(), state, request.getApprovalStatus());
  ...
  public String revoke(String id, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, invoice.getOwner(), PermissionConstants.CONTRACT_INVOICE_UPDATE);

    String originApprovalStatus = invoice.getApprovalStatus();
    if (!Strings.CI.equals(invoice.getCreateUser(), userId)
        || !Strings.CI.equals(invoice.getApprovalStatus(), ApprovalState.APPROVING.toString())) {
  ...
  private LogDTO getApprovalLogDTO(
      ...
      String newState) {
    LogDTO logDTO =
        new LogDTO(orgId, id, userId, LogType.APPROVAL, LogModule.CONTRACT_INVOICE, response);
    Map<String, String> oldMap = new HashMap<>();
    oldMap.put(
        "approvalStatus", Translator.get("contract.approval_status." + originStatus.toLowerCase()));
  ...
  public BigDecimal calculateCustomerInvoiceAmount(
      String customerId, String userId, String organizationId) {
    return extContractInvoiceMapper.calculateCustomerInvoiceAmount(
        customerId, userId, organizationId);
  }

  public BigDecimal calculateContractInvoiceAmount(
      String contractId, String userId, String organizationId) {
    return extContractInvoiceMapper.calculateContractInvoiceAmount(
        contractId, userId, organizationId);
  }

  public ModuleFormConfigDTO getBusinessFormConfig(String organizationId) {
    ModuleFormConfigDTO businessFormConfig =
    ...
    for (BaseField field : businessFormConfig.getFields()) {
      if (businessTitleKeySet.contains(field.getId())
          || Strings.CS.equals(
              field.getId(), BusinessModuleField.CONTRACT_PRODUCT_SUM_AMOUNT.getKey())) {
        // 特殊表单，设置可见
