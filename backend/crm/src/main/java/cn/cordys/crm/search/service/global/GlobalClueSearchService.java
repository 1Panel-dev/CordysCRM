...
@Service
public class GlobalClueSearchService
    extends BaseSearchService<BasePageRequest, GlobalClueResponse> {
  ...
  @Override
  public Pager<List<GlobalClueResponse>> startSearchNoOption(
    ...
    if (CollectionUtils.isNotEmpty(userSearchConfigs)) {
      List<UserSearchConfig> clueSearchConfigs =
          userSearchConfigs.stream()
              .filter(
                  t -> Strings.CI.equals(t.getModuleType(), SearchModuleEnum.SEARCH_ADVANCED_CLUE))
              .toList();
      ...
      for (UserSearchConfig userSearchConfig : clueSearchConfigs) {
        ...
        if (StringUtils.isBlank(userSearchConfig.getBusinessKey())) {
          fieldIdSet.add(userSearchConfig.getFieldId());
            ...
            && !Strings.CI.equals(
                userSearchConfig.getBusinessKey(), BusinessModuleField.CLUE_OWNER.getBusinessKey())
            && !Strings.CI.equals(
    ...
    List<SearchFieldMaskConfig> searchFieldMaskConfigs =
        getSearchFieldMaskConfigs(orgId, SearchModuleEnum.SEARCH_ADVANCED_CLUE);
    List<GlobalClueResponse> buildList =
        buildListData(
            globalClueResponses, orgId, userId, searchFieldMaskConfigs, fieldIdSet, internalKeyMap);
    return PageUtils.setPageInfo(page, buildList);
  }

  public List<GlobalClueResponse> buildListData(
      List<GlobalClueResponse> list,
      String orgId,
      String userId,
      List<SearchFieldMaskConfig> searchFieldMaskConfigs,
    ...
    List<String> clueIds =
        list.stream().map(GlobalClueResponse::getId).collect(Collectors.toList());
    Map<String, List<BaseModuleFieldValue>> clueFiledMap =
        clueFieldService.getResourceFieldMap(clueIds, true);

    List<String> ownerIds = list.stream().map(GlobalClueResponse::getOwner).distinct().toList();
