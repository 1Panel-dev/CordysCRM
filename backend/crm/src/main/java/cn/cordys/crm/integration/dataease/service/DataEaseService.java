...
public class DataEaseService {
  ...
  public DeAuthDTO getEmbeddedDeToken(String account, DeThirdConfigRequest thirdConfig) {
    // 生成token
    String token = generateJwtToken(thirdConfig.getAgentId(), thirdConfig.getAppSecret(), account);

    return DeAuthDTO.builder().token(token).url(thirdConfig.getRedirectUrl()).build();
  }
  ...
  public DeThirdConfigRequest getDeConfig(String organizationId) {
    var config =
        Optional.ofNullable(
            ...
            .orElseThrow(() -> new GenericException("未找到 DataEase 相关配置"));

    var detail =
        extOrganizationConfigDetailMapper
            .getOrgConfigDetailByType(
                config.getId(), null, List.of(ThirdDetailType.DE_BOARD.name()))
            .stream()
            .findFirst()
            .orElseThrow(() -> new GenericException("未找到 DataEase 配置详情"));

    var baseDTO =
        Optional.ofNullable(
                JSON.parseObject(new String(detail.getContent()), ThirdConfigBaseDTO.class))
            .orElseThrow(() -> new GenericException("DataEase 配置内容解析失败"));

    return JSON.MAPPER.convertValue(baseDTO.getConfig(), DeThirdConfigRequest.class);
  }
