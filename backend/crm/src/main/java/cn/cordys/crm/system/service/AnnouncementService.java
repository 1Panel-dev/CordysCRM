...
public class AnnouncementService {
  ...
  @Resource private BaseMapper<Announcement> announcementMapper;
  @Resource private BaseMapper<Notification> notificationBaseMapper;
  @Resource private ExtAnnouncementMapper extAnnouncementMapper;
  @Resource private ExtDepartmentMapper extDepartmentMapper;
  @Resource private ExtNotificationMapper notificationMapper;
  @Resource private ExtUserMapper userMapper;
  ...
  public void add(AnnouncementRequest request, String userId) {
    ...
    announcement.setReceiveType(
        JSON.toJSONString(announcementReceiveTypeDTO).getBytes(StandardCharsets.UTF_8));
    ...
    if (request.getStartTime() <= announcement.getCreateTime()
        && request.getEndTime() >= announcement.getCreateTime()) {
      convertNotification(userId, announcement, userIds);
      announcement.setNotice(true);
    }
    announcementMapper.insert(announcement);
    AnnouncementLogDTO announcementLogDTO =
        buildNewLogDTO(request, announcement, announcementReceiveTypeDTO);
    // 添加日志上下文
  ...
  private AnnouncementLogDTO buildNewLogDTO(
      AnnouncementRequest request,
      Announcement announcement,
      AnnouncementReceiveTypeDTO announcementReceiveTypeDTO) {
    ...
    if (request.getStartTime() != null) {
      announcementLogDTO.setStartTime(
          new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(request.getStartTime()));
    }
    if (request.getEndTime() != null) {
      announcementLogDTO.setEndTime(
          new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(request.getEndTime()));
    }
    return announcementLogDTO;
  }

  private AnnouncementLogDTO getOldLogDTO(Announcement originalAnnouncement) {
    AnnouncementReceiveTypeDTO oldReceiveTypeDTO =
        JSON.parseObject(
            new String(originalAnnouncement.getReceiveType()), AnnouncementReceiveTypeDTO.class);
    ...
    oldLogDTO.setStartTime(
        new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(originalAnnouncement.getStartTime()));
    oldLogDTO.setEndTime(
        new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(originalAnnouncement.getEndTime()));
    return oldLogDTO;
  }
  ...
  public void update(AnnouncementRequest request, String userId) {
    ...
    if (originalAnnouncement.getNotice()) {
      ...
      if (request.getStartTime() <= System.currentTimeMillis()
          && request.getEndTime() >= System.currentTimeMillis()) {
    ...
    AnnouncementLogDTO announcementLogDTO =
        buildNewLogDTO(request, announcement, announcementReceiveTypeDTO);
    String resourceName =
        Optional.ofNullable(announcement.getSubject()).orElse(originalAnnouncement.getSubject());
    OperationLogContext.setContext(
        LogContextInfo.builder()
  ...
  private void deleteAnnouncementKeyById(String announcementId, String organizationId) {
    ...
    for (NotificationDTO notificationDTO : notificationDTOS) {
      ...
      stringRedisTemplate
          .opsForZSet()
          .remove(USER_ANNOUNCE_PREFIX + notificationDTO.getReceiver(), notificationDTOId);
    ...
    SubListUtils.dealForSubList(
        ...
        (subUserIds) -> {
          ...
          for (String subUserId : subUserIds) {
            ...
            // 根据公告的有效时间判断是否需要发送通知
            if (announcement.getStartTime() <= System.currentTimeMillis()
                && announcement.getEndTime() >= System.currentTimeMillis()) {
              ...
              stringRedisTemplate
                  .opsForZSet()
                  .add(USER_ANNOUNCE_PREFIX + subUserId, id, System.currentTimeMillis());
              ...
              noticeRedisMessage.setNoticeType(
                  NotificationConstants.Type.ANNOUNCEMENT_NOTICE.toString());
              messagePublisher.publish(
                  TopicConstants.SSE_TOPIC, JSON.toJSONString(noticeRedisMessage));
  ...
  @Transactional(rollbackFor = Exception.class)
  @OperationLog(
      module = LogModule.SYSTEM_MESSAGE_ANNOUNCEMENT,
      type = LogType.DELETE,
      resourceId = "{#id}")
  ...
  public List<AnnouncementDTO> page(AnnouncementPageRequest request) {
    ...
    if (CollectionUtils.isNotEmpty(announcementDTOS)) {
      for (AnnouncementDTO announcementDTO : announcementDTOS) {
        announcementDTO.setContentText(
            new String(announcementDTO.getContent(), StandardCharsets.UTF_8));
  ...
  private void setReceiverNameOption(AnnouncementDTO announcementDTO) {
    AnnouncementReceiveTypeDTO announcementReceiveTypeDTO =
        JSON.parseObject(
            new String(announcementDTO.getReceiveType()), AnnouncementReceiveTypeDTO.class);
    if (CollectionUtils.isNotEmpty(announcementReceiveTypeDTO.getDeptIds())) {
      List<OptionDTO> idNameByIds =
          extDepartmentMapper.getIdNameByIds(announcementReceiveTypeDTO.getDeptIds());
    ...
    if (CollectionUtils.isNotEmpty(announcementReceiveTypeDTO.getUserIds())) {
      List<OptionDTO> idNameByIds =
          userMapper.selectUserOptionByIds(announcementReceiveTypeDTO.getUserIds());
      List<OptionScopeDTO> optionScopeDTOList = new ArrayList<>();
      for (OptionDTO idNameById : idNameByIds) {
        OptionScopeDTO optionScopeDTO = new OptionScopeDTO();
        BeanUtils.copyBean(optionScopeDTO, idNameById);
  ...
  public List<String> getReceiverName(AnnouncementReceiveTypeDTO announcementReceiveTypeDTO) {
    ...
    if (CollectionUtils.isNotEmpty(announcementReceiveTypeDTO.getDeptIds())) {
      List<String> names =
          extDepartmentMapper.getNameByIds(announcementReceiveTypeDTO.getDeptIds());
