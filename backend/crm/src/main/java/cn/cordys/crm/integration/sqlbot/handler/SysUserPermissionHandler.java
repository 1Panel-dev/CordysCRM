...
public class SysUserPermissionHandler extends OrgTablePermissionHandler {
  ...
  protected String getSysUserTableSql(
      TableHandleParam tableHandleParam, List<FieldDTO> sqlBotFields) {
    return MessageFormat.format(
        SYS_USER_SQL_TEMPLATE, getSelectSystemFileSql(sqlBotFields), tableHandleParam.getOrgId());
  }
  ...
  protected String getSelectSystemFileSql(FieldDTO sqlBotField) {
    ...
    if (Strings.CS.equals(fieldName, "employee_type")) {
      List<OptionDTO> options =
          List.of(
              new OptionDTO("formal", "正式"),
              new OptionDTO("internship", "实习"),
              new OptionDTO("outsourcing", "外包"));
