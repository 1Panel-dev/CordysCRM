...
public class CluePoolService {
  ...
  @Resource private BaseMapper<CluePoolHiddenField> cluePoolHiddenFieldMapper;
  @Resource private BaseMapper<CluePoolPickRule> cluePoolPickRuleMapper;
  @Resource private BaseMapper<CluePoolRecycleRule> cluePoolRecycleRuleMapper;
  @Resource private ExtCluePoolMapper extCluePoolMapper;
  ...
  public List<CluePoolDTO> page(BasePageRequest request, String organizationId) {
    ...
    List<String> poolIds = pools.stream().map(CluePoolDTO::getId).toList();
    ...
    List<CluePoolRecycleRule> recycleRules =
        cluePoolRecycleRuleMapper.selectListByLambda(recycleRuleWrapper);
    ...
    Map<String, List<CluePoolHiddenField>> hiddenFieldMap =
        getCluePoolHiddenFieldsByPoolIds(poolIds).stream()
            .collect(Collectors.groupingBy(CluePoolHiddenField::getPoolId));

    List<BaseField> fields =
        moduleFormCacheService
            .getBusinessFormConfig(FormKey.CLUE.getKey(), organizationId)
            .getFields();
  ...
  public List<CluePoolFieldConfigDTO> getCluePoolFieldConfigs(
        ...
        .map(
            field -> {
              ...
              hiddenFieldDTO.setEditable(
                  !Strings.CS.equals(
                      field.getInternalKey(), BusinessModuleField.CLUE_NAME.getKey()));
  ...
  private void batchInsertCluePoolHiddenFields(String poolId, Set<String> fieldIds) {
    ...
    List<CluePoolHiddenField> cluePoolHiddenFields =
        fieldIds.stream()
            .map(
                fieldId -> {
  ...
  public boolean checkNoPick(String id) {
    ...
    wrapper.eq(Clue::getPoolId, id).eq(Clue::getInSharedPool, true);
   ...
   * @return 默认线索池
   */
  public Map<String, CluePool> getOwnersDefaultPoolMap(
  ...
  public Integer calcReservedDay(
      CluePool pool, CluePoolRecycleRule recycleRule, Long collectionTime, Long createTime) {
  ...
  private boolean matchTime(RuleConditionDTO condition, Clue clue) {
    if (Strings.CS.equals(condition.getColumn(), RecycleConditionColumnKey.STORAGE_TIME)) {
      if (condition.getScope().contains(RecycleConditionScopeKey.CREATED)) {
        ...
        return RecycleConditionUtils.matchTime(condition, clue.getCreateTime())
            || RecycleConditionUtils.matchTime(condition, clue.getCollectionTime());
      }
    } else {
