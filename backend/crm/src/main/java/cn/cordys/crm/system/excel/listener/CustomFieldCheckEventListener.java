...
public class CustomFieldCheckEventListener extends AnalysisEventListener<Map<Integer, String>> {
  ...
  /** 错误, 成功信息 */
  @Getter protected Integer success = 0;

  @Getter protected List<ExcelErrData> errList = new ArrayList<>();
  ...
  /** 错误行号集合 */
  @Getter protected List<Integer> errRows = new ArrayList<>();
  ...
  public CustomFieldCheckEventListener(
      List<BaseField> fields, String sourceTable, String fieldTable, String currentOrg) {
              ...
              field -> {
                if (businessFieldMap.containsKey(field.getInternalKey())
                  ...
                  BusinessModuleField businessModuleField =
                      businessFieldMap.get(field.getInternalKey());
                  String fieldName = businessModuleField.getBusinessKey();
                  List<String> valList =
                      commonMapper.getCheckValList(sourceTable, fieldName, currentOrg);
                  uniqueCheckSet.put(
                      field.getName(), new HashSet<>(valList.stream().distinct().toList()));
                } else {
                  List<String> valList =
                      commonMapper.getCheckFieldValList(
                          sourceTable, fieldTable, field.getId(), currentOrg);
  ...
  private void validateRowData(Integer rowIndex, Map<Integer, String> rowData) {
    ...
    headMap.forEach(
        (k, v) -> {
          ...
          if (fieldLenLimit.containsKey(v)
              && StringUtils.isNotEmpty(rowData.get(k))
              && rowData.get(k).length() > fieldLenLimit.get(v)) {
            errText
                .append(v)
                .append(Translator.getWithArgs("over.length", fieldLenLimit.get(v)))
                .append(";");
          }
        });
    if (StringUtils.isNotEmpty(errText)) {
      ExcelErrData excelErrData =
          new ExcelErrData(
              rowIndex,
  ...
  private void setCheckLimit(BaseField field) {
    ...
    if (Strings.CS.equalsAny(
        ...
        FieldType.INPUT_NUMBER.name(),
        FieldType.DATE_TIME.name(),
        FieldType.MEMBER.name(),
        FieldType.DEPARTMENT.name(),
        FieldType.DATA_SOURCE.name(),
        FieldType.RADIO.name(),
        FieldType.SELECT.name(),
        FieldType.PHONE.name(),
        FieldType.LOCATION.name(),
        FieldType.INDUSTRY.name())) {
