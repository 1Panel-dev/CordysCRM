...
public class SseService {
  ...
  public Flux<String> addClient(String userId, String clientId) {
    ...
    Map<String, ClientSinkWrapper> inner =
        userClients.computeIfAbsent(
            userId, k -> Collections.synchronizedMap(new LinkedHashMap<>()));
  ...
  public void sendToClient(String userId, String clientId, Object data) {
    Optional.ofNullable(userClients.get(userId))
        ...
        .ifPresent(wrapper -> wrapper.emit(JSON.toJSONString(data)));
  }
  ...
  private SseMessageDTO buildMessage(String userId, String sendType) {
      ...
      if (CollectionUtils.isNotEmpty(sysValues)) {
        ...
        if (CollectionUtils.isNotEmpty(modules)) {
          List<NotificationDTO> list =
              extNotificationMapper.selectLastList(
                  userId, OrganizationContext.getOrganizationId(), modules);
    ...
    dto.setRead(
        Boolean.parseBoolean(stringRedisTemplate.opsForValue().get(USER_READ_PREFIX + userId)));
    return dto;
  }
