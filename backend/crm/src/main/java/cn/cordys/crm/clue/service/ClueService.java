...
public class ClueService {
  ...
  @Resource private BaseMapper<Clue> clueMapper;
  ...
  @Resource private BaseChartService baseChartService;
  @Resource private CluePoolService cluePoolService;
  @Resource private BaseMapper<CluePoolRecycleRule> recycleRuleMapper;
  ...
  @Resource private CommonNoticeSendService commonNoticeSendService;
  ...
  @Resource private BaseMapper<CustomerContact> customerContactMapper;
  @Resource private PermissionCache permissionCache;
  @Resource private ExtProductMapper extProductMapper;
  @Resource private ProductService productService;
  @Resource private PoolCustomerService poolCustomerService;
  @Resource private CustomerCollaborationService customerCollaborationService;
  @Resource private CustomerContactService customerContactService;
  @Resource private ExtUserMapper extUserMapper;
  @Resource private BaseMapper<ClueField> clueFieldMapper;
  @Resource private BaseMapper<ClueFieldBlob> clueFieldBlobMapper;
  @Resource private BaseMapper<FollowUpRecord> followUpRecordMapper;
  @Resource private BaseMapper<FollowUpRecordField> followUpRecordFieldMapper;
  @Resource private BaseMapper<FollowUpRecordFieldBlob> followUpRecordFieldBlobMapper;
  @Resource private BaseMapper<FollowUpPlan> followUpPlanMapper;
  ...
  public PagerWithOption<List<ClueListResponse>> list(
    ...
    List<ClueListResponse> list =
        extClueMapper.list(request, orgId, userId, deptDataPermission, source);
  ...
  public Map<String, List<OptionDTO>> buildOptionMap(
      String orgId, List<ClueListResponse> list, List<ClueListResponse> buildList) {
    // 处理自定义字段选项数据
    ModuleFormConfigDTO customerFormConfig = getFormConfig(orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(list, ClueListResponse::getModuleFields);
    // 获取选项值对应的 option
    ...
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            buildList, ClueListResponse::getOwner, ClueListResponse::getOwnerName);
  ...
  public List<ClueListResponse> buildListData(List<ClueListResponse> list, String orgId) {
    ...
    List<String> clueIds = list.stream().map(ClueListResponse::getId).collect(Collectors.toList());

    Map<String, List<BaseModuleFieldValue>> caseCustomFiledMap =
        clueFieldService.getResourceFieldMap(clueIds, true);

    List<String> ownerIds = list.stream().map(ClueListResponse::getOwner).distinct().toList();

    List<String> followerIds = list.stream().map(ClueListResponse::getFollower).distinct().toList();
    List<String> createUserIds =
        list.stream().map(ClueListResponse::getCreateUser).distinct().toList();
    List<String> updateUserIds =
        list.stream().map(ClueListResponse::getUpdateUser).distinct().toList();
    List<String> userIds =
    ...
    // 获取负责人线索池信息
    Map<String, CluePool> ownersDefaultPoolMap =
        cluePoolService.getOwnersDefaultPoolMap(ownerIds, orgId);
    List<String> poolIds =
        ownersDefaultPoolMap.values().stream().map(CluePool::getId).distinct().toList();
    ...
    if (CollectionUtils.isEmpty(poolIds)) {
      ...
      List<CluePoolRecycleRule> recycleRules =
          recycleRuleMapper.selectListByLambda(recycleRuleWrapper);
      recycleRuleMap =
          recycleRules.stream()
    ...
    list.forEach(
        clueListResponse -> {
          ...
          // 计算剩余归属天数
          clueListResponse.setReservedDays(
              cluePoolService.calcReservedDay(
                  reservePool,
                  reservePool != null ? recycleRuleMap.get(reservePool.getId()) : null,
                  clueListResponse.getCollectionTime(),
                  clueListResponse.getCreateTime()));
  ...
  public ClueGetResponse getWithDataPermissionCheck(String id, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, getResponse.getOwner(), PermissionConstants.CLUE_MANAGEMENT_READ);
    return getResponse;
  }
  ...
  public ClueGetResponse get(String id) {
    ...
    // 获取选项值对应的 option
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(customerFormConfig, clueFields);
    // 补充负责人选项
    List<OptionDTO> ownerFieldOption =
        moduleFormService.getBusinessFieldOption(
            clueGetResponse, ClueGetResponse::getOwner, ClueGetResponse::getOwnerName);
    optionMap.put(BusinessModuleField.CLUE_OWNER.getBusinessKey(), ownerFieldOption);
    ...
    // 线索池原因
    DictConfigDTO dictConf =
        dictService.getDictConf(DictModule.CLUE_POOL_RS.name(), clue.getOrganizationId());
    List<Dict> dictList = dictConf.getDictList();
    Map<String, String> dictMap =
        dictList.stream().collect(Collectors.toMap(Dict::getId, Dict::getName));
    ...
    if (clueGetResponse.getOwner() != null) {
      // 获取负责人线索池信息
      Map<String, CluePool> ownersDefaultPoolMap =
          cluePoolService.getOwnersDefaultPoolMap(
              List.of(clueGetResponse.getOwner()), clue.getOrganizationId());
      List<String> poolIds =
          ownersDefaultPoolMap.values().stream().map(CluePool::getId).distinct().toList();
      Map<String, CluePoolRecycleRule> recycleRuleMap;
      if (CollectionUtils.isEmpty(poolIds)) {
      ...
      // 计算剩余归属天数
      clueGetResponse.setReservedDays(
          cluePoolService.calcReservedDay(
              reservePool,
              reservePool != null ? recycleRuleMap.get(reservePool.getId()) : null,
              clueGetResponse.getCollectionTime(),
              clueGetResponse.getCreateTime()));

      UserDeptDTO userDeptDTO =
          baseService.getUserDeptMapByUserId(clueGetResponse.getOwner(), clue.getOrganizationId());
      if (userDeptDTO != null) {
    ...
    if (clueGetResponse.getFollower() != null) {
      Map<String, String> userNameMap =
          baseService.getUserNameMap(List.of(clueGetResponse.getFollower()));
    ...
    // 附件信息
    clueGetResponse.setAttachmentMap(
        moduleFormService.getAttachmentMap(customerFormConfig, clueFields));
  ...
  public Clue update(ClueUpdateRequest request, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, originClue.getOwner(), PermissionConstants.CLUE_MANAGEMENT_UPDATE);
    ...
    // 获取模块字段
    List<BaseModuleFieldValue> originCustomerFields =
        clueFieldService.getModuleFieldValuesByResourceId(request.getId());

    if (BooleanUtils.isTrue(request.getAgentInvoke())) {
      clueFieldService.updateModuleFieldByAgent(
          clue, originCustomerFields, request.getModuleFields(), orgId, userId);
    } else {
      // 更新模块字段
    ...
    baseService.handleUpdateLog(
        originClue,
        clue,
        originCustomerFields,
  ...
  private void sendTransferNotice(
      List<Clue> originClues, String toUser, String userId, String orgId) {
    originClues.forEach(
        clue ->
            commonNoticeSendService.sendNotice(
                NotificationConstants.Module.CLUE,
                NotificationConstants.Event.TRANSFER_CLUE,
                clue.getName(),
                userId,
                orgId,
                List.of(toUser),
  ...
  private void updateModuleField(
      Clue clue, List<BaseModuleFieldValue> moduleFields, String orgId, String userId) {
   ...
   * @param orgId 组织ID
   */
  public void transitionCustomer(
      ClueTransitionCustomerRequest request, String userId, String orgId) {
    Customer customer = customerService.add(request, userId, orgId);
    Clue clue = clueMapper.selectByPrimaryKey(request.getClueId());
    dataScopeService.checkDataPermission(
        userId, orgId, clue.getOwner(), PermissionConstants.CUSTOMER_MANAGEMENT_ADD);
    ...
    commonNoticeSendService.sendNotice(
        NotificationConstants.Module.CLUE,
        NotificationConstants.Event.CLUE_CONVERT_CUSTOMER,
        clue.getName(),
        userId,
        orgId,
  ...
  public void delete(String id, String userId, String orgId) {
    Clue clue = clueMapper.selectByPrimaryKey(id);
    dataScopeService.checkDataPermission(
        userId, orgId, clue.getOwner(), PermissionConstants.CLUE_MANAGEMENT_DELETE);
    // 删除客户
    ...
    commonNoticeSendService.sendNotice(
        NotificationConstants.Module.CLUE,
        NotificationConstants.Event.CLUE_DELETED,
        clue.getName(),
        userId,
        orgId,
        List.of(clue.getOwner()),
  ...
  public void batchTransfer(ClueBatchTransferRequest request, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, ownerIds, PermissionConstants.CLUE_MANAGEMENT_UPDATE);
  ...
  public void batchDelete(List<String> ids, String userId, String orgId) {
    ...
    dataScopeService.checkDataPermission(
        userId, orgId, owners, PermissionConstants.CLUE_MANAGEMENT_DELETE);
  ...
  private List<String> getOwners(List<Clue> clues) {
    return clues.stream().map(Clue::getOwner).distinct().toList();
  }
   ...
   */
  public BatchAffectResponse batchToPool(
      BatchPoolReasonRequest request, String currentUser, String orgId) {
    ...
    List<String> ownerIds = getOwners(clues);
    dataScopeService.checkDataPermission(
        currentUser, orgId, ownerIds, PermissionConstants.CLUE_MANAGEMENT_RECYCLE);
    ...
    for (Clue clue : clues) {
      ...
      commonNoticeSendService.sendNotice(
          NotificationConstants.Module.CLUE,
          NotificationConstants.Event.CLUE_MOVED_POOL,
          clue.getName(),
          currentUser,
          orgId,
          List.of(clue.getOwner()),
    ...
    return BatchAffectResponse.builder()
        .success(success)
        .fail(request.getIds().size() - success)
        .build();
  }
  ...
  public ResourceTabEnableDTO getTabEnableConfig(String userId, String organizationId) {
    List<RolePermissionDTO> rolePermissions =
        permissionCache.getRolePermissions(userId, organizationId);
    return PermissionUtils.getTabEnableConfig(
        userId, PermissionConstants.CLUE_MANAGEMENT_READ, rolePermissions);
  }
   ...
   * @param orgId 组织ID
   */
  public void batchTransition(
      BatchReTransitionCustomerRequest request, String currentUser, String orgId) {
  ...
  public void transitionCs(String clueId, Customer transitionCs, String currentUser, String orgId) {
    ...
    // 关联
    TransformCsAssociateDTO transformCsAssociateDTO =
    ...
    // 转移线索的计划&记录
    batchCopyCluePlanAndRecord(
        clue.getId(), transitionCs.getId(), null, transformCsAssociateDTO.getContactId());
    // 刷新客户的最新跟进时间
    if (clue.getFollowTime() != null
        && (transitionCs.getFollowTime() == null
            || transitionCs.getFollowTime() < clue.getFollowTime())) {
  ...
  public String transform(ClueTransformRequest request, String currentUser, String orgId) {
    ...
    boolean uniqueCheck =
        moduleFormService.hasFieldUniqueCheck(
            FormKey.CUSTOMER.getKey(), orgId, BusinessModuleField.CUSTOMER_NAME.getKey());
    ...
    if (clue.getFollowTime() != null
        && (transformCustomer.getFollowTime() == null
            || transformCustomer.getFollowTime() < clue.getFollowTime())) {
      // 刷新同名客户最新跟进时间
      Customer updateCustomer = new Customer();
    ...
    if (request.getOppCreated()) {
      ...
      Opportunity transformOpportunity =
          generateOpportunityByLinkForm(
              clue, transformCsAssociateDTO.getContactId(), transformCustomer, currentUser, orgId);
      // 转移线索的计划&记录
      batchCopyCluePlanAndRecord(
          clue.getId(),
          transformCustomer.getId(),
          transformOpportunity.getId(),
          transformCsAssociateDTO.getContactId());
      paramMap.put("template", Translator.get("message.clue_convert_business_text"));
      paramMap.put("name", clue.getName());
      commonNoticeSendService.sendNotice(
          NotificationConstants.Module.CLUE,
          NotificationConstants.Event.CLUE_CONVERT_BUSINESS,
          paramMap,
          currentUser,
          orgId,
          List.of(clue.getOwner()),
      ...
      // 转移线索的计划&记录
      batchCopyCluePlanAndRecord(
          clue.getId(), transformCustomer.getId(), null, transformCsAssociateDTO.getContactId());
   ...
   */
  public void batchCopyCluePlanAndRecord(
      String clueId, String customerId, String opportunityId, String contactId) {
    // 记录
    LambdaQueryWrapper<FollowUpRecord> recordLambdaQueryWrapper = new LambdaQueryWrapper<>();
    recordLambdaQueryWrapper
        .eq(FollowUpRecord::getClueId, clueId)
        .eq(FollowUpRecord::getType, FollowUpPlanType.CLUE.name());
    List<FollowUpRecord> followUpRecords =
        followUpRecordMapper.selectListByLambda(recordLambdaQueryWrapper);
    if (CollectionUtils.isNotEmpty(followUpRecords)) {
      ...
      List<FollowUpRecordField> followUpRecordFields =
          followUpRecordFieldMapper.selectListByLambda(fieldLambdaQueryWrapper);
      LambdaQueryWrapper<FollowUpRecordFieldBlob> fieldBlobLambdaQueryWrapper =
          new LambdaQueryWrapper<>();
      fieldBlobLambdaQueryWrapper.in(FollowUpRecordFieldBlob::getResourceId, ids);
      List<FollowUpRecordFieldBlob> followUpRecordFieldBlobs =
          followUpRecordFieldBlobMapper.selectListByLambda(fieldBlobLambdaQueryWrapper);
      followUpRecords.forEach(
          record -> {
            String recordId = IDGenerator.nextStr();
            followUpRecordFields.stream()
                .filter(
                    recordField -> Strings.CS.equals(recordField.getResourceId(), record.getId()))
                .forEach(
                    field -> {
                      field.setId(IDGenerator.nextStr());
                      field.setResourceId(recordId);
                    });
            followUpRecordFieldBlobs.stream()
                .filter(
                    recordFieldBlob ->
                        Strings.CS.equals(recordFieldBlob.getResourceId(), record.getId()))
                .forEach(
                    fieldBlob -> {
    ...
    planLambdaQueryWrapper
        .eq(FollowUpPlan::getClueId, clueId)
        .eq(FollowUpPlan::getType, FollowUpPlanType.CLUE.name());
    List<FollowUpPlan> followUpPlans =
        followUpPlanMapper.selectListByLambda(planLambdaQueryWrapper);
    if (CollectionUtils.isNotEmpty(followUpPlans)) {
      ...
      List<FollowUpPlanField> followUpPlanFields =
          followUpPlanFieldMapper.selectListByLambda(fieldLambdaQueryWrapper);
      LambdaQueryWrapper<FollowUpPlanFieldBlob> fieldBlobLambdaQueryWrapper =
          new LambdaQueryWrapper<>();
      fieldBlobLambdaQueryWrapper.in(FollowUpPlanFieldBlob::getResourceId, ids);
      List<FollowUpPlanFieldBlob> followUpPlanFieldBlobs =
          followUpPlanFieldBlobMapper.selectListByLambda(fieldBlobLambdaQueryWrapper);
      followUpPlans.forEach(
          plan -> {
            String planId = IDGenerator.nextStr();
            followUpPlanFields.stream()
                .filter(planField -> Strings.CS.equals(planField.getResourceId(), plan.getId()))
                .forEach(
                    field -> {
                      field.setId(IDGenerator.nextStr());
                      field.setResourceId(planId);
                    });
            followUpPlanFieldBlobs.stream()
                .filter(
                    planFieldBlob -> Strings.CS.equals(planFieldBlob.getResourceId(), plan.getId()))
                .forEach(
                    fieldBlob -> {
                      fieldBlob.setId(IDGenerator.nextStr());
  ...
  public void checkTransformPermission(boolean checkOpportunityPermission) {
    ...
    if (checkOpportunityPermission
        && !PermissionUtils.hasPermission(PermissionConstants.OPPORTUNITY_MANAGEMENT_ADD)) {
  ...
  public Customer selectorCs(List<Customer> customers, String clueOwner) {
    ...
    Optional<Customer> find =
        customers.stream()
            .filter(
                customer ->
                    !customer.getInSharedPool()
                        && Strings.CS.equals(customer.getOwner(), clueOwner))
            .findFirst();
  ...
  public Customer generateCustomerByLinkForm(Clue clue, String currentUser, String orgId) {
    ...
    ModuleFormConfigDTO customerFormConfig =
        moduleFormService.getBusinessFormConfig(FormKey.CUSTOMER.getKey(), orgId);
    FormLinkFill<Customer> customerLinkFillDTO;
    try {
      customerLinkFillDTO =
          moduleFormService.fillFormLinkValue(
              new Customer(),
              get(clue.getId()),
              customerFormConfig,
              orgId,
    ...
    // 部分内置字段未配置联动, 取线索值即可
    addRequest.setName(
        customerLinkFillDTO.getEntity() == null
                || StringUtils.isEmpty(customerLinkFillDTO.getEntity().getName())
            ? clue.getName()
            : customerLinkFillDTO.getEntity().getName());
    addRequest.setOwner(
        customerLinkFillDTO.getEntity() == null
                || StringUtils.isEmpty(customerLinkFillDTO.getEntity().getOwner())
            ? clue.getOwner()
            : customerLinkFillDTO.getEntity().getOwner());
   ...
   * @return 商机
   */
  public Opportunity generateOpportunityByLinkForm(
      Clue clue, String contactId, Customer transformCustomer, String currentUser, String orgId) {
    ModuleFormConfigDTO opportunityFormConfig =
        moduleFormService.getBusinessFormConfig(FormKey.OPPORTUNITY.getKey(), orgId);
    FormLinkFill<Opportunity> opportunityLinkFillDTO;
    try {
      opportunityLinkFillDTO =
          moduleFormService.fillFormLinkValue(
              new Opportunity(),
              get(clue.getId()),
   ...
   * @param orgId 组织ID
   */
  public TransformCsAssociateDTO transformCsAssociate(
      Clue clue, Customer transformCs, String currentUser, String orgId) {
    TransformCsAssociateDTO transformDTO = new TransformCsAssociateDTO();
    // 如果当前线索负责人不是关联客户的负责人，且不是客户协作人, 则添加协作关系
    if (!Strings.CS.equals(transformCs.getOwner(), clue.getOwner())
        && !customerCollaborationService.hasCollaboration(clue.getOwner(), transformCs.getId())) {
      CustomerCollaborationAddRequest collaborationAddRequest =
          new CustomerCollaborationAddRequest();
    ...
    if (StringUtils.isNotEmpty(clue.getContact())) {
      boolean unique =
          customerContactService.checkCustomerContactUnique(
              clue.getContact(), clue.getPhone(), transformCs.getId(), orgId);
      if (unique) {
  ...
  public void downloadImportTpl(HttpServletResponse response, String currentOrg) {
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            response,
            moduleFormService.getCustomImportHeadsNoRef(FormKey.CLUE.getKey(), currentOrg),
            Translator.get("clue.import_tpl.name"),
            Translator.get(SheetKey.DATA),
            Translator.get(SheetKey.COMMENT),
            new CustomTemplateWriteHandler(
                moduleFormService.getAllCustomImportFields(FormKey.CLUE.getKey(), currentOrg)),
  ...
  public ImportResponse realImport(MultipartFile file, String currentOrg, String currentUser) {
    try {
      ...
      CustomImportAfterDoConsumer<Clue, BaseResourceSubField> afterDo =
          (clues, clueFields, clueFieldBlobs) -> {
            List<LogDTO> logs = new ArrayList<>();
            clues.forEach(
                clue -> {
                  clue.setCollectionTime(clue.getCreateTime());
            ...
            clueFieldMapper.batchInsert(
                clueFields.stream()
                    .map(field -> BeanUtils.copyBean(new ClueField(), field))
                    .toList());
            clueFieldBlobMapper.batchInsert(
                clueFieldBlobs.stream()
                    .map(field -> BeanUtils.copyBean(new ClueFieldBlob(), field))
                    .toList());
            // record logs
            logService.batchAdd(logs);
          };
      CustomFieldImportEventListener<Clue> eventListener =
          new CustomFieldImportEventListener<>(
              fields, Clue.class, currentOrg, currentUser, "clue_field", afterDo, 2000, null, null);
  ...
  private ImportResponse checkImportExcel(MultipartFile file, String currentOrg) {
    try {
      ...
      CustomFieldCheckEventListener eventListener =
          new CustomFieldCheckEventListener(fields, "clue", "clue_field", currentOrg);
      FastExcelFactory.read(file.getInputStream(), eventListener)
          .headRowNumber(1)
          .ignoreEmptyRow(true)
          .sheet()
  ...
  public void batchUpdate(ResourceBatchEditRequest request, String userId, String organizationId) {
    ...
    if (Strings.CS.equals(
        field.getBusinessKey(), BusinessModuleField.CLUE_OWNER.getBusinessKey())) {
      // 修改负责人，走批量转移接口
    ...
    if (Strings.CS.equals(
        field.getBusinessKey(), BusinessModuleField.CLUE_PRODUCTS.getBusinessKey())) {
    ...
    clueFieldService.batchUpdate(
        request,
        field,
        originClues,
        Clue.class,
        LogModule.CLUE_INDEX,
        extClueMapper::batchUpdate,
        userId,
        organizationId);
  }

  public List<ChartResult> chart(
      ChartAnalysisRequest request,
      String userId,
      String orgId,
      DeptDataPermissionDTO deptDataPermission) {
    ModuleFormConfigDTO formConfig = getFormConfig(orgId);
    formConfig.getFields().addAll(BaseChartService.getChartBaseFields());
    ChartAnalysisDbRequest chartAnalysisDbRequest =
        ConditionFilterUtils.parseChartAnalysisRequest(request, formConfig);
    ClueChartAnalysisDbRequest clueChartAnalysisDbRequest =
        BeanUtils.copyBean(new ClueChartAnalysisDbRequest(), chartAnalysisDbRequest);
    List<ChartResult> chartResults =
        extClueMapper.chart(clueChartAnalysisDbRequest, userId, orgId, deptDataPermission);
  ...
  public void processTransferredCluePlanAndRecord() {
    ...
    List<Clue> clues = clueMapper.selectListByLambda(wrapper);
    List<Clue> transferredClues =
        clues.stream().filter(clue -> StringUtils.isNotBlank(clue.getTransitionId())).toList();
    Map<String, String> clueTransferMap =
        transferredClues.stream().collect(Collectors.toMap(Clue::getId, Clue::getTransitionId));
    List<String> clueIds = transferredClues.stream().map(Clue::getId).toList();
    // 记录
    LambdaQueryWrapper<FollowUpRecord> recordLambdaQueryWrapper = new LambdaQueryWrapper<>();
    recordLambdaQueryWrapper
        .in(FollowUpRecord::getClueId, clueIds)
        .eq(FollowUpRecord::getType, FollowUpPlanType.CLUE.name());
    List<FollowUpRecord> clueRecords =
        followUpRecordMapper.selectListByLambda(recordLambdaQueryWrapper);
        ...
        clueRecord -> {
          ...
          if (StringUtils.isNotBlank(clueRecord.getCustomerId())) {
            ...
            if (clueFollowDTO == null) {
              followDTOMap.put(
                  customerId,
                  ClueFollowDTO.builder()
                      .follower(clueRecord.getOwner())
                      .followerTime(clueRecord.getFollowTime())
                      .build());
            } else {
              if (clueRecord.getFollowTime() != null
                  && (clueFollowDTO.getFollowerTime() == null
                      || clueRecord.getFollowTime() > clueFollowDTO.getFollowerTime())) {
    ...
    LambdaQueryWrapper<FollowUpPlan> planLambdaQueryWrapper = new LambdaQueryWrapper<>();
    planLambdaQueryWrapper
        .in(FollowUpPlan::getClueId, clueIds)
        .eq(FollowUpPlan::getType, FollowUpPlanType.CLUE.name());
    ...
    customers.forEach(
        customer -> {
          if (customer.getFollowTime() == null
              || customer.getFollowTime() < followDTOMap.get(customer.getId()).getFollowerTime()) {
