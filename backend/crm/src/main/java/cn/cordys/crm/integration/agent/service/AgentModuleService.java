...
public class AgentModuleService extends MoveNodeService {
  ...
  @Resource private BaseMapper<AgentModule> agentModuleMapper;
  @Resource private ExtAgentModuleMapper extAgentModuleMapper;
  ...
  public void rename(AgentModuleRenameRequest request, String userId) {
    ...
    checkFileName(
        request.getName(),
        originalAgentModule.getParentId(),
        originalAgentModule.getOrganizationId(),
        originalAgentModule.getId());
    ...
    // 添加日志上下文
    String resourceName =
        Optional.ofNullable(agentModule.getName()).orElse(originalAgentModule.getName());
    OperationLogContext.setContext(
        LogContextInfo.builder()
  ...
  public void delete(List<String> ids, String userId, String orgId) {
    ...
    // 添加日志上下文
    agentModules.forEach(
        agentModule -> {
          LogDTO logDTO =
              new LogDTO(
                  agentModule.getOrganizationId(),
                  agentModule.getId(),
                  userId,
                  LogType.DELETE,
                  LogModule.AGENT,
