...
public abstract class BaseModuleLogService {
  @Resource private BaseMapper<Product> productMapper;
  ...
  public abstract List<JsonDifferenceDTO> handleLogField(
      List<JsonDifferenceDTO> differences, String orgId);

  /**
   * 处理非业务字段的自定义字段 同时会翻译其他字段的 ColumnName
   *
   * @param differences
   * @param orgId
   * @param formKey
   */
  protected List<JsonDifferenceDTO> handleModuleLogField(
      List<JsonDifferenceDTO> differences, String orgId, String formKey) {
    ModuleFormConfigDTO formConfig =
        Objects.requireNonNull(CommonBeanFactory.getBean(ModuleFormCacheService.class))
            .getBusinessFormConfig(formKey, orgId);

    List<BaseField> fields = formConfig.getFields();

    // 模块字段 map
    Map<String, BaseField> moduleFieldMap =
        fields.stream().collect(Collectors.toMap(BaseField::getId, Function.identity()));
    ...
    Map<String, BaseField> businessModuleFieldMap =
        fields.stream()
            .filter(
                f ->
                    StringUtils.isNotEmpty(f.getBusinessKey())
                        && StringUtils.isEmpty(f.getResourceFieldId()))
    ...
    differences.forEach(
        differ -> {
          boolean addDiffer =
              differ.getOldValue() != null
                  && (differ.getOldValue() instanceof String
                      ? !StringUtils.isBlank((String) differ.getOldValue())
                          && !Strings.CI.equals((String) differ.getOldValue(), "null")
                      : !((List<?>) differ.getOldValue()).isEmpty());
          if (differ.getNewValue() != null
              && (differ.getNewValue() instanceof String
                  ? !StringUtils.isBlank((String) differ.getNewValue())
                      && !Strings.CI.equals((String) differ.getNewValue(), "null")
                  : !((List<?>) differ.getNewValue()).isEmpty())) {
            addDiffer = true;
          }
          if (addDiffer) {
              ...
              (key, value) -> {
                ...
                for (BaseField subField : ((SubField) value).getSubFields()) {
                  ...
                  if (StringUtils.isNotBlank(differ.getColumn())) {
                    differColumn =
                        differ.getColumn().substring(differ.getColumn().lastIndexOf("-") + 1);
                  }
                  if (Strings.CS.equals(subBusinessKey, differColumn)
                      || Strings.CS.equals(subFieldIdKey, differColumn)) {
    ...
    Map<String, List<OptionDTO>> optionMap =
        Objects.requireNonNull(CommonBeanFactory.getBean(ModuleFormService.class))
            .getOptionMap(formConfig, optionFieldValues);
    // 子表选项字段的选项值 map
    Map<String, List<OptionDTO>> subOptionMap =
        Objects.requireNonNull(CommonBeanFactory.getBean(ModuleFormService.class))
            .getOptionMap(formConfig, optionSubFieldValues);

    Set<String> subFieldIds =
        optionSubFieldValues.stream()
            .map(BaseModuleFieldValue::getFieldId)
            .collect(Collectors.toSet());
    returnDifferences.forEach(
        differ -> {
          ...
          if (moduleField != null) {
            ...
            if (optionMap.containsKey(differ.getColumn())) {
              ...
              if (subOptionMap.containsKey(differColumn)) {
                    ...
                    (key, value) -> {
                      if (value.isSubField()) {
                        if (value instanceof SubField) {
                          ...
                          for (BaseField subField : ((SubField) value).getSubFields()) {
                            String subKey =
                                StringUtils.isNotBlank(subField.getBusinessKey())
                                    ? subField.getBusinessKey()
                                    : subField.getId();
                            if (Strings.CS.equals(subKey, finalDifferColumn)) {
                              // 设置字段值名称
                              setColumnValueName(
                                  subOptionMap.get(finalDifferColumn), differ, subField);
                            }
                          }
                        }
                      }
                ...
                // 如果prefix =
                // differ.column，则说明没有-符号，设置为Translator.get("log."+differ.getColumn()),否则设置为prefix
                if (Strings.CS.equals(prefix, differ.getColumn())) {
                  differ.setColumnName(
                      Translator.get("log." + differ.getColumn(), differ.getColumn()));
                } else {
                  differ.setColumn(prefix);
                }
                differ.setOldValueName(differ.getOldValue());
  ...
  private void setColumnValueName(
      List<OptionDTO> options, JsonDifferenceDTO differ, BaseField moduleField) {
    if (options == null) {
      // 解析各种数据
    ...
    for (OptionDTO option : options) {
      if (differ.getOldValue() instanceof String strValue
          && Strings.CS.equals(option.getId(), strValue)) {
        // 设置旧值名称
        differ.setOldValueName(option.getName());
      }
      if (differ.getNewValue() instanceof String strValue
          && Strings.CS.equals(option.getId(), strValue)) {
        // 设置新值名称
  ...
  public Object transformFieldValue(BaseField field, Object value) {
    AbstractModuleFieldResolver customFieldResolver =
        ModuleFieldResolverFactory.getResolver(field.getType());
    // 将数据库中的字符串值,转换为对应的对象值
    return customFieldResolver.transformToValue(
        field, value instanceof List ? JSON.toJSONString(value) : value.toString());
  }
  ...
  protected void setContactFieldName(JsonDifferenceDTO differ) {
    CustomerContactService customerContactService =
        CommonBeanFactory.getBean(CustomerContactService.class);
  ...
  protected void setApprovalName(JsonDifferenceDTO differ) {
    if (differ.getOldValue() != null) {
      differ.setOldValueName(
          Translator.get(
              "contract.approval_status." + differ.getOldValue().toString().toLowerCase()));
    }
    if (differ.getNewValue() != null) {
      differ.setNewValueName(
          Translator.get(
              "contract.approval_status." + differ.getNewValue().toString().toLowerCase()));
    }
  }

  protected void setBusinessTitleName(JsonDifferenceDTO differ) {
    BusinessTitleService businessTitleService =
        CommonBeanFactory.getBean(BusinessTitleService.class);
    assert businessTitleService != null;
    if (differ.getOldValue() != null) {
      String customerName =
          businessTitleService.getBusinessTitleName(differ.getOldValue().toString());
      differ.setOldValueName(customerName);
    }
    if (differ.getNewValue() != null) {
  ...
  protected void setProductName(JsonDifferenceDTO differ) {
    Optional.ofNullable(differ.getOldValue())
        .ifPresent(
            oldValue -> {
              List<String> ids = ((Collection<?>) oldValue).stream().map(String::valueOf).toList();
    ...
    Optional.ofNullable(differ.getNewValue())
        .ifPresent(
            newValue -> {
              List<String> ids = ((Collection<?>) newValue).stream().map(String::valueOf).toList();
              List<Product> products = productMapper.selectByIds(ids);
