...
public class ProductService {
  ...
  @Resource private BaseMapper<Product> productBaseMapper;
  @Resource private ExtProductMapper extProductMapper;
  @Resource private BaseService baseService;
  @Resource private ProductFieldService productFieldService;
  ...
  public PagerWithOption<List<ProductListResponse>> list(ProductPageRequest request, String orgId) {
    ...
    // 处理自定义字段选项数据
    ModuleFormConfigDTO productFormConfig =
        moduleFormCacheService.getBusinessFormConfig(FormKey.PRODUCT.getKey(), orgId);
    // 获取所有模块字段的值
    List<BaseModuleFieldValue> moduleFieldValues =
        moduleFormService.getBaseModuleFieldValues(list, ProductListResponse::getModuleFields);
    // 获取选项值对应的 option
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(productFormConfig, moduleFieldValues);
    return PageUtils.setPageInfoWithOption(page, buildList, optionMap);
  }

  private List<ProductListResponse> buildListData(List<ProductListResponse> list) {
    List<String> productIds =
        list.stream().map(ProductListResponse::getId).collect(Collectors.toList());

    Map<String, List<BaseModuleFieldValue>> productFiledMap =
        productFieldService.getResourceFieldMap(productIds, true);

    list.forEach(
        productListResponse -> {
          // 获取自定义字段
          List<BaseModuleFieldValue> productFields =
              productFiledMap.get(productListResponse.getId());
  ...
  public ProductGetResponse get(String id) {
    ...
    // 获取模块字段
    List<BaseModuleFieldValue> productFields =
        productFieldService.getModuleFieldValuesByResourceId(id);

    productGetResponse.setModuleFields(productFields);
    ModuleFormConfigDTO productFormConfig =
        moduleFormCacheService.getBusinessFormConfig(
            FormKey.PRODUCT.getKey(), product.getOrganizationId());
    Map<String, List<OptionDTO>> optionMap =
        moduleFormService.getOptionMap(productFormConfig, productFields);
    productGetResponse.setOptionMap(optionMap);
    // 附件信息
    productGetResponse.setAttachmentMap(
        moduleFormService.getAttachmentMap(productFormConfig, productFields));
    return baseService.setCreateAndUpdateUserName(productGetResponse);
  }

  @OperationLog(
      module = LogModule.PRODUCT_MANAGEMENT,
      type = LogType.ADD,
  ...
  public Product update(ProductEditRequest request, String userId, String orgId) {
    ...
    List<BaseModuleFieldValue> originCustomerFields =
        productFieldService.getModuleFieldValuesByResourceId(request.getId());
  ...
  private void updateModuleField(
      Product product, List<BaseModuleFieldValue> moduleFields, String orgId, String userId) {
  ...
  private void batchUpdateModuleField(
      List<String> productIds, List<BaseModuleFieldValue> moduleFields) {
    if (moduleFields == null) {
      // 如果为 null，则不更新
      return;
    }
  ...
  public void batchUpdate(ResourceBatchEditRequest request, String userId, String organizationId) {
    ...
    productFieldService.batchUpdate(
        request,
        field,
        products,
        Product.class,
        LogModule.PRODUCT_MANAGEMENT,
        extProductMapper::batchUpdate,
        userId,
        organizationId);
  }
  ...
  public void downloadImportTpl(HttpServletResponse response, String currentOrg) {
    new EasyExcelExporter()
        .exportMultiSheetTplWithSharedHandler(
            response,
            moduleFormService.getCustomImportHeadsNoRef(FormKey.PRODUCT.getKey(), currentOrg),
            Translator.get("product.import_tpl.name"),
            Translator.get(SheetKey.DATA),
            Translator.get(SheetKey.COMMENT),
            new CustomTemplateWriteHandler(
                moduleFormService.getAllCustomImportFields(FormKey.PRODUCT.getKey(), currentOrg)),
  ...
  public ImportResponse realImport(MultipartFile file, String currentOrg, String currentUser) {
    try {
      ...
      CustomImportAfterDoConsumer<Product, BaseResourceSubField> afterDo =
          (products, productFields, productFieldBlobs) -> {
            ...
            productFieldMapper.batchInsert(
                productFields.stream()
                    .map(field -> BeanUtils.copyBean(new ProductField(), field))
                    .toList());
            productFieldBlobMapper.batchInsert(
                productFieldBlobs.stream()
                    .map(field -> BeanUtils.copyBean(new ProductFieldBlob(), field))
                    .toList());
            // record logs
            logService.batchAdd(logs);
          };
      CustomFieldImportEventListener<Product> eventListener =
  ...
  private ImportResponse checkImportExcel(MultipartFile file, String currentOrg) {
    try {
      ...
      CustomFieldCheckEventListener eventListener =
          new CustomFieldCheckEventListener(fields, "product", "product_field", currentOrg);
      FastExcelFactory.read(file.getInputStream(), eventListener)
          .headRowNumber(1)
          .ignoreEmptyRow(true)
          .sheet()
