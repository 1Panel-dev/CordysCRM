...
public class ExportTaskCenterService {
  ...
  public List<ExportTask> list(ExportTaskCenterQueryRequest request, String userId) {
    ...
    return extExportTaskMapper.selectExportTaskList(
        request, oneDayBefore.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli(), userId);
  }
  ...
  public ResponseEntity<org.springframework.core.io.Resource> download(String taskId) {
    ...
    try {
      FileRequest request =
          new FileRequest(filePath, StorageType.LOCAL.name(), exportTask.getFileName() + ".xlsx");
  ...
  private String getFilePath(ExportTask exportTask) {
    return File.separator
        + DefaultRepositoryDir.getExportDir(exportTask.getOrganizationId())
        + File.separator
    ...
    exportTasks.forEach(
        exportTask -> {
          try {
            ...
            FileRequest request =
                new FileRequest(
                    getFilePath(exportTask),
                    StorageType.LOCAL.name(),
                    exportTask.getFileName() + ".xlsx");
