  ...
  public List<JsonDifferenceDTO> handleLogField(List<JsonDifferenceDTO> differences, String orgId) {
    ...
    for (JsonDifferenceDTO differ : differences) {
      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.INVOICE_OWNER.getBusinessKey())) {
        setUserFieldName(differ);
      } else if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.INVOICE_CONTRACT_ID.getBusinessKey())) {
        setContractFieldName(differ);
      } else if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.INVOICE_BUSINESS_TITLE_ID.getBusinessKey())) {
