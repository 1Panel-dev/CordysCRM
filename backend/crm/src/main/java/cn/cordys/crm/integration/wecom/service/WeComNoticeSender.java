...
public class WeComNoticeSender extends AbstractNoticeSender {
  ...
  private void sendWeCom(String context, NoticeModel noticeModel, String organizationId) {
    List<Receiver> receivers =
        super.getReceivers(
            noticeModel.getReceivers(), noticeModel.isExcludeSelf(), noticeModel.getOperator());
    if (CollectionUtils.isEmpty(receivers)) {
      return;
    }
    List<String> userIds = receivers.stream().map(Receiver::getUserId).distinct().toList();
    ...
    // 获取企业微信通知的配置数据
    OrganizationConfigDetail orgConfigDetailByIdAndType =
        extOrganizationConfigDetailMapper.getOrgConfigDetailByIdAndType(
            organizationConfig.getId(), ThirdDetailType.WECOM_SYNC.toString());
    if (orgConfigDetailByIdAndType == null || orgConfigDetailByIdAndType.getContent() == null) {
      log.warn("没有配置企业微信通知信息，无法发送消息");
      return;
    }
    var thirdConfigurationDTO =
        JSON.parseObject(
    ...
    if (thirdConfigurationDTO.getConfig() == null) {
      wecomThirdConfigRequest =
          JSON.parseObject(
              new String(orgConfigDetailByIdAndType.getContent()), WecomThirdConfigRequest.class);
    } else {
      wecomThirdConfigRequest =
          JSON.MAPPER.convertValue(
              thirdConfigurationDTO.getConfig(), WecomThirdConfigRequest.class);
    }
    ...
    tokenService.sendNoticeByToken(
        weComSendDTO, wecomThirdConfigRequest.getCorpId(), wecomThirdConfigRequest.getAppSecret());
  }
}
