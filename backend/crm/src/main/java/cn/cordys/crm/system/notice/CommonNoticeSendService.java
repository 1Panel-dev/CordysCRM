...
public class CommonNoticeSendService {
  ...
  public List<String> getNoticeReceiveUserIds(
      MessageTaskConfigDTO messageTaskConfigDTO, String createUser, String owner, String orgId) {
    List<String> receiveUserIds = new ArrayList<>();
    if (CollectionUtils.isNotEmpty(messageTaskConfigDTO.getUserIds())) {
      if (messageTaskConfigDTO
          .getUserIds()
          .contains(NotificationConstants.RelatedUser.CREATE_USER)) {
      ...
      List<String> normalUserIds =
          messageTaskConfigDTO.getUserIds().stream()
              .filter(
                  userId ->
                      !userId.equals(NotificationConstants.RelatedUser.CREATE_USER)
                          && !userId.equals(NotificationConstants.RelatedUser.OWNER))
              .toList();
      receiveUserIds.addAll(normalUserIds);
    }
    if (messageTaskConfigDTO.isRoleEnable()
        && CollectionUtils.isNotEmpty(messageTaskConfigDTO.getRoleIds())) {
      receiveUserIds.addAll(
          extUserMapper.selectUserIdsByRoleIds(messageTaskConfigDTO.getRoleIds()));
    }
    ...
    List<DepartmentCommander> departmentCommanderList =
        departmentCommanderMapper.selectListByLambda(
            new LambdaQueryWrapper<DepartmentCommander>()
                .in(DepartmentCommander::getDepartmentId, departmentIds));
    if (CollectionUtils.isEmpty(departmentCommanderList)) {
      return receiveUserIds;
    }
    List<String> deptCommanderUserIds =
        departmentCommanderList.stream().map(DepartmentCommander::getUserId).toList();
