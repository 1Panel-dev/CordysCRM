...
public class AttachmentService {
  ...
  @Resource private FileCommonService fileCommonService;
  ...
  public void delete(String attachmentId) {
    ...
    if (attachment == null) {
      // 删除临时文件目录
      fileCommonService.deleteFolder(
          new FileRequest(
              DefaultRepositoryDir.getTempFileDir(attachmentId), StorageType.LOCAL.name(), null),
          true);
    } else {
      // 删除正式文件
      fileCommonService.deleteFolder(
          new FileRequest(
              DefaultRepositoryDir.getTransferFileDir(
                  attachment.getOrganizationId(), attachment.getResourceId(), attachment.getId()),
    ...
    try {
      ...
      if (attachment == null) {
        ...
        responseBuilder
            .header(
                HttpHeaders.CONTENT_DISPOSITION,
                "attachment; filename*=UTF-8''" + encodeName(attachment.getName()))
            .contentLength(attachment.getSize())
            .contentType(
                isSvg(attachment.getName())
                    ? MediaType.parseMediaType("image/svg+xml")
                    : MediaType.parseMediaType("application/octet-stream"));
      }
      return responseBuilder.body(new InputStreamResource(fileStream));
    } catch (Exception e) {
  ...
  public void processTemp(UploadTransferRequest transferRequest) {
    ...
    transferRequest.getTempFileIds().stream()
        .filter(tempFileId -> !transferredIds.contains(tempFileId))
        .forEach(
            tempFileId -> {
              // transfer new pic
              FileRequest request =
                  new FileRequest(
                      DefaultRepositoryDir.getTmpDir() + "/" + tempFileId,
                      StorageType.LOCAL.name(),
                      null);
              ...
              if (!CollectionUtils.isEmpty(folderTempFiles)) {
                ...
                FileCopyRequest copyRequest =
                    new FileCopyRequest(
                        DefaultRepositoryDir.getTempFileDir(tempFileId),
                        DefaultRepositoryDir.getTransferFileDir(
                            transferRequest.getOrganizationId(),
                            transferRequest.getResourceId(),
                            tempFileId),
                        tempFile.getName());
    ...
    List<String> removedIds =
        transferredIds.stream()
            .filter(aId -> !transferRequest.getTempFileIds().contains(aId))
            .toList();
   ...
   */
  public void batchCopyOfIdMap(
      Map<String, String> oldOfNewIdMap, String targetId, String currentUser) {
    LambdaQueryWrapper<Attachment> queryWrapper = new LambdaQueryWrapper<>();
    ...
    oldOfNewIdMap.forEach(
        (oId, nId) -> {
          ...
          FileCopyRequest copyRequest =
              new FileCopyRequest(
                  DefaultRepositoryDir.getTransferFileDir(
                      oldAttachment.getOrganizationId(), oldAttachment.getResourceId(), oId),
                  DefaultRepositoryDir.getTransferFileDir(
                      oldAttachment.getOrganizationId(), targetId, nId),
