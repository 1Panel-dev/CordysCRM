...
public class RoleService {
  ...
  @Resource private BaseMapper<RoleScopeDept> roleScopeDeptMapper;
  @Resource private BaseMapper<RolePermission> rolePermissionMapper;
  ...
  public List<RoleListResponse> getRoleListResponses(String orgId) {
    ...
    List<RoleListResponse> roleListResponseList =
        JSON.parseArray(JSON.toJSONString(roles), RoleListResponse.class);
    // 翻译内置角色名称
  ...
  private List<String> getDeptIdsByRoleId(String roleId) {
    ...
    return roleScopeDeptMapper.select(example).stream()
  ...
  public List<String> getDeptIdsByRoleIds(List<String> roleIds) {
    ...
    return roleScopeDeptList.stream()
        .map(RoleScopeDept::getDepartmentId)
        .distinct()
        .collect(Collectors.toList());
  }
  ...
  public Role add(RoleAddRequest request, String userId, String orgId) {
    ...
    // 创建默认仅可查看本人数据
    role.setDataScope(
        Optional.ofNullable(request.getDataScope()).orElse(RoleDataScope.SELF.name()));
    // 校验名称重复
    checkAddExist(role);
    roleMapper.insert(role);

    // 配置指定部门权限
    if (Strings.CS.equals(request.getDataScope(), RoleDataScope.DEPT_CUSTOM.name())
        && CollectionUtils.isNotEmpty(request.getDeptIds())) {
    ...
    OperationLogContext.setContext(
        LogContextInfo.builder().resourceId(role.getId()).modifiedValue(role).build());
  ...
  public Role update(RoleUpdateRequest request, String userId, String orgId) {
    ...
    // 配置指定部门权限
    if (Strings.CS.equals(dataScope, RoleDataScope.DEPT_CUSTOM.name())
        && request.getDeptIds() != null) {
      // 先删除
      deleteRoleScopeDeptByRoleId(role.getId());
    ...
    RoleLogDTO modifiedRoleLogDTO =
        BeanUtils.copyBean(new RoleLogDTO(), roleMapper.selectByPrimaryKey(request.getId()));
    if (request.getDeptIds() != null) {
    ...
    if (request.getPermissions() != null) {
      final Set<String> newPermissionIdSet =
          request.getPermissions().stream()
              .filter(p -> BooleanUtils.isTrue(p.getEnable()))
              .map(PermissionUpdateRequest::getId)
              .collect(Collectors.toSet());

      final Set<String> originPermissionIdSet =
          originPermissionIds == null ? Collections.emptySet() : new HashSet<>(originPermissionIds);
  ...
  private List<PermissionDefinitionItem> getPermissionDefinitionItems(
      Role role, Set<String> permissionIds) {
    // 获取所有的权限
      ...
      for (PermissionDefinitionItem secondLevel : children) {
        ...
        for (Permission p : permissions) {
          ...
          if (permissionIds.contains(p.getId())
              || (role != null
                  && Strings.CS.equals(role.getId(), InternalRole.ORG_ADMIN.getValue()))) {
            p.setEnable(true);
          } else {
  ...
  public List<PermissionDefinitionItem> getPermissionDefinitions() {
    ...
    try {
      ...
      while (urls.hasMoreElements()) {
        ...
        List<PermissionDefinitionItem> temp =
            JSON.parseArray(content, PermissionDefinitionItem.class);
  ...
  public Set<String> getPermissionIds(List<String> roleIds) {
    return getPermissions(roleIds).stream()
        .map(RolePermission::getPermissionId)
        .collect(Collectors.toSet());
  }
