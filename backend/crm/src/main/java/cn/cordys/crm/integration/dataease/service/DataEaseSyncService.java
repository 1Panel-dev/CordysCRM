...
public class DataEaseSyncService {
  ...
  @Resource private ExtDepartmentMapper extDepartmentMapper;
  @Resource private ExtOrganizationMapper extOrganizationMapper;
  ...
  @Resource private DataEaseService dataEaseService;
  ...
  public void syncDataEase(String orgId) {
    ...
    if (thirdConfig == null
        || StringUtils.isAnyBlank(
            thirdConfig.getDeAccessKey(),
            thirdConfig.getDeSecretKey(),
            thirdConfig.getDeOrgID(),
            thirdConfig.getRedirectUrl())) {
      return;
    }
    try {
  ...
  private void syncUser(DeTempResourceDTO deTempResourceDTO) {
    ...
    // 记录CRM用户ID和用户的映射关系
    Map<String, UserDTO> crmUserMap =
        crmUsers.stream().collect(Collectors.toMap(UserDTO::getId, Function.identity()));
    ...
    // 获取所有的角色
    List<String> roleIds =
        crmRoles.stream().map(RoleListResponse::getId).collect(Collectors.toList());

    // 角色名称集合
    Set<String> roleNameSet =
        crmRoles.stream().map(RoleListResponse::getName).collect(Collectors.toSet());
  ...
  private void setSyncUserTempParam(DeTempResourceDTO deTempResourceDTO, List<String> roleIds) {
    ...
    Map<String, List<UserRole>> userRoleMap =
        roleService.getUserRoleByRoleIds(roleIds).stream()
    ...
    Map<String, RoleListResponse> crmRoleMap =
        crmRoles.stream().collect(Collectors.toMap(RoleListResponse::getId, Function.identity()));
  ...
  private void updateUser(
      DeTempResourceDTO deTempResourceDTO,
      Map<String, UserDTO> crmUserMap,
      Set<String> roleNameSet,
      UserPageDTO deUser) {
    ...
    if (!Strings.CS.equals(deUser.getName(), crmUser.getName())
        || !Strings.CS.equals(deUser.getEmail(), crmUser.getEmail())) {
      needUpdate = true;
    ...
    Set<String> crmRoleNames =
        userCrmRoles.stream().map(RoleListResponse::getName).collect(Collectors.toSet());
    Set<String> deRoleNames =
        roleItems.stream().map(OptionDTO::getName).collect(Collectors.toSet());

    // 获取DE中存在的CRM角色
    Set<String> linkDeRoleNames =
        deRoleNames.stream().filter(roleNameSet::contains).collect(Collectors.toSet());
    if (!linkDeRoleNames.equals(crmRoleNames)) {
      // 如果DE中的角色不包含CRM中的角色，则需要更新
      needUpdate = true;
      List<String> updateRoleIds =
          crmRoleNames.stream()
              .map(crmRoleName -> roleMap.get(crmRoleName).getId())
              .collect(Collectors.toList());
      // DE中非CRM的角色保留
      List<String> deRoleIds =
          deRoleNames.stream()
    ...
    UserVariableTempDTO userVariableTempDTO =
        getUserVariableTempDTO(
            userCrmRoles, customDeptRoleDeptMap, tree, rolePermissionMap, crmUser);
    ...
    for (DataScopeVariable value : DataScopeVariable.values()) {
      ...
      // 记录待更新的用户变量
      UserCreateRequest.Variable variable =
          getCreateVariable(
              deTempResourceDTO.getSysVariableMap(),
              value.name(),
              deTempResourceDTO.getVariableValueMap(),
              crmDataScope);
    ...
    for (DataScopeDeptVariable value : DataScopeDeptVariable.values()) {
      ...
      List<String> crmDeptIds =
          Optional.ofNullable(userVariableTempDTO.getUserDeptIdMap().get(value.name()))
              .orElse(List.of());
      ...
      if (CollectionUtils.isNotEmpty(crmDeptIds)) {
        // 记录待更新的用户变量
        UserCreateRequest.Variable variable =
            getCreateVariable(sysVariableMap, value.name(), variableValueMap, crmDeptIds);
        variables.add(variable);
      }
    }
  ...
  private void addUsers(DeTempResourceDTO deTempResourceDTO) {
    List<UserDTO> enableUsers =
        deTempResourceDTO.getCrmUsers().stream().filter(UserDTO::getEnable).toList();
    Map<String, List<String>> customDeptRoleDeptMap = deTempResourceDTO.getCustomDeptRoleDeptMap();
    Map<String, List<UserRole>> userRoleMap = deTempResourceDTO.getCrmUserRoleMap();
    List<BaseTreeNode> tree = deTempResourceDTO.getDeptTree();
    Map<String, RoleListResponse> crmRoleMap = deTempResourceDTO.getCrmRoleMap();
    ...
    enableUsers.stream()
        .filter(
            crmUser ->
                crmUser.getEnable() && !deTempResourceDTO.getDeUserIds().contains(crmUser.getId()))
        .forEach(
            crmUser -> {
              ...
              List<RoleListResponse> userCrmRoles =
                  getUserCrmRoles(userRoleMap, crmRoleMap, crmUser.getId());
              UserVariableTempDTO userVariableTempDTO =
                  getUserVariableTempDTO(
                      userCrmRoles, customDeptRoleDeptMap, tree, rolePermissionMap, crmUser);
              ...
              userCreateRequest.setVariables(
                  getCreateVariables(deTempResourceDTO, userVariableTempDTO));
              try {
  ...
  private List<UserCreateRequest.Variable> getCreateVariables(
      DeTempResourceDTO deTempResourceDTO, UserVariableTempDTO userVariableTempDTO) {
    List<UserCreateRequest.Variable> variables = new ArrayList<>();
    Map<String, SysVariableDTO> sysVariableMap = deTempResourceDTO.getSysVariableMap();
    Map<String, Map<String, String>> variableValueMap = deTempResourceDTO.getVariableValueMap();
    for (int i = 0; i < DataScopeVariable.values().length; i++) {
      ...
      String crmDataScope = userVariableTempDTO.getScopeValueMap().get(value.name());
      UserCreateRequest.Variable variable =
          getCreateVariable(sysVariableMap, value.name(), variableValueMap, crmDataScope);
      variables.add(variable);
    }
  ...
  private Map<String, List<String>> getCustomDeptRoleDeptMap(List<RoleListResponse> crmRoles) {
    // 获取自定义部门角色ID
    List<String> customDeptRolesIds =
        crmRoles.stream()
            .filter(
                role -> Strings.CS.equalsAny(role.getDataScope(), RoleDataScope.DEPT_CUSTOM.name()))
            .map(RoleListResponse::getId)
            .collect(Collectors.toList());

    List<RoleScopeDept> roleScopeDeptList =
        roleService.getRoleScopeDeptByRoleIds(customDeptRolesIds);
    Map<String, List<String>> customDeptRoleDeptMap = new HashMap<>();
    roleScopeDeptList.forEach(
        roleScopeDept -> {
  ...
  private UserCreateRequest.Variable getCreateVariable(
      Map<String, SysVariableDTO> sysVariableMap,
      String variableName,
  ...
  private UserCreateRequest.Variable getCreateVariable(
      ...
      String variableName,
      Map<String, Map<String, String>> variableValueMap,
      List<String> variableValueNames) {
    SysVariableDTO sysVariable = sysVariableMap.get(variableName);
    String variableId = sysVariable.getId();
    UserCreateRequest.Variable variable = new UserCreateRequest.Variable();
    variable.setVariableId(variableId);
  ...
  private UserVariableTempDTO getUserVariableTempDTO(
      List<RoleListResponse> userCrmRoles,
    ...
    for (DataScopeVariable value : DataScopeVariable.values()) {
      // 获取有对应的权限的角色
      List<RoleListResponse> permissionRoles =
          userCrmRoles.stream()
              .filter(
                  role ->
                      rolePermissionMap.get(role.getId()) != null
      ...
      if (Strings.CS.equals(crmDataScope, RoleDataScope.DEPT_CUSTOM.name())) {
        List<String> crmDataScopeDeptIds =
            getCrmDataScopeDeptIds(tree, permissionRoles, customDeptRoleDeptMap, orgUser);
        // 记录用户有权限的部门ID
        userVariableTempDTO
            .getUserDeptIdMap()
            .put(value.getDataScopeDeptVariable().name(), crmDataScopeDeptIds);
      }
    }
   ...
   * @return
   */
  private List<RoleListResponse> getUserCrmRoles(
      Map<String, List<UserRole>> userRoleMap,
      Map<String, RoleListResponse> crmRoleMap,
      String userId) {
    ...
    return userRoleMap.get(userId).stream()
  ...
  private String getCrmDataScope(List<RoleListResponse> userCrmRoles) {
    ...
    for (RoleListResponse userCrmRole : userCrmRoles) {
      ...
      } else if (Strings.CS.equalsAny(
          userCrmRole.getDataScope(),
          RoleDataScope.DEPT_CUSTOM.name(),
  ...
  private List<String> getCrmDataScopeDeptIds(
      List<BaseTreeNode> tree,
      List<RoleListResponse> userCrmRoles,
      Map<String, List<String>> customDeptRoleDeptMap,
      UserDTO orgUser) {
    ...
    for (RoleListResponse userCrmRole : userCrmRoles) {
      if (Strings.CS.equals(userCrmRole.getDataScope(), RoleDataScope.DEPT_CUSTOM.name())) {
        if (CollectionUtils.isNotEmpty(customDeptRoleDeptMap.get(userCrmRole.getId()))) {
          ...
          userCrmRole.getDataScope(), RoleDataScope.DEPT_AND_CHILD.name())) {
  ...
  private void syncRole(DeTempResourceDTO deTempResourceDTO) {
    ...
    Map<String, RoleDTO> roleMap =
        roles.stream().collect(Collectors.toMap(RoleDTO::getName, Function.identity()));
    // 记录角色名和角色的映射
  ...
  private void syncSysVariable(DeTempResourceDTO deTempResourceDTO) {
    ...
    for (DataScopeVariable value : DataScopeVariable.values()) {
      if (!sysVariableMap.containsKey(value.name())) {
        // 创建数据权限系统变量
        SysVariableDTO dataScopeVariable =
            createDataScopeVariable(dataEaseClient, value.name(), variableValueMap);
        sysVariableMap.put(value.name(), dataScopeVariable);
      } else {
        // 同步数据权限系统变量值
        SysVariableDTO sysVariable = sysVariableMap.get(value.name());
        Map<String, SysVariableValueDTO> valueMap =
            dataEaseClient.listSysVariableValue(sysVariable.getId()).stream()
                .collect(Collectors.toMap(SysVariableValueDTO::getValue, Function.identity()));
        ...
        valueMap.forEach(
            (variableValueId, sysVariableValue) ->
                variableValueIdNameMap.put(sysVariableValue.getValue(), sysVariableValue.getId()));

        List<String> dataScopeValues =
            List.of(
                RoleDataScope.ALL.name(),
        ...
        for (String dataScopeValue : dataScopeValues) {
          if (!valueMap.containsKey(dataScopeValue)) {
            // 创建缺失的变量
            SysVariableValueCreateRequest variableValueCreateRequest =
                new SysVariableValueCreateRequest();
            variableValueCreateRequest.setSysVariableId(sysVariable.getId());
            variableValueCreateRequest.setValue(dataScopeValue);
            try {
              SysVariableValueDTO sysVariableValue =
                  dataEaseClient.createSysVariableValue(variableValueCreateRequest);
              variableValueIdNameMap.put(sysVariableValue.getValue(), sysVariableValue.getId());
            } catch (Exception e) {
              log.error(e.getMessage(), e);
            }
    ...
    List<String> deptIds =
        extDepartmentMapper.selectAllDepartmentIds(deTempResourceDTO.getCrmOrgId());
    Set<String> deptIdSet = new HashSet<>(deptIds);
    for (DataScopeDeptVariable value : DataScopeDeptVariable.values()) {
      if (!sysVariableMap.containsKey(value.name())) {
        // 创建数据权限部门变量
        try {
          SysVariableDTO dataScopeDeptVariable =
              createDataScopeDeptVariable(dataEaseClient, value.name(), deptIds, variableValueMap);
        ...
        List<String> addValues =
            deptIds.stream().filter(deptId -> !valueMap.containsKey(deptId)).toList();
  ...
  private SysVariableDTO createDataScopeVariable(
    ...
    SysVariableValueCreateRequest sysVariableValueCreateRequest =
        new SysVariableValueCreateRequest();
    ...
    for (String dataScopeValue : dataScopeValues) {
      ...
      try {
        SysVariableValueDTO sysVariableValue =
            dataEaseClient.createSysVariableValue(sysVariableValueCreateRequest);
        variableValueIdNameMap.put(sysVariableValue.getValue(), sysVariableValue.getId());
      } catch (Exception e) {
        log.error(e.getMessage(), e);
      }
  ...
  private SysVariableDTO createDataScopeDeptVariable(
      DataEaseClient dataEaseClient,
      String sysVariableName,
      List<String> deptIds,
      Map<String, Map<String, String>> variableValueMap) {
