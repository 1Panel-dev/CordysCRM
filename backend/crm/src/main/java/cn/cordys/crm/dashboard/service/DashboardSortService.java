...
public abstract class DashboardSortService {
   ...
   */
  public NodeMoveRequest getNodeMoveRequest(
      String moveId, String targetId, String moveMode, boolean isDesc) {
   ...
   */
  public MoveNodeSortDTO getNodeSortDTO(
      String sortRangeId,
      NodeMoveRequest request,
      Function<String, DropNode> selectIdNodeFunc,
      Function<NodeSortQueryParam, DropNode> selectPosNodeFunc) {
    if (Strings.CS.equals(request.getDragNodeId(), request.getDropNodeId())) {
      // 两种节点不能一样
      throw new GenericException(
          Translator.get("invalid_parameter") + ": drag node  and drop node");
    }

    DropNode dragNode = selectIdNodeFunc.apply(request.getDragNodeId());
    if (dragNode == null) {
      throw new GenericException(
          Translator.get(DRAG_NODE_NOT_EXIST) + ":" + request.getDragNodeId());
    }

    DropNode dropNode = selectIdNodeFunc.apply(request.getDropNodeId());
    if (dropNode == null) {
      throw new GenericException(
          Translator.get(DRAG_NODE_NOT_EXIST) + ":" + request.getDropNodeId());
    }
  ...
  public void sort(MoveNodeSortDTO sortDTO) {
    ...
    NodeSortCountResultDTO countResultDTO =
        NodeSortUtils.countModuleSort(
