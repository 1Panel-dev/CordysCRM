...
public class ContractPaymentPlanLogService extends BaseModuleLogService {
  ...
  public List<JsonDifferenceDTO> handleLogField(List<JsonDifferenceDTO> differences, String orgId) {
    differences =
        super.handleModuleLogField(differences, orgId, FormKey.CONTRACT_PAYMENT_PLAN.getKey());

    for (JsonDifferenceDTO differ : differences) {
      if (Strings.CS.equals(
          differ.getColumn(), BusinessModuleField.CONTRACT_PAYMENT_PLAN_OWNER.getBusinessKey())) {
        setUserFieldName(differ);
      } else if (Strings.CS.equals(
          differ.getColumn(),
          BusinessModuleField.CONTRACT_PAYMENT_PLAN_CONTRACT.getBusinessKey())) {
  ...
  protected void setPlanStatusFieldName(JsonDifferenceDTO differ) {
    ...
    if (differ.getOldValue() != null) {
      String oldName =
          Translator.get(
              "contract.payment_plan.status." + differ.getOldValue().toString().toLowerCase());
      differ.setOldValueName(oldName);
    }
    if (differ.getNewValue() != null) {
      String newName =
          Translator.get(
              "contract.payment_plan.status." + differ.getNewValue().toString().toLowerCase());
