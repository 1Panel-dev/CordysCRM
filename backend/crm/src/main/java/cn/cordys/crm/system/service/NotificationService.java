...
public class NotificationService {
  ...
  @Resource private BaseMapper<Notification> notificationMapper;
  @Resource private ExtNotificationMapper extNotificationMapper;
  @Resource private StringRedisTemplate stringRedisTemplate;
  @Resource private SendModuleService sendModuleService;
  @Resource private MessagePublisher messagePublisher;

  private static void buildSourceCount(
      List<NotificationDTO> notifications, List<OptionCountDTO> options) {
    Map<String, Integer> countMap = new HashMap<>();
    Map<String, List<Notification>> resourceMap =
    ...
    resourceMap.forEach(
        (k, v) -> {
          if (k.contains(NotificationConstants.Module.CUSTOMER)) {
            ...
            countMap.merge(
                NotificationConstants.Type.ANNOUNCEMENT_NOTICE.name(), v.size(), Integer::sum);
          }
        });
  ...
  public List<NotificationDTO> listNotification(
      NotificationRequest notificationRequest, String userId, String organizationId) {
    buildParam(notificationRequest, userId);
    List<NotificationDTO> notifications =
        extNotificationMapper.listNotification(notificationRequest, organizationId);
    notifications.forEach(
        notification -> notification.setContentText(new String(notification.getContent())));
  ...
  public int readAll(String organizationId, String userId) {
    ...
    // 所有这个用户的公告都不再显示
    Set<String> values =
        stringRedisTemplate.opsForZSet().range(USER_ANNOUNCE_PREFIX + userId, 0, -1);
  ...
  public List<OptionCountDTO> countNotification(
      NotificationRequest notificationRequest, String organizationId, String userId) {
  ...
  public List<NotificationDTO> listLastNotification(String userId, String organizationId) {
    ...
    List<NotificationDTO> notifications =
        extNotificationMapper.selectLastList(userId, organizationId, modules);
    notifications.forEach(
        notification -> notification.setContentText(new String(notification.getContent())));
    return notifications;
  }
  ...
  public List<NotificationDTO> listLastAnnouncement(String userId, String organizationId) {
    List<NotificationDTO> notifications =
        extNotificationMapper.selectLastAnnouncementList(userId, organizationId);
    notifications.forEach(
        notification -> notification.setContentText(new String(notification.getContent())));
    return notifications;
  }
