...
- 代码必须包含详略得当的注释，确保维护者能够理解关键业务意图和实现取舍。
- 注释重点说明“为什么这样做”，包括业务规则、权限原因、兼容逻辑、边界条件和非直观算法。
- 不为赋值、判空、普通循环或方法名已经表达清楚的代码添加逐行翻译式注释。
- 公共扩展点、复杂 Service 方法和容易误用的工具方法应使用 Javadoc 说明职责、参数、返回值及必要的异常条件。
- Controller、DTO 的字段注释应与实际接口语义一致，不复制已经过时的产品描述。
- Mapper XML 或复杂 SQL 应说明查询目的、特殊关联条件和性能考虑，不逐句解释 SQL 语法。
- 修改逻辑时同步更新相关注释；错误或过期的注释必须删除，不能保留作“历史记录”。
- 校验、数据查询、业务计算、持久化、事件或日志处理、结果组装等逻辑块之间应保留一个空行。
- 连续且不可分割的语句保持紧凑，不在每条语句之间机械插入空行。
- 单个方法出现过多逻辑块或需要大量段落注释时，应优先提取语义清晰的私有方法。
...
- 复杂逻辑具有说明意图和边界的有效注释，且不存在过期注释。
- 方法内容已按逻辑块合理留白，没有连续堆叠无关步骤。
